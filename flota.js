...
const MOD='flota', COLL='accesos', TITLE='Ingresos';
...
function paint(){
  ...
  <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;flex-wrap:wrap;flex-shrink:0">
    ...
    ${p.canAdd?`<button id="_add" style="padding:8px 18px;background:${c.green};color:#fff;border:none;border-radius:20px;font-size:12px;font-weight:700;cursor:pointer">+ Nuevo embalaje</button>`:''}
  </div>
...
function rLista(){
  ...
  const th=cols.map(col=>{const sd=_sortCol===col.id?(_sortDir==='asc'?' ▲':' ▼'):'';return`<th style="padding:8px 12px;text-align:left;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:${c.t3};border-bottom:1px solid ${c.border};white-space:nowrap;cursor:pointer;background:${c.bg2};user-select:none" onclick="window._flSort('${col.id}')">${col.label}${sd}</th>`;}).join('');
...
function _cell(d,id,p,c){
  switch(id){
    ...
    case'acciones':return`<td style="padding:8px 12px"><div style="display:flex;gap:3px;flex-wrap:wrap">
      <button style="font-size:11px;background:none;border:1px solid ${c.border};border-radius:6px;padding:3px 7px;cursor:pointer" onclick="window._flPrint('${d.id}')" title="Imprimir">🖨</button>
      <button style="font-size:11px;background:${c.purple};color:#fff;border:none;border-radius:6px;padding:3px 7px;cursor:pointer" onclick="window._flCut('${d.id}')" title="Troquelado">✂</button>
      <button style="font-size:11px;background:none;border:1px solid ${c.border};border-radius:6px;padding:3px 7px;cursor:pointer" onclick="window._flTrack('${d.id}')" title="Tracking">📡</button>
      ${p.canEdit?`<button style="font-size:11px;background:#fef3c7;color:#92400e;border:1px solid #fde68a;border-radius:6px;padding:3px 7px;cursor:pointer" onclick="window._flEdit('${d.id}')" title="Editar">✏️</button>`:''}
      ${d.estado==='EN_RECINTO'&&p.canEdit?`<button style="font-size:11px;background:#fffbeb;color:${c.amber};border:1px solid #fde68a;border-radius:6px;padding:3px 7px;cursor:pointer" onclick="window._flSal('${d.id}')" title="Salida">↩</button>`:''}
      ${d.estado==='SALIDA'&&p.canEdit?`<button style="font-size:11px;background:#ecfdf5;color:${c.green};border:1px solid #a7f3d0;border-radius:6px;padding:3px 7px;cursor:pointer" onclick="window._flReact('${d.id}')" title="Reactivar">↺</button>`:''}
      ${p.canDel?`<button style="font-size:11px;background:#fef2f2;color:${c.red};border:1px solid #fecaca;border-radius:6px;padding:3px 7px;cursor:pointer" onclick="window._flDel('${d.id}')" title="Eliminar">🗑</button>`:''}
    </div></td>`;
...
function openModal(editId=null){
  ...
  m.innerHTML=`<div style="background:${c.card};border-radius:20px;border:1px solid ${c.border};box-shadow:0 20px 60px rgba(0,0,0,.14);width:100%;max-width:660px;max-height:92vh;overflow-y:auto;color:${c.text}">
    ...
    <div style="display:flex;gap:8px;justify-content:flex-end;padding:12px 22px 18px;border-top:1px solid ${c.border}">
      ...
      <button id="_sv" style="padding:9px 22px;background:${c.blue};color:#fff;border:none;border-radius:10px;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit">${editId?'Guardar':'Crear Embalaje'}</button>
    </div></div>`;
...
// ═══ WINDOW BINDINGS ═══
window._flSort=(col)=>{if(_sortCol===col)_sortDir=_sortDir==='asc'?'desc':'asc';else{_sortCol=col;_sortDir='asc';}applyF();rc();};
window._flDet=(id)=>openModal(id);
window._flEdit=(id)=>openModal(id);
window._flDel=(id)=>_del(id);
window._flSal=(id)=>_sal(id);
window._flReact=(id)=>_react(id);
window._flPrint=(id)=>toast('🖨 Impresión (módulo impresión)','#2c5ee8');
window._flCut=(id)=>toast('✂ Troquelado (próximamente)','#7c3aed');
window._flTrack=(id)=>toast('📡 Tracking (próximamente)','#2c5ee8');
window._flNewAl=()=>openAlM();
window._flEditAl=(id)=>openAlM(id);
window._flDelAl=async(id)=>{try{const{fsDel}=await import('./firestore.js');await fsDel(`${COLL}_especiales/${id}`);toast('Eliminada','#f59e0b');}catch(e){toast(_s('error'),'#ef4444');}};
