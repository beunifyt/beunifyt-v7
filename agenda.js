...
function cell(d,id,p,c){
  switch(id){
    ...
    case'titulo':return`<td style="padding:8px 12px;font-weight:600">${safeHtml(d.titulo||'–')}</td>`;
    case'fecha':return`<td style="padding:8px 12px;font-size:11px;white-space:nowrap">${formatDate(d.fecha)}</td>`;
    case'hora':return`<td style="padding:8px 12px;font-size:11px">${safeHtml(d.hora||'–')}</td>`;
    case'tipo':return`<td style="padding:8px 12px;font-size:11px">${safeHtml(d.tipo||'–')}</td>`;
    case'servicio':return`<td style="padding:8px 12px;font-size:11px">${safeHtml(d.servicio||'–')}</td>`;
...
function openModal(editId=null){
  ...
  m.querySelector('#_sv').onclick=async()=>{
    ...
    if(!fd.titulo){toast('Título requerido','#ef4444');return;}
    const dup=!editId&&_data.find(d=>(d.titulo||'')===(fd.titulo||'')&&(d.fecha||'')===(fd.fecha||''));
    if(dup&&!confirm('⚠️ "'+fd.titulo+'" en '+fd.fecha+' ya existe. ¿Duplicar?'))return;
    if(!editId){if(fd.pos){fd.pos=parseInt(fd.pos)||nextPos(_data);}else{fd.pos=nextPos(_data);}}
    try{
      if(editId){const{fsUpdate}=await import('./firestore.js');await fsUpdate(`${COLL}/${editId}`,fd);_log('edit',fd.titulo,'Edición');}
      else{fd.fecha=fd.fecha||nowLocal();fd.estado=fd.estado||'PENDIENTE';fd.creadoPor=_u.uid;const{fsAdd}=await import('./firestore.js');await fsAdd(COLL,fd);_log('new',fd.titulo,'Nueva reserva');}
