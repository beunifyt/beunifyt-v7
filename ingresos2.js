...
const MOD='ingresos2', COLL='accesos', TITLE='Ingresos';
...
function paint(){
  ...
  <div style="flex:1;display:flex;flex-direction:column;overflow:hidden;background:${c.card};border:1px solid ${c.border};border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.04)">
    ...
    <div style="display:flex;align-items:center;gap:6px;padding:8px 12px;border-bottom:1px solid ${c.border};overflow-x:auto;flex-shrink:0;scrollbar-width:none">
      <div style="flex:1;min-width:140px;display:flex;align-items:center;background:${c.bg2};border:1px solid ${c.border};border-radius:20px;padding:4px 10px;gap:6px"><span style="font-size:13px;opacity:.5">🔍</span><input id="_q" type="search" placeholder="Matrícula, conductor, empresa..." value="${safeHtml(_q)}" style="border:none;background:transparent;flex:1;font-size:12px;outline:none;color:${c.text};font-family:inherit"></div>
...
function rLista(){
  ...
  const th=cols.map(col=>{const sd=_sortCol===col.id?(_sortDir==='asc'?' ▲':' ▼'):'';return`<th style="padding:8px 12px;text-align:left;font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:${c.t3};border-bottom:1px solid ${c.border};white-space:nowrap;cursor:pointer;background:${c.bg2};user-select:none" onclick="window._i2Sort('${col.id}')">${col.label}${sd}</th>`;}).join('');
...
window._i2Det=(id)=>openModal(id);
window._i2Edit=(id)=>openModal(id);
...
window._i2DelAl=async(id)=>{try{const{fsDel}=await import('./firestore.js');await fsDel(`${COLL}_especiales/${id}`);toast('Eliminada','#f59e0b');}catch(e){toast(_s('error'),'#ef4444');}};
