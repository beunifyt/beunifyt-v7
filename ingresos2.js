...
function _loadColCfg() {
  try { return JSON.parse(localStorage.getItem('be_ing2_cols') || 'null'); } catch(e) { return null; }
}
function _saveColCfg(cfg) {
  try { localStorage.setItem('be_ing2_cols', JSON.stringify(cfg)); } catch(e) {}
}

let _ingColsVis   = _loadColCfg()?.vis   || [...ING_COLS_DEFAULT];
let _ingColsOrder = _loadColCfg()?.order || [...ING_COLS_DEFAULT];
let _ingColTpls   = JSON.parse(localStorage.getItem('be_ing2_col_tpls') || '{}');

function _saveColTpls() {
  try { localStorage.setItem('be_ing2_col_tpls', JSON.stringify(_ingColTpls)); } catch(e) {}
}
...
function _loadCamposState() {
  try { return JSON.parse(localStorage.getItem('be_ing2_campos') || 'null'); } catch(e) { return null; }
}
...
function _saveCamposState() {
  try { localStorage.setItem('be_ing2_campos', JSON.stringify(_camposState)); } catch(e) {}
...
export function renderIngresos2() {
  ...
  if (iF.fecha2)   items = items.filter(i => i.entrada?.startsWith(iF.fecha2));
  if (iF.hall2)    items = items.filter(i => i.hall === iF.hall2 || (i.halls||[]).includes(iF.hall2));
  if (iF.activos2) items = items.filter(i => !i.salida);

  const sub = iF._sub2 || 'lista';
  ...
  document.getElementById('tabContent').innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:3px;padding:4px 0;flex-wrap:wrap;min-height:34px;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none">
      ...
      ].map(([s,l]) => `<button class="btn btn-sm ${sub===s?'btn-p':'btn-gh'}" onclick="iF._sub2='${s}';renderIngresos2()">${l}</button>`).join('')}
      ...
      ${sub !== 'historial' && sub !== 'campos' && canClean()
        ? `<button class="btn btn-sm" onclick="cleanTab('ingresos2')">🗑 Limpiar</button>` : ''}
      ${sub !== 'historial' && isSA()
        ? `<button class="btn btn-danger btn-sm" onclick="vaciarTab('ingresos2')">💥 Vaciar</button>` : ''}
      ${sub === 'lista'
        ? `<button class="btn btn-gh btn-sm" onclick="_openColPanel2()" title="Gestionar columnas">⚙ Cols</button>` : ''}
    </div>
    ...
    <div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;border-bottom:1px solid var(--border);padding-bottom:4px">
      <div class="sbox" style="flex:1;min-width:120px">
        ...
        <input type="search" placeholder="Pos, matrícula, nombre..." value="${esc(iF.q2||'')}"
          id="srch-ingresos" oninput="iF.q=this.value;debounceSearch('ingresos',renderIngresos2)">
      </div>
      <input type="date" value="${iF.fecha2||''}"
        oninput="iF.fecha2=this.value;debounceSearch('ingresos2-date',renderIngresos2)"
        style="height:32px;padding:4px 8px;font-size:11px;width:auto;min-width:110px;max-width:130px">
      <span class="pill" style="border:1.5px solid ${iF.activos2?'var(--blue)':'var(--border)'};background:${iF.activos2?'var(--blue)':'var(--bg2)'};color:${iF.activos2?'#fff':'var(--text3)'};cursor:pointer"
        onclick="iF.activos2=!iF.activos2;renderIngresos2()">Solo activos</span>
      ${iF.q2||iF.fecha2||iF.hall2||iF.activos2
        ? `<span class="pill pill-r" onclick="iF.q='';iF.fecha2='';iF.hall2='';iF.activos2=false;renderIngresos2()">✕</span>` : ''}
    ...
    <div style="display:flex;flex-wrap:wrap;gap:3px;margin-bottom:6px">
      <span class="pill" style="font-size:10px;font-weight:700;padding:3px 8px;cursor:pointer;
        border:1.5px solid ${!iF.hall2?'#7dd3fc':'#93c5fd'};background:${!iF.hall2?'#e0f2fe':'#dbeafe'};color:${!iF.hall2?'#0369a1':'#1e40af'}"
        onclick="iF.hall2='';renderIngresos2()">Todos</span>
      ${getRecintoHalls().map(h=>`<span class="pill" style="font-size:10px;font-weight:700;padding:3px 8px;cursor:pointer;
        background:${iF.hall2===h?'#3b82f6':'#dbeafe'};color:${iF.hall2===h?'#fff':'#1e40af'};
        border:1.5px solid ${iF.hall2===h?'#2563eb':'#93c5fd'}"
        onclick="iF.hall2='${h}';renderIngresos2()">${h}</span>`).join('')}
    </div>` : ''}
...
function _ingLista(items) {
  const s = getSort('ingresos2');
  items = sortArr(items, s.col||'pos', s.dir||'desc');

  if (!items.length) {
    return `<div class="empty"><div class="ei">🚦</div><div class="et">${DB.ingresos2.length?'Sin resultados':'Sin ingresos registrados'}</div></div>`;
  }
  ...
  const tbody = items.map(i => {
    const _tev = getTabEvent('ingresos2');
  ...
  switch(cid) {
    ...
    case 'acciones':
      return `<td><div style="display:flex;gap:2px;flex-wrap:wrap">
        ...
        ${canStatus()?`<button class="btn btn-xs" style="background:var(--purple);color:#fff" onclick="registrarPasoTracking('${i.id}','ingresos2')" title="Tracking">📡</button>`:''}
        ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="_ingSource='ingresos';_openIngModalV8(DB.ingresos2.find(x=>x.id==='${i.id}'))">✏️</button>`:''}
...
async function _salida(id) {
  const item = DB.ingresos2.find(x => x.id === id);
  if (!item) return;
...
function _ingColCtx(e, colId) {
  ...
  menu.innerHTML = `
    ...
    <div class="ctx-item" onclick="window._op.setSort('ingresos2','${colId}');document.getElementById('ingColCtxMenu').remove()">▲ Ordenar A → Z</div>
    <div class="ctx-item" onclick="window._op.setSort('ingresos2','${colId}');document.getElementById('ingColCtxMenu').remove()">▼ Ordenar Z → A</div>
    <div style="height:1px;background:var(--border);margin:3px 0"></div>
    <div class="ctx-item" onclick="_openColPanel2();document.getElementById('ingColCtxMenu').remove()">⚙ Gestionar columnas</div>`;
    ...
    panel.innerHTML = `
      ...
      <div style="flex:1;overflow-y:auto;padding:12px 14px">
        ...
        <button class="btn btn-gh btn-sm" style="width:100%;margin-top:10px;font-size:11px"
          onclick="_ingColsVis=[...ING_COLS_DEFAULT];_ingColsOrder=[...ING_COLS_DEFAULT];_saveColCfg({vis:_ingColsVis,order:_ingColsOrder});renderIngresos2();renderPanelContent()">
...
window._openColPanel2       = _openColPanel;
  ...
  _modal({
    ...
    onOk: async () => {
      ...
      const data = {
        ...
        pos:        isEdit ? item.pos : _nextPos('ingresos2'),
        matricula:  mat,
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px">
      ...
      <div style="display:flex;gap:6px;align-items:center">
        <select class="sbox" style="height:30px;padding:0 8px;font-size:11px"
          onchange="iF._modFiltro2=this.value;renderIngresos2()">
        ...
        ${isSA()?`<button class="btn btn-danger btn-sm" onclick="if(confirm('¿Limpiar historial?')){DB.editHistory=[];saveDB();renderIngresos2();}">🗑 Limpiar</button>`:''}
      </div>
    </div>
...
registerFn('_ingLN2',         _ingEspecial);
registerFn('_ingHistorial2',  _ingModificaciones);

window.renderIngresos2 = renderIngresos2;
window._openIngModalV8_2 = _openIngModalV8;
