...
const _hist    = { ing1:[], ing2:[], ag:[] };
let   _histIdx = { ing1:-1, ing2:-1, ag:-1 };
let   _histLock = false;
...
async function _saveCfg(key, cfg) {
  ...
  try { await localDB.set('cache', `printCfg_${key}`, cfg); } catch {}
...
async function _saveTpl(tpl) {
  ...
  _tplsCache = tpls;
  try { localStorage.setItem('beu_printTpls', JSON.stringify(tpls)); } catch {}
  const ev = AppState.get('currentEvent');
  if (!ev?.id) return;
  try {
    const db = getDB();
...
async function _deleteTpl(name) {
  ...
  try { localStorage.setItem('beu_printTpls', JSON.stringify(_tplsCache)); } catch {}
  const ev = AppState.get('currentEvent');
  if (!ev?.id) return;
  try {
    const db = getDB();
...
#impLeft{width:236px;min-width:236px;flex-shrink:0;overflow-y:auto;border-right:1px solid var(--border);background:var(--bg2);padding:7px;display:flex;flex-direction:column;gap:5px}
...
#impStatus{padding:1px 7px;font-size:9px;color:var(--text3);background:var(--bg2);border-bottom:1px solid var(--border);flex-shrink:0;display:flex;align-items:center;gap:8px;min-height:18px}
...
async function _switchSub(key, initial=false) {
  ...
  ['ing1','ing2','ag'].forEach(k => {
    const b = document.getElementById({ing1:'impTabIng1',ing2:'impTabIng2',ag:'impTabAg'}[k]);
...
function _applyScale() {
  ...
  const autoSc = Math.min(availW / paper.pw, availH / paper.ph, 1);
  const sc = _manualZoom !== null ? _manualZoom / 100 : autoSc;
  _scale = Math.max(sc, 0.05);
  pv.style.width  = paper.pw + 'px';
  pv.style.height = paper.ph + 'px';
  pv.style.transform = `scale(${_scale})`;
  pv.style.transformOrigin = 'top left';
  pvWrap.style.width  = Math.round(paper.pw * _scale) + 'px';
  pvWrap.style.height = Math.round(paper.ph * _scale) + 'px';
  document.getElementById('impZoomLbl').textContent = Math.round(_scale * 100) + '%';
  document.getElementById('impZoomSlider').value = Math.round(_scale * 100);
  document.getElementById('impScaleInfo').textContent =
    `${cfg.paperSize||'A4'} · ${paper.pw}×${paper.ph}px · ${Math.round(_scale*100)}%`;
}
...
function _restoreSnap(snap) {
  ...
  document.querySelectorAll('.imp-pfc').forEach(e => e.remove());
  _placed = {};
  try { _placed = JSON.parse(snap); } catch {}
...
async function _resetDia0() {
  ...
  for (const k of ['ing1','ing2','ag']) {
    _cfgCache[k] = _defCfg();
    await _saveCfg(k, _cfgCache[k]);
    try { await localDB.del('cache',`printCfg_${k}`); } catch {}
    _labelMode[k] = 0; _hist[k]=[]; _histIdx[k]=-1;
  }
  _tplsCache = [];
  try { localStorage.removeItem('beu_printTpls'); } catch {}
  const ev = AppState.get('currentEvent');
  if (ev?.id) {
    try {
      const db=getDB();
...
async function _initImpresionInner(containerId) {
  ...
  await Promise.all([_loadCfg('ing1'),_loadCfg('ing2'),_loadCfg('ag')]);
  await _loadTpls();
  _renderShell(el);
  _bindGlobalEvents();
  _finalizeGlobal();
...
export function setLastPrintEntry(entry) {
  ...
  try { AppState.set('lastPrintEntry', entry); } catch {}
}
