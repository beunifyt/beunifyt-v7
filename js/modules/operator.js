...
let DB = {
  ...
  printCfg1:{}, printCfg2:{}, printCfgAg:{}, printCfgFlota:{}, printTemplates:[], printCfgModes:{},
};
...
<div id="appHdr" style="background:var(--bg2);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 12px;height:44px;position:sticky;top:0;z-index:200">
  ...
  <div style="position:relative;flex-shrink:0;margin-right:6px"><input type="search" id="globalSearch" placeholder="🔍 Buscar..." oninput="window._op.globalSearch&&window._op.globalSearch(this.value)" style="width:160px;height:28px;padding:4px 10px 4px 10px;font-size:11px;border:1.5px solid var(--border);border-radius:20px;background:var(--bg3);color:var(--text);outline:none;font-family:inherit"></div>
...
function renderDash() {
  ...
  // AI Dashboard summary button
  h += `<div style="margin-top:12px;border-top:1px solid var(--border);padding-top:10px">
    <button class="btn btn-gh btn-sm" onclick="window._op.dashAISummary()" id="dashAIBtn" style="margin-bottom:6px">🤖 Resumen AI del evento</button>
    <div id="dashAIResult" style="display:none;padding:12px;background:var(--bg3);border-radius:var(--r);border:1px solid var(--border);font-size:12px;line-height:1.6;color:var(--text2);white-space:pre-wrap"></div>
  </div>`;
...
function renderIngresos(){
  ...
  document.getElementById('tabContent').innerHTML=`
    <div style="display:flex;align-items:center;gap:3px;padding:4px 0;flex-wrap:nowrap;min-height:34px;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none">
      ${[['lista','📋 Lista'],['listanegra','⭐ Especial'],['historial','📝 Modificaciones'],...(canCampos()?[['campos','⚙ Campos']]:[])] .map(([s,l])=>`<button class="btn btn-sm ${sub===s?'btn-p':'btn-gh'}" onclick="iF._sub='${s}';renderIngresos()">${l}</button>`).join('')}
      <span style="flex:1"></span>
      ${canAdd()&&sub!=='campos'?`<button class="btn btn-sm" style="background:#0d9f6e;color:#fff;border:none;border-radius:20px;font-weight:700" onclick="_ingSource='ingresos';openIngModal()">+ Referencia</button><button class="btn btn-sm btn-gh af-toggle-btn" onclick="toggleAutoFill()" style="flex-shrink:0;border-radius:20px">⚡ ${_autoFillOn?tr('on')||'ON':tr('off')||'OFF'}</button><button class="btn btn-sm btn-gh pos-toggle-btn" onclick="togglePosAuto()" style="flex-shrink:0;border-radius:20px">🔢 ${_posAutoOn?tr('on')||'ON':tr('off')||'OFF'}</button>`:''}
...
function _ingLista(items){
  ...
  return`
  ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr>${thSort('ingresos','pos','#')}${thSort('ingresos','matricula','Matrícula')}${thSort('ingresos','llamador','Llamador')}${thSort('ingresos','referencia','Ref')}${thSort('ingresos','nombre','Conductor/Empresa')}${thSort('ingresos','telefono','Tel.')}<th>Hall</th><th>Stand</th><th style="font-size:10px">Evento</th>${thSort('ingresos','salida','Estado')}${thSort('ingresos','entrada','Entrada')}<th>Acc.</th></tr></thead><tbody>
...
function _ingEspera(){
  ...
  ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>Matrícula</th><th>Prioridad</th><th>Conductor</th><th>Empresa</th><th>Hall</th><th>Hora</th><th>Acc.</th></tr></thead><tbody>
...
function _ingLN(){
  ...
  return`<div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--r);padding:5px 10px;margin-bottom:4px;font-size:11px;color:var(--text3);display:flex;align-items:center;gap:4px">🔗 <b>Datos compartidos</b> — esta lista es la misma en todas las pestañas</div>
  <div class="sec-act" style="margin-bottom:10px">${canSpecial()?`<button class="btn btn-r btn-sm" onclick="openLNModal()">+ Añadir matrícula</button>`:''} ${canExport()?`<button class="btn btn-gh btn-sm" onclick="exportListaNegra()">⬇ Excel</button>`:''}</div>
  <div class="sg sg3" style="margin-bottom:10px">
  ...
  ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>Matrícula</th><th>Nivel</th><th>Motivo</th><th>Empresa</th><th>Válido hasta</th><th>Acc.</th></tr></thead><tbody>
...
function _ingMsgs(){
  ...
  <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">
    ...
    ${isSup()&&DB.mensajesRampa.length?`<button class="btn btn-danger btn-sm" onclick="if(confirm('¿Limpiar todos?')){DB.mensajesRampa=[];saveDB();renderMensajesTab();renderHdr();}">🗑 Limpiar</button>`:''}
    ${canExport()?`<button class="btn btn-gh btn-sm" onclick="exportMensajes()">⬇ Excel</button>`:''}
  </div>
  ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>Tipo</th><th>Título</th><th>Mensaje</th><th>Matrícula</th><th>Hora</th><th>Autor</th><th></th></tr></thead><tbody>
...
function _ingPrintCfg(cfgKey){
  const isAg=cfgKey==='ag',isTab2=cfgKey==='ing2',isFlota=cfgKey==='flota';
  const cfg=isFlota?DB.printCfgFlota:isAg?DB.printCfgAg:isTab2?DB.printCfg2:DB.printCfg1;
  ...
  const tpls=(DB.printTemplates||[]).map((t,idx)=>{
    ...
    return`<div style="display:flex;align-items:flex-start;gap:5px;padding:5px 7px;border-radius:5px;background:var(--bg3);border:0.5px solid var(--border)">
      ...
      <div style="margin-left:6px"><button class="btn btn-xs btn-danger" onclick="delPrintTpl(${idx},'${cfgKey}')" title="Eliminar plantilla">✕</button></div>
    </div>`;
  }).join('');
  ...
  <div style="display:flex;flex-direction:column;gap:6px;position:sticky;top:0;align-self:start;max-height:calc(100vh - 70px);overflow-y:auto;padding-right:3px">
    ...
    <div style="border-top:0.5px solid var(--border);padding-top:6px;margin-top:10px">
      <div id="pctpl-confirm-${cfgKey}" style="display:none;background:var(--all);border:0.5px solid #fde68a;border-radius:5px;padding:6px 8px;margin-bottom:5px;font-size:11px">
        <div style="font-weight:700;margin-bottom:4px">¿Confirmar guardado?</div>
  ...
  <div style="display:flex;flex-direction:column;gap:0;position:sticky;top:60px;align-self:start;max-height:calc(100vh - 70px);overflow-y:auto;padding-right:2px">
    <div style="display:flex;align-items:center;gap:5px;padding-bottom:3px;flex-wrap:nowrap;position:sticky;top:0;background:var(--bg);z-index:20;padding-top:1px">
      ...
      <button class="btn btn-sm" style="padding:3px 8px;font-size:10px" onclick="if(!canDelTpl()){toast('Sin permiso','var(--red)');return;}pcClearAll('${cfgKey}')">🗑 Limpiar</button>
      ...
      <span style="flex:1"></span>
      <button class="btn btn-sm" style="padding:3px 9px;font-size:10px;background:#7c3aed;color:#fff;border-color:#7c3aed;border-radius:20px" onclick="if(!canDelTpl()){toast('Sin permiso','var(--red)');return;}resetPrintCfgDia0('${cfgKey}')">🔄 Día 0</button>
    </div>
  ...
  const PF_FIELDS=[
    ...
    {k:'tipoVehiculo',l:'Tipo Vehículo',demo:'Semirremolque'},
    {k:'descargaTipo',l:'Descarga',demo:'Manual'},
...
function _ingEventos(tab){
  ...
  <div style="margin-bottom:4px;display:flex;gap:6px;flex-wrap:wrap">
    ...
    ${actIds.length&&isSA()?`<button class="btn btn-warning btn-sm" onclick="if(confirm('¿Desactivar evento?')){DB.activeEventId=null;saveDB();${renderFn}();renderHdr();}">⏹ Desactivar evento</button>`:''}
  </div>
  ...
  ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>Evento</th><th>Fechas</th><th>Recinto</th><th>Estado</th><th>Puertas</th><th>Acc.</th></tr></thead><tbody>
...
function renderIngresos2(){
  ...
  document.getElementById('tabContent').innerHTML=`
    <div style="display:flex;align-items:center;gap:3px;padding:4px 0;flex-wrap:nowrap;min-height:34px;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none">
      ${[['lista','📋 Lista'],['listanegra','⭐ Especial'],['historial','📝 Modificaciones'],...(canCampos()?[['campos','⚙ Campos']]:[])] .map(([s,l])=>`<button class="btn btn-sm ${sub2===s?'btn-p':'btn-gh'}" onclick="iF['_sub2']='${s}';renderIngresos2()" style="flex-shrink:0">${l}</button>`).join('')}
      ...
      ${sub2!=='historial'&&sub2!=='campos'?`<button class="btn btn-s btn-sm" style="flex-shrink:0" onclick="if(!canImport()){toast('Sin permiso','var(--red)');return;}document.getElementById('xlsxIng2').click()">📥 Importar</button><button class="btn btn-gh btn-sm" style="flex-shrink:0" onclick="dlTemplateIng2()">📋 Plantilla</button>`:''}
    ...
    ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr>
      ${thSort('ingresos2','pos','#')}${thSort('ingresos2','matricula','Matrícula')}${thSort('ingresos2','llamador','Llamador')}${thSort('ingresos2','referencia','Ref')}${thSort('ingresos2','nombre','Conductor/Empresa')}${thSort('ingresos2','telefono','Tel.')}<th>Hall</th><th>Stand</th><th style="font-size:10px">Evento</th>${thSort('ingresos2','salida','Estado')}${thSort('ingresos2','entrada','Entrada')}<th>Acc.</th>
    </tr></thead><tbody>
      ${items.map(i=>`<tr>
        <td style="font-weight:700;color:var(--text3)">${i.pos||''}</td>
...
function renderFlota(){
  const now=new Date();
  const _fSub=window._flotaSub||'lista';
  
  // Handle print subtab
  if(_fSub==='print'){
    document.getElementById('tabContent').innerHTML=
      '<div style="display:flex;gap:3px;margin-bottom:6px;border-bottom:1px solid var(--border);padding-bottom:4px">'+
      '<button class="btn btn-sm btn-gh" onclick="window._flotaSub=\'lista\';renderFlota()">📋 Lista</button>'+
      '<button class="btn btn-sm btn-p">🖨 Impresión</button>'+
      '</div>'+_ingPrintCfg('flota');
    setTimeout(()=>{initPrintLayout('flota');initPcCanvas('flota');},100);
    return;
  }
  if(_fSub==='especial'){
    document.getElementById('tabContent').innerHTML=
      '<div style="display:flex;gap:3px;margin-bottom:6px;border-bottom:1px solid var(--border);padding-bottom:4px">'+
      '<button class="btn btn-sm btn-gh" onclick="window._flotaSub=\'lista\';renderFlota()">📋 Lista</button>'+
      '<button class="btn btn-sm btn-p">⭐ Especial</button>'+
      '<button class="btn btn-sm btn-gh" onclick="window._flotaSub=\'campos\';renderFlota()">⚙ Campos</button>'+
      '</div>'+_ingLN();
    return;
  }
  if(_fSub==='campos'){
    document.getElementById('tabContent').innerHTML=
      '<div style="display:flex;gap:3px;margin-bottom:6px;border-bottom:1px solid var(--border);padding-bottom:4px">'+
      '<button class="btn btn-sm btn-gh" onclick="window._flotaSub=\'lista\';renderFlota()">📋 Lista</button>'+
      '<button class="btn btn-sm btn-gh" onclick="window._flotaSub=\'especial\';renderFlota()">⭐ Especial</button>'+
      '<button class="btn btn-sm btn-p">⚙ Campos</button>'+
      '</div>'+renderCamposSubtab('movimientos');
    return;
  }
  if(_fSub==='historial'){
    document.getElementById('tabContent').innerHTML=
      '<div style="display:flex;gap:3px;margin-bottom:6px;border-bottom:1px solid var(--border);padding-bottom:4px">'+
      '<button class="btn btn-sm btn-gh" onclick="window._flotaSub=\'lista\';renderFlota()">📋 Lista</button>'+
      '<button class="btn btn-sm btn-p">📝 Modificaciones</button>'+
      '</div>'+_ingHistorial('movimientos');
    return;
  }
  ...
  document.getElementById('tabContent').innerHTML=`
    ...
    <div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;border-bottom:1px solid var(--border);padding-bottom:4px">
      ...
      <button class="btn btn-sm btn-gh" style="flex-shrink:0" onclick="window._flotaSub='especial';renderFlota()">⭐ Especial</button>
      <button class="btn btn-sm btn-gh" style="flex-shrink:0" onclick="window._flotaSub='historial';renderFlota()">📝 Modificaciones</button>
      ${canCampos()?'<button class="btn btn-sm btn-gh" style="flex-shrink:0" onclick="window._flotaSub=\'campos\';renderFlota()">⚙ Campos</button>':''}
      <span style="flex:1"></span>
    ...
    ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr>${thSort('flota','posicion','#')}${thSort('flota','matricula','Tractora')}<th>Remolque</th>${thSort('flota','nombre','Conductor')}${thSort('flota','empresa','Empresa')}<th>Hall</th><th>Carga</th>${thSort('flota','status','Estado')}<th>Tacógrafo</th><th>Acc.</th></tr></thead><tbody>
...
function renderConductores(){
  ...
  document.getElementById('tabContent').innerHTML=`
    <div style="display:flex;align-items:center;gap:3px;padding:4px 0;flex-wrap:nowrap;min-height:34px;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none">
      ...
      <button class="btn btn-sm btn-gh" style="flex-shrink:0" onclick="window._condSub='historial';renderConductores()">📝 Modificaciones</button>
    ...
    ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr>${thSort('conductores','matricula','Matrícula')}${thSort('conductores','nombre','Nombre')}${thSort('conductores','empresa','Empresa')}<th>Teléfono</th>${thSort('conductores','hall','Hall')}<th>Idioma</th><th>Eventos</th><th>Ingresos</th><th>Acc.</th></tr></thead><tbody>
...
function renderAnalytics() {
  ...
  const today = new Date().toISOString().slice(0,10);
  const allIngs = [...DB.ingresos, ...(DB.ingresos2||[])];
  const allAg = DB.agenda||[];
  const allCond = DB.conductores||[];
  const allMov = DB.movimientos||[];

  // Data collections for cross-field analysis
  const _collections = {
    'Referencia': {data:DB.ingresos, fields:['matricula','empresa','hall','stand','llamador','referencia','montador','expositor','tipoVehiculo','descargaTipo','nombre']},
    'Ingresos': {data:DB.ingresos2||[], fields:['matricula','empresa','hall','stand','nombre','tipoVehiculo']},
    'Agenda': {data:allAg, fields:['matricula','empresa','hall','conductor','estado','fecha','hora','referencia']},
    'Conductores': {data:allCond, fields:['nombre','empresa','matricula','hall','tipoVehiculo']},
    'Embalaje': {data:allMov, fields:['matricula','empresa','hall','status','tipoCarga']},
  };

  // Sub-tab state
  const sub = window._anaSub || 'resumen';

  // ── Stats
  const enRecinto = allIngs.filter(i=>!i.salida).length;
  const hoy = allIngs.filter(i=>i.entrada?.startsWith(today)).length;
  ...
  const empTop = Object.entries(empC).sort((a,b)=>b[1]-a[1]).slice(0,10);
  const empMax = empTop.length ? empTop[0][1] : 1;
  const hallC = {}; allIngs.forEach(i => { (i.halls||[i.hall]).filter(Boolean).forEach(h=>{ hallC[h]=(hallC[h]||0)+1; }); });
  const hallTop = Object.entries(hallC).sort((a,b)=>b[1]-a[1]).slice(0,10);
  const hallMax = hallTop.length ? hallTop[0][1] : 1;

  // ── Hourly distribution
  const hourC = {}; allIngs.forEach(i => { if(i.entrada) { const h = i.entrada.slice(11,13); hourC[h]=(hourC[h]||0)+1; }});
  const hours = Object.entries(hourC).sort((a,b)=>a[0].localeCompare(b[0]));
  const hourMax = hours.length ? Math.max(...hours.map(h=>h[1])) : 1;

  // ── Daily trend
  const dayC = {}; allIngs.forEach(i => { if(i.entrada) { const d=i.entrada.slice(0,10); dayC[d]=(dayC[d]||0)+1; }});
  const days = Object.entries(dayC).sort((a,b)=>a[0].localeCompare(b[0])).slice(-14);
  const dayMax = days.length ? Math.max(...days.map(d=>d[1])) : 1;

  // ── Subtabs
  let h = `<div style="display:flex;align-items:center;gap:3px;padding:4px 0;border-bottom:1px solid var(--border);margin-bottom:6px;overflow-x:auto;scrollbar-width:none">
    ${[['resumen','📊 Resumen'],['graficos','📈 Gráficos'],['cruzar','🔀 Campos cruzados'],['ai','🤖 AI Análisis']].map(([s,l])=>`<button class="btn btn-sm ${sub===s?'btn-p':'btn-gh'}" onclick="window._anaSub='${s}';window._op.renderAnalytics()">${l}</button>`).join('')}
  </div>`;

  if (sub === 'resumen') {
    h += `<div class="sg sg4" style="margin-bottom:8px">
      <div style="text-align:center;padding:10px;background:var(--gll);border-radius:var(--r);border:1px solid #bbf7d0"><div class="stat-n" style="color:var(--green)">${enRecinto}</div><div class="stat-l">En recinto</div></div>
      <div style="text-align:center;padding:10px;background:var(--bll);border-radius:var(--r);border:1px solid #bfdbfe"><div class="stat-n" style="color:var(--blue)">${hoy}</div><div class="stat-l">Hoy</div></div>
      <div style="text-align:center;padding:10px;background:var(--bg3);border-radius:var(--r);border:1px solid var(--border)"><div class="stat-n">${DB.ingresos.length}</div><div class="stat-l">Total Ref.</div></div>
      <div style="text-align:center;padding:10px;background:var(--bg3);border-radius:var(--r);border:1px solid var(--border)"><div class="stat-n">${(DB.ingresos2||[]).length}</div><div class="stat-l">Total Ing.</div></div>
    </div>
    <div class="sg sg3" style="margin-bottom:8px">
      <div class="card"><div style="font-weight:800;margin-bottom:8px;font-size:12px">🚗 Tipo vehículo</div>
        ${tvTop.map(kv=>`<div class="bar-row"><span style="font-size:10px;min-width:90px;color:var(--text2)">${esc(kv[0])}</span><div class="bar-bg"><div class="bar-fill" style="width:${Math.round(kv[1]/tvMax*100)}%;background:#3b82f6"></div></div><span class="bar-val">${kv[1]}</span></div>`).join('')||'<div style="padding:8px;font-size:11px;color:var(--text3)">Sin datos</div>'}
      </div>
      <div class="card"><div style="font-weight:800;margin-bottom:8px;font-size:12px">📦 Tipo descarga</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          <div style="text-align:center;background:var(--gll);border:1px solid #bbf7d0;border-radius:var(--r);padding:10px"><div class="stat-n" style="color:var(--green)">${descC.mano||0}</div><div class="stat-l">🤾 A Mano</div></div>
          <div style="text-align:center;background:var(--bll);border:1px solid #bfdbfe;border-radius:var(--r);padding:10px"><div class="stat-n" style="color:var(--blue)">${descC.maquinaria||0}</div><div class="stat-l">🏗 Maquinaria</div></div>
        </div>
      </div>
      <div class="card"><div style="font-weight:800;margin-bottom:8px;font-size:12px">🏢 Top empresas</div>
        ${empTop.map(kv=>`<div class="bar-row"><span style="font-size:10px;min-width:90px;color:var(--text2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100px">${esc(kv[0])}</span><div class="bar-bg"><div class="bar-fill" style="width:${Math.round(kv[1]/empMax*100)}%;background:var(--teal)"></div></div><span class="bar-val">${kv[1]}</span></div>`).join('')||'<div style="padding:8px;font-size:11px;color:var(--text3)">Sin datos</div>'}
      </div>
    </div>`;
  }

  else if (sub === 'graficos') {
    h += `<div class="sg sg2" style="margin-bottom:8px">
      <div class="card"><div style="font-weight:800;margin-bottom:8px;font-size:12px">🏭 Distribución por Hall</div>
        ${hallTop.map(kv=>`<div class="bar-row"><span style="font-size:10px;min-width:50px;font-weight:700;color:var(--blue)">${esc(kv[0])}</span><div class="bar-bg"><div class="bar-fill" style="width:${Math.round(kv[1]/hallMax*100)}%;background:#6366f1"></div></div><span class="bar-val">${kv[1]}</span></div>`).join('')||'<div style="padding:8px;font-size:11px;color:var(--text3)">Sin datos</div>'}
      </div>
      <div class="card"><div style="font-weight:800;margin-bottom:8px;font-size:12px">⏰ Distribución horaria</div>
        <div style="display:flex;align-items:flex-end;gap:2px;height:120px;padding-top:8px">
          ${hours.map(([hr,cnt])=>`<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px"><div style="width:100%;background:#3b82f6;border-radius:3px 3px 0 0;min-height:2px;height:${Math.round(cnt/hourMax*100)}px"></div><span style="font-size:8px;color:var(--text3)">${hr}h</span></div>`).join('')||'<div style="padding:8px;font-size:11px;color:var(--text3);width:100%;text-align:center">Sin datos</div>'}
        </div>
      </div>
    </div>
    <div class="card" style="margin-bottom:8px"><div style="font-weight:800;margin-bottom:8px;font-size:12px">📅 Tendencia diaria (últimos 14 días)</div>
      <div style="display:flex;align-items:flex-end;gap:3px;height:100px;padding-top:8px">
        ${days.map(([day,cnt])=>`<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px"><div style="width:100%;background:#0d9f6e;border-radius:3px 3px 0 0;min-height:2px;height:${Math.round(cnt/dayMax*90)}px"></div><span style="font-size:7px;color:var(--text3);transform:rotate(-45deg);white-space:nowrap">${day.slice(5)}</span></div>`).join('')||'<div style="padding:8px;font-size:11px;color:var(--text3);width:100%;text-align:center">Sin datos</div>'}
      </div>
    </div>
    <div class="sg sg2">
      <div class="card"><div style="font-weight:800;margin-bottom:8px;font-size:12px">📊 Agenda: estado</div>
        ${['PENDIENTE','LLEGADO','SALIDA','HECHO'].map(s=>{const cnt=allAg.filter(a=>a.estado===s).length;return cnt?`<div class="bar-row"><span style="font-size:10px;min-width:80px">${sAgBadge(s)}</span><div class="bar-bg"><div class="bar-fill" style="width:${Math.round(cnt/Math.max(allAg.length,1)*100)}%;background:${s==='LLEGADO'?'var(--green)':s==='PENDIENTE'?'var(--amber)':'var(--blue)'}"></div></div><span class="bar-val">${cnt}</span></div>`:''}).join('')||'<div style="padding:8px;font-size:11px;color:var(--text3)">Sin datos</div>'}
      </div>
      <div class="card"><div style="font-weight:800;margin-bottom:8px;font-size:12px">📦 Embalaje: estado</div>
        ${Object.entries(SCFG).map(([k,v])=>{const cnt=allMov.filter(m=>m.status===k).length;return cnt?`<div class="bar-row"><span style="font-size:10px;min-width:70px">${sBadge(k)}</span><div class="bar-bg"><div class="bar-fill" style="width:${Math.round(cnt/Math.max(allMov.length,1)*100)}%;background:${v.c}"></div></div><span class="bar-val">${cnt}</span></div>`:''}).join('')||'<div style="padding:8px;font-size:11px;color:var(--text3)">Sin datos</div>'}
      </div>
    </div>`;
  }

  else if (sub === 'cruzar') {
    // Cross-field analysis UI
    const sel1 = window._anaField1 || '';
    const sel2 = window._anaField2 || '';
    const selCol = window._anaCol || 'Referencia';
    const allFields = _collections[selCol]?.fields || [];

    h += `<div class="card" style="margin-bottom:8px">
      <div style="font-weight:800;margin-bottom:10px;font-size:13px">🔀 Análisis de campos cruzados</div>
      <div class="sg sg3" style="margin-bottom:10px">
        <div class="fg"><label class="flbl">Colección</label><select onchange="window._anaCol=this.value;window._anaField1='';window._anaField2='';window._op.renderAnalytics()" style="border-radius:20px;height:34px">
          ${Object.keys(_collections).map(c=>`<option ${c===selCol?'selected':''}>${c}</option>`).join('')}
        </select></div>
        <div class="fg"><label class="flbl">Campo X</label><select onchange="window._anaField1=this.value;window._op.renderAnalytics()" style="border-radius:20px;height:34px">
          <option value="">-- Seleccionar --</option>
          ${allFields.map(f=>`<option value="${f}" ${f===sel1?'selected':''}>${f}</option>`).join('')}
        </select></div>
        <div class="fg"><label class="flbl">Campo Y</label><select onchange="window._anaField2=this.value;window._op.renderAnalytics()" style="border-radius:20px;height:34px">
          <option value="">-- Seleccionar --</option>
          ${allFields.map(f=>`<option value="${f}" ${f===sel2?'selected':''}>${f}</option>`).join('')}
        </select></div>
      </div>`;

    if (sel1 && sel2) {
      const data = _collections[selCol]?.data || [];
      const cross = {};
      data.forEach(row => {
        const v1 = String(row[sel1]||'').trim() || '(vacío)';
        const v2 = String(row[sel2]||'').trim() || '(vacío)';
        if (!cross[v1]) cross[v1] = {};
        cross[v1][v2] = (cross[v1][v2]||0) + 1;
      });
      const rows = Object.entries(cross).sort((a,b)=>Object.values(b[1]).reduce((s,v)=>s+v,0)-Object.values(a[1]).reduce((s,v)=>s+v,0)).slice(0,15);
      const allV2 = [...new Set(data.map(r=>String(r[sel2]||'').trim()||'(vacío)'))].slice(0,10);
      const maxVal = Math.max(...rows.flatMap(([k,v])=>Object.values(v)), 1);

      h += `<div style="font-weight:700;font-size:12px;margin-bottom:6px">${sel1} × ${sel2} — ${data.length} registros</div>
      <div class="tbl-wrap"><table class="dtbl"><thead><tr><th>${sel1}</th>${allV2.map(v=>`<th style="font-size:9px;max-width:80px;overflow:hidden;text-overflow:ellipsis">${esc(v)}</th>`).join('')}<th>Total</th></tr></thead><tbody>
        ${rows.map(([v1,vals])=>{const total=Object.values(vals).reduce((s,v)=>s+v,0);return`<tr><td style="font-weight:700;font-size:11px">${esc(v1)}</td>${allV2.map(v2=>{const cnt=vals[v2]||0;return`<td style="text-align:center;font-size:11px;${cnt?`background:rgba(37,99,235,${Math.min(cnt/maxVal,.8)});color:${cnt/maxVal>.4?'#fff':'var(--text)'}`:''}">${cnt||''}</td>`;}).join('')}<td style="font-weight:800;text-align:center">${total}</td></tr>`;}).join('')}
      </tbody></table></div>`;
    } else {
      h += `<div style="padding:20px;text-align:center;color:var(--text3);font-size:12px">Selecciona dos campos para ver la correlación</div>`;
    }
    h += `</div>`;
  }
    ...
    const lastResult = window._anaAIResult || '';
    h += `<div class="card" style="margin-bottom:8px">
      <div style="font-weight:800;margin-bottom:10px;font-size:13px">🤖 Análisis con Inteligencia Artificial</div>
      <div style="margin-bottom:10px;font-size:12px;color:var(--text2)">Escribe lo que quieres averiguar sobre tus datos y la AI analizará todas las colecciones.</div>
      <div style="display:flex;gap:6px;margin-bottom:10px">
        <input type="text" id="anaAIQuery" placeholder="Ej: ¿Qué empresa tiene más retrasos? ¿Cuál es la hora pico?" style="flex:1;border-radius:20px;height:38px;padding:0 16px;font-size:13px" value="${esc(window._anaAIQuery||'')}" onkeydown="if(event.key==='Enter')window._op.runAIAnalysis()">
        <button class="btn btn-p" onclick="window._op.runAIAnalysis()" id="anaAIBtn" style="white-space:nowrap">🤖 Analizar</button>
      </div>
      <div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:10px">
        ${['¿Hora pico de llegadas?','¿Empresa con más ingresos?','¿Qué hall está más ocupado?','¿Tendencia de los próximos días?','¿Conductores más frecuentes?','Resumen ejecutivo del evento'].map(q=>`<button class="btn btn-gh btn-xs" onclick="document.getElementById('anaAIQuery').value='${q}';window._op.runAIAnalysis()" style="font-size:10px">${q}</button>`).join('')}
      </div>
      <div id="anaAIResult" style="min-height:60px;padding:12px;background:var(--bg3);border-radius:var(--r);border:1px solid var(--border);font-size:13px;line-height:1.7;color:var(--text2);white-space:pre-wrap">${lastResult||'<span style="color:var(--text4)">Los resultados del análisis aparecerán aquí...</span>'}</div>
    </div>`;
  }
...
// AI Analysis engine using Anthropic API
async function runAIAnalysis() {
  const query = (document.getElementById('anaAIQuery')?.value||'').trim();
  if (!query) { toast('Escribe una pregunta','var(--amber)'); return; }
  window._anaAIQuery = query;
  const btn = document.getElementById('anaAIBtn');
  const resultEl = document.getElementById('anaAIResult');
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Analizando...'; }
  if (resultEl) resultEl.innerHTML = '<span style="color:var(--text3)">🔄 Procesando datos...</span>';

  // Build data summary for AI
  const allIngs = [...DB.ingresos, ...(DB.ingresos2||[])];
  const summary = {
    totalReferencia: DB.ingresos.length,
    totalIngresos: (DB.ingresos2||[]).length,
    enRecinto: allIngs.filter(i=>!i.salida).length,
    totalAgenda: (DB.agenda||[]).length,
    totalConductores: (DB.conductores||[]).length,
    totalEmbalaje: (DB.movimientos||[]).length,
    empresasTop: Object.entries((() => { const c={}; allIngs.forEach(i=>{ if(i.empresa) c[i.empresa]=(c[i.empresa]||0)+1; }); return c; })()).sort((a,b)=>b[1]-a[1]).slice(0,10),
    hallsDistrib: Object.entries((() => { const c={}; allIngs.forEach(i=>{ (i.halls||[i.hall]).filter(Boolean).forEach(h=>{ c[h]=(c[h]||0)+1; }); }); return c; })()).sort((a,b)=>b[1]-a[1]),
    horasDistrib: Object.entries((() => { const c={}; allIngs.forEach(i=>{ if(i.entrada){ const h=i.entrada.slice(11,13); c[h]=(c[h]||0)+1; } }); return c; })()).sort((a,b)=>a[0].localeCompare(b[0])),
    diasTendencia: Object.entries((() => { const c={}; allIngs.forEach(i=>{ if(i.entrada){ const d=i.entrada.slice(0,10); c[d]=(c[d]||0)+1; } }); return c; })()).sort((a,b)=>a[0].localeCompare(b[0])).slice(-14),
    agendaEstados: {pendiente:(DB.agenda||[]).filter(a=>a.estado==='PENDIENTE').length, llegado:(DB.agenda||[]).filter(a=>a.estado==='LLEGADO').length, salida:(DB.agenda||[]).filter(a=>a.estado==='SALIDA').length},
    vehiculosTipo: Object.entries((() => { const c={}; allIngs.forEach(i=>{ if(i.tipoVehiculo) c[i.tipoVehiculo]=(c[i.tipoVehiculo]||0)+1; }); return c; })()),
  };
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Eres un analista de datos experto para una plataforma de control logístico de eventos (ferias, congresos). Analiza estos datos y responde en español de forma concisa y profesional con insights accionables.
...
2. Insights clave que detectes
3. Si aplica, una previsión o recomendación
Usa emojis para hacer la respuesta visual. Máximo 300 palabras.`
        }]
      })
    });
    const data = await resp.json();
    const text = data.content?.map(b=>b.text||'').join('\n') || 'Sin respuesta';
    window._anaAIResult = text;
    if (resultEl) resultEl.textContent = text;
  } catch(e) {
    const fallback = _localAnalysis(query, summary);
    window._anaAIResult = fallback;
...
function _localAnalysis(query, summary) {
  let r = '📊 Análisis local (sin conexión AI):\n\n';
  r += `📌 Total registros: ${summary.totalReferencia} ref + ${summary.totalIngresos} ing\n`;
  r += `🏭 En recinto ahora: ${summary.enRecinto}\n`;
  if (summary.empresasTop.length) r += `🏢 Top empresa: ${summary.empresasTop[0][0]} (${summary.empresasTop[0][1]} registros)\n`;
  if (summary.hallsDistrib.length) r += `🏭 Hall más activo: ${summary.hallsDistrib[0][0]} (${summary.hallsDistrib[0][1]})\n`;
  if (summary.horasDistrib.length) {
    const peak = summary.horasDistrib.sort((a,b)=>b[1]-a[1])[0];
    r += `⏰ Hora pico: ${peak[0]}:00 (${peak[1]} llegadas)\n`;
  }
  if (summary.diasTendencia.length > 1) {
    const last = summary.diasTendencia[summary.diasTendencia.length-1][1];
    const prev = summary.diasTendencia[summary.diasTendencia.length-2][1];
    const trend = last > prev ? '📈 Tendencia al alza' : last < prev ? '📉 Tendencia a la baja' : '➡️ Estable';
    r += `${trend}: ${prev} → ${last} (últimos 2 días)\n`;
  }
  r += `\n📅 Agenda: ${summary.agendaEstados.pendiente} pendientes, ${summary.agendaEstados.llegado} llegados`;
  return r;
}
...
function renderVehiculos() {
  ...
  <div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;flex-wrap:nowrap;border-bottom:1px solid var(--border);padding-bottom:4px;overflow-x:auto;scrollbar-width:none">
    <div class="sbox" style="flex:0 1 240px;min-width:120px"><span class="sico">🔍</span><input type="search" placeholder="Matrícula, usuario..." value="${esc(iF.qVeh||'')}" oninput="window._iF.qVeh=this.value;window._op.renderVehiculos()" style="font-size:12px"></div>
    ...
    <span style="flex:1"></span>
    ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'vehiculos\')">⬇ Excel</button>':''}
    ${isSA()?'<button class="btn btn-gh btn-sm" onclick="window._op.vaciarTab(\'vehiculos\')">💥 Vaciar</button>':''}
  </div>`;
  if (!filtered.length) { h += '<div class="empty"><div class="ei">📜</div><div class="et">Sin historial de cambios</div></div>'; }
  else {
    ...
    <div class="sg sg2">
      ...
      <div class="fg"><label class="flbl">Contraseña</label>
        <div style="position:relative;display:flex;align-items:center">
          <input id="uPass" type="password" placeholder="mín. 8 caracteres" style="padding-right:36px;width:100%"><button type="button" onclick="(function(){var i=document.getElementById('uPass');i.type=i.type==='password'?'text':'password';})()" style="position:absolute;right:8px;background:none;border:none;cursor:pointer;color:var(--text3);display:flex"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
      ...
      <div class="fg"><label class="flbl">PIN (mín. 6 dígitos)</label>
        <div style="position:relative;display:flex;align-items:center">
          <input id="uPin" type="password" maxlength="8" inputmode="numeric" placeholder="------" style="padding-right:36px;width:100%"><button type="button" onclick="(function(){var i=document.getElementById('uPin');i.type=i.type==='password'?'text':'password';})()" style="position:absolute;right:8px;background:none;border:none;cursor:pointer;color:var(--text3);display:flex"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
  ...
  <div style="display:flex;flex-direction:column;gap:6px">
    ${COLECCIONES.map(col => `
      <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--bg2);border:1px solid var(--border);border-radius:var(--r2)">
        ...
        <button class="btn btn-gh btn-sm" onclick="window._op.dlTemplateForCol&&window._op.dlTemplateForCol('${col.key}')">📋 Plantilla</button>
...
let CUR_LANG = AppState.get('currentLang') || 'es';
AppState.watch('currentLang', v => { CUR_LANG = v || 'es'; });

const I18N_OP = {
  es:{on:'ON',off:'OFF',dashboard:'Dashboard',referencia:'Referencia',ingresos:'Ingresos',embalaje:'Embalaje',conductores:'Conductores',agenda:'Agenda',analisis:'Análisis',historial:'Historial',archivos:'Archivos',papelera:'Papelera',impresion:'Impresión',recintos:'Recintos',eventos:'Eventos',mensajes:'Mensajes',usuarios:'Usuarios',empresas:'Empresas',migracion:'Migración',buscar:'Buscar...',tema:'Tema',salir:'Salir',guardar:'Guardar',cancelar:'Cancelar',eliminar:'Eliminar',importar:'Importar',plantilla:'Plantilla',excel:'Excel',limpiar:'Limpiar',vaciar:'Vaciar',nuevo:'Nuevo',editar:'Editar',ver:'Ver',sinDatos:'Sin datos',sinResultados:'Sin resultados',enRecinto:'En recinto',hoy:'Hoy',total:'Total',lista:'Lista',especial:'Especial',modificaciones:'Modificaciones',campos:'Campos',acciones:'Acciones',estado:'Estado',entrada:'Entrada',salida:'Salida',matricula:'Matrícula',conductor:'Conductor',empresa:'Empresa',hall:'Hall',stand:'Stand',telefono:'Teléfono',todos:'Todos',soloActivos:'Solo activos',resumen:'Resumen',graficos:'Gráficos',camposCruzados:'Campos cruzados',aiAnalisis:'AI Análisis',resumenAI:'Resumen AI del evento',analizando:'Analizando...',pregunta:'Escribe tu pregunta...',sinEvento:'Sin evento',cerrarSesion:'¿Cerrar sesión?',confirmar:'Confirmar',registros:'registros',noEncontrado:'No encontrado',obligatorio:'obligatorio',exportado:'Exportado',guardado:'Guardado',reactivado:'Reactivado',salidaReg:'Salida registrada',llegadaReg:'Llegada registrada',bloqueado:'Bloqueado',sinPermiso:'Sin permiso'},
  en:{on:'ON',off:'OFF',dashboard:'Dashboard',referencia:'Reference',ingresos:'Entries',embalaje:'Packaging',conductores:'Drivers',agenda:'Schedule',analisis:'Analytics',historial:'History',archivos:'Files',papelera:'Trash',impresion:'Print',recintos:'Venues',eventos:'Events',mensajes:'Messages',usuarios:'Users',empresas:'Companies',migracion:'Migration',buscar:'Search...',tema:'Theme',salir:'Logout',guardar:'Save',cancelar:'Cancel',eliminar:'Delete',importar:'Import',plantilla:'Template',excel:'Excel',limpiar:'Clean',vaciar:'Empty',nuevo:'New',editar:'Edit',ver:'View',sinDatos:'No data',sinResultados:'No results',enRecinto:'On site',hoy:'Today',total:'Total',lista:'List',especial:'Special',modificaciones:'Changes',campos:'Fields',acciones:'Actions',estado:'Status',entrada:'Entry',salida:'Exit',matricula:'Plate',conductor:'Driver',empresa:'Company',hall:'Hall',stand:'Stand',telefono:'Phone',todos:'All',soloActivos:'Active only',resumen:'Summary',graficos:'Charts',camposCruzados:'Cross fields',aiAnalisis:'AI Analysis',resumenAI:'AI event summary',analizando:'Analyzing...',pregunta:'Type your question...',sinEvento:'No event',cerrarSesion:'Log out?',confirmar:'Confirm',registros:'records',noEncontrado:'Not found',obligatorio:'required',exportado:'Exported',guardado:'Saved',reactivado:'Reactivated',salidaReg:'Exit registered',llegadaReg:'Arrival registered',bloqueado:'Locked',sinPermiso:'No permission'},
  fr:{on:'ON',off:'OFF',dashboard:'Tableau de bord',referencia:'Référence',ingresos:'Entrées',embalaje:'Emballage',conductores:'Chauffeurs',agenda:'Agenda',analisis:'Analyses',historial:'Historique',archivos:'Fichiers',papelera:'Corbeille',impresion:'Impression',recintos:'Sites',eventos:'Événements',mensajes:'Messages',usuarios:'Utilisateurs',empresas:'Entreprises',migracion:'Migration',buscar:'Rechercher...',tema:'Thème',salir:'Déconnexion',guardar:'Enregistrer',cancelar:'Annuler',eliminar:'Supprimer',importar:'Importer',plantilla:'Modèle',excel:'Excel',limpiar:'Nettoyer',vaciar:'Vider',nuevo:'Nouveau',editar:'Modifier',ver:'Voir',sinDatos:'Pas de données',sinResultados:'Aucun résultat',enRecinto:'Sur site',hoy:"Aujourd'hui",total:'Total',lista:'Liste',especial:'Spécial',modificaciones:'Modifications',campos:'Champs',acciones:'Actions',estado:'Statut',entrada:'Entrée',salida:'Sortie',matricula:'Immatriculation',conductor:'Chauffeur',empresa:'Entreprise',hall:'Hall',stand:'Stand',telefono:'Téléphone',todos:'Tous',soloActivos:'Actifs seulement',resumen:'Résumé',graficos:'Graphiques',camposCruzados:'Champs croisés',aiAnalisis:'Analyse IA',resumenAI:"Résumé IA de l'événement",analizando:'Analyse en cours...',pregunta:'Écrivez votre question...',sinEvento:'Aucun événement',cerrarSesion:'Se déconnecter ?',confirmar:'Confirmer',registros:'enregistrements',noEncontrado:'Non trouvé',obligatorio:'obligatoire',exportado:'Exporté',guardado:'Enregistré',reactivado:'Réactivé',salidaReg:'Sortie enregistrée',llegadaReg:'Arrivée enregistrée',bloqueado:'Bloqué',sinPermiso:'Sans autorisation'},
  de:{on:'AN',off:'AUS',dashboard:'Übersicht',referencia:'Referenz',ingresos:'Eingänge',embalaje:'Verpackung',conductores:'Fahrer',agenda:'Terminplan',analisis:'Analysen',historial:'Verlauf',archivos:'Dateien',papelera:'Papierkorb',impresion:'Druck',recintos:'Standorte',eventos:'Veranstaltungen',mensajes:'Nachrichten',usuarios:'Benutzer',empresas:'Unternehmen',migracion:'Migration',buscar:'Suchen...',tema:'Design',salir:'Abmelden',guardar:'Speichern',cancelar:'Abbrechen',eliminar:'Löschen',importar:'Importieren',plantilla:'Vorlage',excel:'Excel',limpiar:'Bereinigen',vaciar:'Leeren',nuevo:'Neu',editar:'Bearbeiten',ver:'Ansehen',sinDatos:'Keine Daten',sinResultados:'Keine Ergebnisse',enRecinto:'Vor Ort',hoy:'Heute',total:'Gesamt',lista:'Liste',especial:'Spezial',modificaciones:'Änderungen',campos:'Felder',acciones:'Aktionen',estado:'Status',entrada:'Einfahrt',salida:'Ausfahrt',matricula:'Kennzeichen',conductor:'Fahrer',empresa:'Firma',hall:'Halle',stand:'Stand',telefono:'Telefon',todos:'Alle',soloActivos:'Nur aktive',resumen:'Zusammenfassung',graficos:'Diagramme',camposCruzados:'Kreuzfelder',aiAnalisis:'KI-Analyse',resumenAI:'KI-Zusammenfassung',analizando:'Wird analysiert...',pregunta:'Schreiben Sie Ihre Frage...',sinEvento:'Kein Event',cerrarSesion:'Abmelden?',confirmar:'Bestätigen',registros:'Datensätze',noEncontrado:'Nicht gefunden',obligatorio:'erforderlich',exportado:'Exportiert',guardado:'Gespeichert',reactivado:'Reaktiviert',salidaReg:'Ausfahrt registriert',llegadaReg:'Ankunft registriert',bloqueado:'Gesperrt',sinPermiso:'Keine Berechtigung'},
  it:{on:'ON',off:'OFF',dashboard:'Pannello',referencia:'Riferimento',ingresos:'Ingressi',embalaje:'Imballaggio',conductores:'Autisti',agenda:'Agenda',analisis:'Analisi',historial:'Storico',archivos:'File',papelera:'Cestino',impresion:'Stampa',recintos:'Sedi',eventos:'Eventi',mensajes:'Messaggi',usuarios:'Utenti',empresas:'Aziende',migracion:'Migrazione',buscar:'Cerca...',tema:'Tema',salir:'Esci',guardar:'Salva',cancelar:'Annulla',eliminar:'Elimina',importar:'Importa',plantilla:'Modello',excel:'Excel',limpiar:'Pulisci',vaciar:'Svuota',nuevo:'Nuovo',editar:'Modifica',ver:'Vedi',sinDatos:'Nessun dato',sinResultados:'Nessun risultato',enRecinto:'In sede',hoy:'Oggi',total:'Totale',lista:'Lista',especial:'Speciale',modificaciones:'Modifiche',campos:'Campi',acciones:'Azioni',estado:'Stato',entrada:'Entrata',salida:'Uscita',matricula:'Targa',conductor:'Autista',empresa:'Azienda',hall:'Padiglione',stand:'Stand',telefono:'Telefono',todos:'Tutti',soloActivos:'Solo attivi',resumen:'Riepilogo',graficos:'Grafici',camposCruzados:'Campi incrociati',aiAnalisis:'Analisi IA',resumenAI:"Riepilogo IA dell'evento",analizando:'Analisi in corso...',pregunta:'Scrivi la tua domanda...',sinEvento:'Nessun evento',cerrarSesion:'Disconnettersi?',confirmar:'Conferma',registros:'registri',noEncontrado:'Non trovato',obligatorio:'obbligatorio',exportado:'Esportato',guardado:'Salvato',reactivado:'Riattivato',salidaReg:'Uscita registrata',llegadaReg:'Arrivo registrato',bloqueado:'Bloccato',sinPermiso:'Senza permesso'},
  pt:{on:'ON',off:'OFF',dashboard:'Painel',referencia:'Referência',ingresos:'Entradas',embalaje:'Embalagem',conductores:'Motoristas',agenda:'Agenda',analisis:'Análises',historial:'Histórico',archivos:'Ficheiros',papelera:'Lixeira',impresion:'Impressão',recintos:'Recintos',eventos:'Eventos',mensajes:'Mensagens',usuarios:'Utilizadores',empresas:'Empresas',migracion:'Migração',buscar:'Pesquisar...',tema:'Tema',salir:'Sair',guardar:'Guardar',cancelar:'Cancelar',eliminar:'Eliminar',importar:'Importar',plantilla:'Modelo',excel:'Excel',limpiar:'Limpar',vaciar:'Esvaziar',nuevo:'Novo',editar:'Editar',ver:'Ver',sinDatos:'Sem dados',sinResultados:'Sem resultados',enRecinto:'No recinto',hoy:'Hoje',total:'Total',lista:'Lista',especial:'Especial',modificaciones:'Alterações',campos:'Campos',acciones:'Ações',estado:'Estado',entrada:'Entrada',salida:'Saída',matricula:'Matrícula',conductor:'Motorista',empresa:'Empresa',hall:'Hall',stand:'Stand',telefono:'Telefone',todos:'Todos',soloActivos:'Só ativos',resumen:'Resumo',graficos:'Gráficos',camposCruzados:'Campos cruzados',aiAnalisis:'Análise IA',resumenAI:'Resumo IA do evento',analizando:'A analisar...',pregunta:'Escreva a sua pergunta...',sinEvento:'Sem evento',cerrarSesion:'Terminar sessão?',confirmar:'Confirmar',registros:'registos',noEncontrado:'Não encontrado',obligatorio:'obrigatório',exportado:'Exportado',guardado:'Guardado',reactivado:'Reativado',salidaReg:'Saída registada',llegadaReg:'Chegada registada',bloqueado:'Bloqueado',sinPermiso:'Sem permissão'},
  pl:{on:'WŁ',off:'WYŁ',dashboard:'Panel',referencia:'Referencja',ingresos:'Wjazdy',embalaje:'Pakowanie',conductores:'Kierowcy',agenda:'Harmonogram',analisis:'Analizy',historial:'Historia',archivos:'Pliki',papelera:'Kosz',impresion:'Druk',recintos:'Obiekty',eventos:'Wydarzenia',mensajes:'Wiadomości',usuarios:'Użytkownicy',empresas:'Firmy',migracion:'Migracja',buscar:'Szukaj...',tema:'Motyw',salir:'Wyloguj',guardar:'Zapisz',cancelar:'Anuluj',eliminar:'Usuń',importar:'Importuj',plantilla:'Szablon',excel:'Excel',limpiar:'Wyczyść',vaciar:'Opróżnij',nuevo:'Nowy',editar:'Edytuj',ver:'Zobacz',sinDatos:'Brak danych',sinResultados:'Brak wyników',enRecinto:'Na miejscu',hoy:'Dziś',total:'Razem',lista:'Lista',especial:'Specjalne',modificaciones:'Zmiany',campos:'Pola',acciones:'Akcje',estado:'Status',entrada:'Wjazd',salida:'Wyjazd',matricula:'Tablica rej.',conductor:'Kierowca',empresa:'Firma',hall:'Hala',stand:'Stoisko',telefono:'Telefon',todos:'Wszystkie',soloActivos:'Tylko aktywne',resumen:'Podsumowanie',graficos:'Wykresy',camposCruzados:'Pola krzyżowe',aiAnalisis:'Analiza AI',resumenAI:'Podsumowanie AI',analizando:'Analizuję...',pregunta:'Wpisz pytanie...',sinEvento:'Brak wydarzenia',cerrarSesion:'Wylogować?',confirmar:'Potwierdź',registros:'rekordy',noEncontrado:'Nie znaleziono',obligatorio:'wymagane',exportado:'Wyeksportowano',guardado:'Zapisano',reactivado:'Reaktywowano',salidaReg:'Wyjazd zarejestrowany',llegadaReg:'Przyjazd zarejestrowany',bloqueado:'Zablokowany',sinPermiso:'Brak uprawnień'},
  ro:{on:'ON',off:'OFF',dashboard:'Panou',referencia:'Referință',ingresos:'Intrări',embalaje:'Ambalare',conductores:'Șoferi',agenda:'Program',analisis:'Analize',historial:'Istoric',archivos:'Fișiere',papelera:'Coș',impresion:'Tipărire',recintos:'Locații',eventos:'Evenimente',mensajes:'Mesaje',usuarios:'Utilizatori',empresas:'Companii',migracion:'Migrare',buscar:'Căutare...',tema:'Temă',salir:'Deconectare',guardar:'Salvare',cancelar:'Anulare',eliminar:'Ștergere',importar:'Import',plantilla:'Șablon',excel:'Excel',limpiar:'Curățare',vaciar:'Golire',nuevo:'Nou',editar:'Editare',ver:'Vezi',sinDatos:'Fără date',sinResultados:'Fără rezultate',enRecinto:'Pe loc',hoy:'Azi',total:'Total',lista:'Listă',especial:'Special',modificaciones:'Modificări',campos:'Câmpuri',acciones:'Acțiuni',estado:'Stare',entrada:'Intrare',salida:'Ieșire',matricula:'Nr. înmatriculare',conductor:'Șofer',empresa:'Firmă',hall:'Hală',stand:'Stand',telefono:'Telefon',todos:'Toate',soloActivos:'Doar active',resumen:'Rezumat',graficos:'Grafice',camposCruzados:'Câmpuri încrucișate',aiAnalisis:'Analiză AI',resumenAI:'Rezumat AI',analizando:'Se analizează...',pregunta:'Scrie întrebarea...',sinEvento:'Fără eveniment',cerrarSesion:'Deconectare?',confirmar:'Confirmă',registros:'înregistrări',noEncontrado:'Negăsit',obligatorio:'obligatoriu',exportado:'Exportat',guardado:'Salvat',reactivado:'Reactivat',salidaReg:'Ieșire înregistrată',llegadaReg:'Sosire înregistrată',bloqueado:'Blocat',sinPermiso:'Fără permisiune'},
  nl:{on:'AAN',off:'UIT',dashboard:'Dashboard',referencia:'Referentie',ingresos:'Ingangen',embalaje:'Verpakking',conductores:'Chauffeurs',agenda:'Planning',analisis:'Analyses',historial:'Geschiedenis',archivos:'Bestanden',papelera:'Prullenbak',impresion:'Afdrukken',recintos:'Locaties',eventos:'Evenementen',mensajes:'Berichten',usuarios:'Gebruikers',empresas:'Bedrijven',migracion:'Migratie',buscar:'Zoeken...',tema:'Thema',salir:'Uitloggen',guardar:'Opslaan',cancelar:'Annuleren',eliminar:'Verwijderen',importar:'Importeren',plantilla:'Sjabloon',excel:'Excel',limpiar:'Opschonen',vaciar:'Legen',nuevo:'Nieuw',editar:'Bewerken',ver:'Bekijken',sinDatos:'Geen gegevens',sinResultados:'Geen resultaten',enRecinto:'Op locatie',hoy:'Vandaag',total:'Totaal',lista:'Lijst',especial:'Speciaal',modificaciones:'Wijzigingen',campos:'Velden',acciones:'Acties',estado:'Status',entrada:'Ingang',salida:'Uitgang',matricula:'Kenteken',conductor:'Chauffeur',empresa:'Bedrijf',hall:'Hal',stand:'Stand',telefono:'Telefoon',todos:'Alle',soloActivos:'Alleen actief',resumen:'Samenvatting',graficos:'Grafieken',camposCruzados:'Kruisvelden',aiAnalisis:'AI-analyse',resumenAI:'AI-samenvatting',analizando:'Analyseren...',pregunta:'Typ uw vraag...',sinEvento:'Geen evenement',cerrarSesion:'Uitloggen?',confirmar:'Bevestigen',registros:'records',noEncontrado:'Niet gevonden',obligatorio:'verplicht',exportado:'Geëxporteerd',guardado:'Opgeslagen',reactivado:'Gereactiveerd',salidaReg:'Uitgang geregistreerd',llegadaReg:'Aankomst geregistreerd',bloqueado:'Geblokkeerd',sinPermiso:'Geen toestemming'},
  hu:{on:'BE',off:'KI',dashboard:'Vezérlőpult',referencia:'Referencia',ingresos:'Bejáratok',embalaje:'Csomagolás',conductores:'Sofőrök',agenda:'Ütemterv',analisis:'Elemzések',historial:'Előzmények',archivos:'Fájlok',papelera:'Lomtár',impresion:'Nyomtatás',recintos:'Helyszínek',eventos:'Események',mensajes:'Üzenetek',usuarios:'Felhasználók',empresas:'Cégek',migracion:'Migráció',buscar:'Keresés...',tema:'Téma',salir:'Kijelentkezés',guardar:'Mentés',cancelar:'Mégse',eliminar:'Törlés',importar:'Importálás',plantilla:'Sablon',excel:'Excel',limpiar:'Tisztítás',vaciar:'Ürítés',nuevo:'Új',editar:'Szerkesztés',ver:'Megtekintés',sinDatos:'Nincs adat',sinResultados:'Nincs találat',enRecinto:'Helyszínen',hoy:'Ma',total:'Összesen',lista:'Lista',especial:'Speciális',modificaciones:'Módosítások',campos:'Mezők',acciones:'Műveletek',estado:'Állapot',entrada:'Bejárat',salida:'Kijárat',matricula:'Rendszám',conductor:'Sofőr',empresa:'Cég',hall:'Csarnok',stand:'Stand',telefono:'Telefon',todos:'Mind',soloActivos:'Csak aktív',resumen:'Összefoglaló',graficos:'Grafikonok',camposCruzados:'Keresztmezők',aiAnalisis:'AI elemzés',resumenAI:'AI összefoglaló',analizando:'Elemzés...',pregunta:'Írd be a kérdésed...',sinEvento:'Nincs esemény',cerrarSesion:'Kijelentkezés?',confirmar:'Megerősítés',registros:'rekord',noEncontrado:'Nem található',obligatorio:'kötelező',exportado:'Exportálva',guardado:'Mentve',reactivado:'Újraaktiválva',salidaReg:'Kijárat rögzítve',llegadaReg:'Érkezés rögzítve',bloqueado:'Zárolva',sinPermiso:'Nincs jogosultság'},
  cs:{on:'ZAP',off:'VYP',dashboard:'Panel',referencia:'Reference',ingresos:'Příjezdy',embalaje:'Balení',conductores:'Řidiči',agenda:'Plán',analisis:'Analýzy',historial:'Historie',archivos:'Soubory',papelera:'Koš',impresion:'Tisk',recintos:'Areály',eventos:'Události',mensajes:'Zprávy',usuarios:'Uživatelé',empresas:'Firmy',migracion:'Migrace',buscar:'Hledat...',tema:'Vzhled',salir:'Odhlásit',guardar:'Uložit',cancelar:'Zrušit',eliminar:'Smazat',importar:'Importovat',plantilla:'Šablona',excel:'Excel',limpiar:'Vyčistit',vaciar:'Vyprázdnit',nuevo:'Nový',editar:'Upravit',ver:'Zobrazit',sinDatos:'Žádná data',sinResultados:'Žádné výsledky',enRecinto:'Na místě',hoy:'Dnes',total:'Celkem',lista:'Seznam',especial:'Speciální',modificaciones:'Změny',campos:'Pole',acciones:'Akce',estado:'Stav',entrada:'Vjezd',salida:'Výjezd',matricula:'SPZ',conductor:'Řidič',empresa:'Firma',hall:'Hala',stand:'Stánek',telefono:'Telefon',todos:'Vše',soloActivos:'Pouze aktivní',resumen:'Shrnutí',graficos:'Grafy',camposCruzados:'Křížová pole',aiAnalisis:'AI analýza',resumenAI:'AI shrnutí',analizando:'Analyzuji...',pregunta:'Napište dotaz...',sinEvento:'Žádná událost',cerrarSesion:'Odhlásit se?',confirmar:'Potvrdit',registros:'záznamů',noEncontrado:'Nenalezeno',obligatorio:'povinné',exportado:'Exportováno',guardado:'Uloženo',reactivado:'Reaktivováno',salidaReg:'Výjezd zaznamenán',llegadaReg:'Příjezd zaznamenán',bloqueado:'Zablokováno',sinPermiso:'Bez oprávnění'},
  hr:{on:'UKL',off:'ISKL',dashboard:'Nadzorna ploča',referencia:'Referenca',ingresos:'Ulazi',embalaje:'Pakiranje',conductores:'Vozači',agenda:'Raspored',analisis:'Analize',historial:'Povijest',archivos:'Datoteke',papelera:'Smeće',impresion:'Ispis',recintos:'Lokacije',eventos:'Događaji',mensajes:'Poruke',usuarios:'Korisnici',empresas:'Tvrtke',migracion:'Migracija',buscar:'Traži...',tema:'Tema',salir:'Odjava',guardar:'Spremi',cancelar:'Odustani',eliminar:'Obriši',importar:'Uvezi',plantilla:'Predložak',excel:'Excel',limpiar:'Očisti',vaciar:'Isprazni',nuevo:'Novi',editar:'Uredi',ver:'Pogledaj',sinDatos:'Nema podataka',sinResultados:'Nema rezultata',enRecinto:'Na lokaciji',hoy:'Danas',total:'Ukupno',lista:'Popis',especial:'Posebno',modificaciones:'Izmjene',campos:'Polja',acciones:'Akcije',estado:'Status',entrada:'Ulaz',salida:'Izlaz',matricula:'Registracija',conductor:'Vozač',empresa:'Tvrtka',hall:'Hala',stand:'Štand',telefono:'Telefon',todos:'Sve',soloActivos:'Samo aktivni',resumen:'Sažetak',graficos:'Grafovi',camposCruzados:'Unakrsna polja',aiAnalisis:'AI analiza',resumenAI:'AI sažetak',analizando:'Analiziram...',pregunta:'Napišite pitanje...',sinEvento:'Nema događaja',cerrarSesion:'Odjaviti se?',confirmar:'Potvrdi',registros:'zapisa',noEncontrado:'Nije pronađeno',obligatorio:'obavezno',exportado:'Izvezeno',guardado:'Spremljeno',reactivado:'Reaktivirano',salidaReg:'Izlaz zabilježen',llegadaReg:'Dolazak zabilježen',bloqueado:'Zaključano',sinPermiso:'Bez dozvole'},
  sk:{on:'ZAP',off:'VYP',dashboard:'Panel',referencia:'Referencia',ingresos:'Príjazdy',embalaje:'Balenie',conductores:'Vodiči',agenda:'Plán',analisis:'Analýzy',historial:'História',archivos:'Súbory',papelera:'Kôš',impresion:'Tlač',recintos:'Areály',eventos:'Udalosti',mensajes:'Správy',usuarios:'Používatelia',empresas:'Firmy',migracion:'Migrácia',buscar:'Hľadať...',tema:'Vzhľad',salir:'Odhlásiť',guardar:'Uložiť',cancelar:'Zrušiť',eliminar:'Zmazať',importar:'Importovať',plantilla:'Šablóna',excel:'Excel',limpiar:'Vyčistiť',vaciar:'Vyprázdniť',nuevo:'Nový',editar:'Upraviť',ver:'Zobraziť',sinDatos:'Žiadne dáta',sinResultados:'Žiadne výsledky',enRecinto:'Na mieste',hoy:'Dnes',total:'Celkom',lista:'Zoznam',especial:'Špeciálne',modificaciones:'Zmeny',campos:'Polia',acciones:'Akcie',estado:'Stav',entrada:'Vjazd',salida:'Výjazd',matricula:'EČV',conductor:'Vodič',empresa:'Firma',hall:'Hala',stand:'Stánok',telefono:'Telefón',todos:'Všetko',soloActivos:'Len aktívne',resumen:'Zhrnutie',graficos:'Grafy',camposCruzados:'Krížové polia',aiAnalisis:'AI analýza',resumenAI:'AI zhrnutie',analizando:'Analyzujem...',pregunta:'Napíšte otázku...',sinEvento:'Žiadna udalosť',cerrarSesion:'Odhlásiť sa?',confirmar:'Potvrdiť',registros:'záznamov',noEncontrado:'Nenájdené',obligatorio:'povinné',exportado:'Exportované',guardado:'Uložené',reactivado:'Reaktivované',salidaReg:'Výjazd zaznamenaný',llegadaReg:'Príjazd zaznamenaný',bloqueado:'Zablokované',sinPermiso:'Bez oprávnenia'},
  sl:{on:'VKL',off:'IZKL',dashboard:'Nadzorna plošča',referencia:'Referenca',ingresos:'Vhodi',embalaje:'Pakiranje',conductores:'Vozniki',agenda:'Urnik',analisis:'Analize',historial:'Zgodovina',archivos:'Datoteke',papelera:'Koš',impresion:'Tiskanje',recintos:'Lokacije',eventos:'Dogodki',mensajes:'Sporočila',usuarios:'Uporabniki',empresas:'Podjetja',migracion:'Migracija',buscar:'Išči...',tema:'Tema',salir:'Odjava',guardar:'Shrani',cancelar:'Prekliči',eliminar:'Izbriši',importar:'Uvozi',plantilla:'Predloga',excel:'Excel',limpiar:'Počisti',vaciar:'Izprazni',nuevo:'Nov',editar:'Uredi',ver:'Poglej',sinDatos:'Ni podatkov',sinResultados:'Ni rezultatov',enRecinto:'Na lokaciji',hoy:'Danes',total:'Skupaj',lista:'Seznam',especial:'Posebno',modificaciones:'Spremembe',campos:'Polja',acciones:'Dejanja',estado:'Stanje',entrada:'Vhod',salida:'Izhod',matricula:'Registrska',conductor:'Voznik',empresa:'Podjetje',hall:'Dvorana',stand:'Stojnica',telefono:'Telefon',todos:'Vse',soloActivos:'Samo aktivni',resumen:'Povzetek',graficos:'Grafi',camposCruzados:'Navzkrižna polja',aiAnalisis:'AI analiza',resumenAI:'AI povzetek',analizando:'Analiziram...',pregunta:'Napišite vprašanje...',sinEvento:'Ni dogodka',cerrarSesion:'Odjaviti se?',confirmar:'Potrdi',registros:'zapisov',noEncontrado:'Ni najdeno',obligatorio:'obvezno',exportado:'Izvoženo',guardado:'Shranjeno',reactivado:'Reaktivirano',salidaReg:'Izhod zabeležen',llegadaReg:'Prihod zabeležen',bloqueado:'Zaklenjeno',sinPermiso:'Brez dovoljenja'},
  sv:{on:'PÅ',off:'AV',dashboard:'Översikt',referencia:'Referens',ingresos:'Inpasseringar',embalaje:'Förpackning',conductores:'Förare',agenda:'Schema',analisis:'Analyser',historial:'Historik',archivos:'Filer',papelera:'Papperskorg',impresion:'Utskrift',recintos:'Platser',eventos:'Evenemang',mensajes:'Meddelanden',usuarios:'Användare',empresas:'Företag',migracion:'Migrering',buscar:'Sök...',tema:'Tema',salir:'Logga ut',guardar:'Spara',cancelar:'Avbryt',eliminar:'Radera',importar:'Importera',plantilla:'Mall',excel:'Excel',limpiar:'Rensa',vaciar:'Töm',nuevo:'Ny',editar:'Redigera',ver:'Visa',sinDatos:'Inga data',sinResultados:'Inga resultat',enRecinto:'På plats',hoy:'Idag',total:'Totalt',lista:'Lista',especial:'Speciell',modificaciones:'Ändringar',campos:'Fält',acciones:'Åtgärder',estado:'Status',entrada:'Infart',salida:'Utfart',matricula:'Registreringsnr',conductor:'Förare',empresa:'Företag',hall:'Hall',stand:'Monter',telefono:'Telefon',todos:'Alla',soloActivos:'Endast aktiva',resumen:'Sammanfattning',graficos:'Diagram',camposCruzados:'Korsfält',aiAnalisis:'AI-analys',resumenAI:'AI-sammanfattning',analizando:'Analyserar...',pregunta:'Skriv din fråga...',sinEvento:'Inget evenemang',cerrarSesion:'Logga ut?',confirmar:'Bekräfta',registros:'poster',noEncontrado:'Hittades ej',obligatorio:'obligatorisk',exportado:'Exporterad',guardado:'Sparad',reactivado:'Reaktiverad',salidaReg:'Utfart registrerad',llegadaReg:'Ankomst registrerad',bloqueado:'Låst',sinPermiso:'Ingen behörighet'},
  fi:{on:'ON',off:'OFF',dashboard:'Hallintapaneeli',referencia:'Viite',ingresos:'Sisääntulot',embalaje:'Pakkaus',conductores:'Kuljettajat',agenda:'Aikataulu',analisis:'Analyysit',historial:'Historia',archivos:'Tiedostot',papelera:'Roskakori',impresion:'Tulostus',recintos:'Kohteet',eventos:'Tapahtumat',mensajes:'Viestit',usuarios:'Käyttäjät',empresas:'Yritykset',migracion:'Migraatio',buscar:'Etsi...',tema:'Teema',salir:'Kirjaudu ulos',guardar:'Tallenna',cancelar:'Peruuta',eliminar:'Poista',importar:'Tuo',plantilla:'Malli',excel:'Excel',limpiar:'Puhdista',vaciar:'Tyhjennä',nuevo:'Uusi',editar:'Muokkaa',ver:'Näytä',sinDatos:'Ei tietoja',sinResultados:'Ei tuloksia',enRecinto:'Paikalla',hoy:'Tänään',total:'Yhteensä',lista:'Lista',especial:'Erikois',modificaciones:'Muutokset',campos:'Kentät',acciones:'Toiminnot',estado:'Tila',entrada:'Sisääntulo',salida:'Ulostulo',matricula:'Rekisterinro',conductor:'Kuljettaja',empresa:'Yritys',hall:'Halli',stand:'Osasto',telefono:'Puhelin',todos:'Kaikki',soloActivos:'Vain aktiiviset',resumen:'Yhteenveto',graficos:'Kaaviot',camposCruzados:'Ristikkäiskentät',aiAnalisis:'AI-analyysi',resumenAI:'AI-yhteenveto',analizando:'Analysoidaan...',pregunta:'Kirjoita kysymyksesi...',sinEvento:'Ei tapahtumaa',cerrarSesion:'Kirjaudu ulos?',confirmar:'Vahvista',registros:'tietuetta',noEncontrado:'Ei löytynyt',obligatorio:'pakollinen',exportado:'Viety',guardado:'Tallennettu',reactivado:'Uudelleenaktivoitu',salidaReg:'Lähtö kirjattu',llegadaReg:'Saapuminen kirjattu',bloqueado:'Lukittu',sinPermiso:'Ei oikeutta'},
  el:{on:'ON',off:'OFF',dashboard:'Πίνακας',referencia:'Αναφορά',ingresos:'Είσοδοι',embalaje:'Συσκευασία',conductores:'Οδηγοί',agenda:'Πρόγραμμα',analisis:'Αναλύσεις',historial:'Ιστορικό',archivos:'Αρχεία',papelera:'Κάδος',impresion:'Εκτύπωση',recintos:'Χώροι',eventos:'Εκδηλώσεις',mensajes:'Μηνύματα',usuarios:'Χρήστες',empresas:'Εταιρείες',migracion:'Μετάβαση',buscar:'Αναζήτηση...',tema:'Θέμα',salir:'Αποσύνδεση',guardar:'Αποθήκευση',cancelar:'Ακύρωση',eliminar:'Διαγραφή',importar:'Εισαγωγή',plantilla:'Πρότυπο',excel:'Excel',limpiar:'Καθαρισμός',vaciar:'Άδειασμα',nuevo:'Νέο',editar:'Επεξεργασία',ver:'Προβολή',sinDatos:'Χωρίς δεδομένα',sinResultados:'Χωρίς αποτελέσματα',enRecinto:'Στον χώρο',hoy:'Σήμερα',total:'Σύνολο',lista:'Λίστα',especial:'Ειδικό',modificaciones:'Αλλαγές',campos:'Πεδία',acciones:'Ενέργειες',estado:'Κατάσταση',entrada:'Είσοδος',salida:'Έξοδος',matricula:'Πινακίδα',conductor:'Οδηγός',empresa:'Εταιρεία',hall:'Αίθουσα',stand:'Περίπτερο',telefono:'Τηλέφωνο',todos:'Όλα',soloActivos:'Μόνο ενεργά',resumen:'Σύνοψη',graficos:'Γραφήματα',camposCruzados:'Διασταυρ. πεδία',aiAnalisis:'AI ανάλυση',resumenAI:'AI σύνοψη',analizando:'Ανάλυση...',pregunta:'Γράψτε την ερώτησή σας...',sinEvento:'Χωρίς εκδήλωση',cerrarSesion:'Αποσύνδεση;',confirmar:'Επιβεβαίωση',registros:'εγγραφές',noEncontrado:'Δεν βρέθηκε',obligatorio:'υποχρεωτικό',exportado:'Εξήχθη',guardado:'Αποθηκεύτηκε',reactivado:'Επανενεργοποιήθηκε',salidaReg:'Έξοδος καταχωρήθηκε',llegadaReg:'Άφιξη καταχωρήθηκε',bloqueado:'Κλειδωμένο',sinPermiso:'Χωρίς άδεια'},
  bg:{on:'ВКЛ',off:'ИЗКЛ',dashboard:'Табло',referencia:'Референция',ingresos:'Входове',embalaje:'Опаковане',conductores:'Шофьори',agenda:'График',analisis:'Анализи',historial:'История',archivos:'Файлове',papelera:'Кошче',impresion:'Печат',recintos:'Обекти',eventos:'Събития',mensajes:'Съобщения',usuarios:'Потребители',empresas:'Фирми',migracion:'Миграция',buscar:'Търси...',tema:'Тема',salir:'Изход',guardar:'Запази',cancelar:'Отмяна',eliminar:'Изтрий',importar:'Импорт',plantilla:'Шаблон',excel:'Excel',limpiar:'Почисти',vaciar:'Изпразни',nuevo:'Нов',editar:'Редакция',ver:'Преглед',sinDatos:'Няма данни',sinResultados:'Няма резултати',enRecinto:'На място',hoy:'Днес',total:'Общо',lista:'Списък',especial:'Специални',modificaciones:'Промени',campos:'Полета',acciones:'Действия',estado:'Статус',entrada:'Вход',salida:'Изход',matricula:'Рег. номер',conductor:'Шофьор',empresa:'Фирма',hall:'Зала',stand:'Щанд',telefono:'Телефон',todos:'Всички',soloActivos:'Само активни',resumen:'Резюме',graficos:'Графики',camposCruzados:'Кръстосани полета',aiAnalisis:'AI анализ',resumenAI:'AI резюме',analizando:'Анализирам...',pregunta:'Напишете въпрос...',sinEvento:'Няма събитие',cerrarSesion:'Изход?',confirmar:'Потвърди',registros:'записа',noEncontrado:'Не е намерено',obligatorio:'задължително',exportado:'Експортирано',guardado:'Запазено',reactivado:'Реактивирано',salidaReg:'Изход записан',llegadaReg:'Пристигане записано',bloqueado:'Заключено',sinPermiso:'Без разрешение'},
  uk:{on:'УВІМК',off:'ВИМК',dashboard:'Панель',referencia:'Довідка',ingresos:'Вʼїзди',embalaje:'Пакування',conductores:'Водії',agenda:'Розклад',analisis:'Аналітика',historial:'Історія',archivos:'Файли',papelera:'Кошик',impresion:'Друк',recintos:'Обʼєкти',eventos:'Події',mensajes:'Повідомлення',usuarios:'Користувачі',empresas:'Компанії',migracion:'Міграція',buscar:'Пошук...',tema:'Тема',salir:'Вийти',guardar:'Зберегти',cancelar:'Скасувати',eliminar:'Видалити',importar:'Імпорт',plantilla:'Шаблон',excel:'Excel',limpiar:'Очистити',vaciar:'Спорожнити',nuevo:'Новий',editar:'Редагувати',ver:'Переглянути',sinDatos:'Немає даних',sinResultados:'Немає результатів',enRecinto:'На місці',hoy:'Сьогодні',total:'Всього',lista:'Список',especial:'Спеціальне',modificaciones:'Зміни',campos:'Поля',acciones:'Дії',estado:'Стан',entrada:'Вʼїзд',salida:'Виїзд',matricula:'Номерний знак',conductor:'Водій',empresa:'Компанія',hall:'Зал',stand:'Стенд',telefono:'Телефон',todos:'Усі',soloActivos:'Тільки активні',resumen:'Підсумок',graficos:'Графіки',camposCruzados:'Перехресні поля',aiAnalisis:'AI аналіз',resumenAI:'AI підсумок',analizando:'Аналізую...',pregunta:'Напишіть запитання...',sinEvento:'Немає події',cerrarSesion:'Вийти?',confirmar:'Підтвердити',registros:'записів',noEncontrado:'Не знайдено',obligatorio:'обовʼязкове',exportado:'Експортовано',guardado:'Збережено',reactivado:'Реактивовано',salidaReg:'Виїзд зареєстровано',llegadaReg:'Прибуття зареєстровано',bloqueado:'Заблоковано',sinPermiso:'Без дозволу'},
  ru:{on:'ВКЛ',off:'ВЫКЛ',dashboard:'Панель',referencia:'Справка',ingresos:'Въезды',embalaje:'Упаковка',conductores:'Водители',agenda:'Расписание',analisis:'Аналитика',historial:'История',archivos:'Файлы',papelera:'Корзина',impresion:'Печать',recintos:'Объекты',eventos:'Мероприятия',mensajes:'Сообщения',usuarios:'Пользователи',empresas:'Компании',migracion:'Миграция',buscar:'Поиск...',tema:'Тема',salir:'Выйти',guardar:'Сохранить',cancelar:'Отмена',eliminar:'Удалить',importar:'Импорт',plantilla:'Шаблон',excel:'Excel',limpiar:'Очистить',vaciar:'Опустошить',nuevo:'Новый',editar:'Редактировать',ver:'Просмотр',sinDatos:'Нет данных',sinResultados:'Нет результатов',enRecinto:'На месте',hoy:'Сегодня',total:'Итого',lista:'Список',especial:'Специальное',modificaciones:'Изменения',campos:'Поля',acciones:'Действия',estado:'Статус',entrada:'Въезд',salida:'Выезд',matricula:'Гос. номер',conductor:'Водитель',empresa:'Компания',hall:'Зал',stand:'Стенд',telefono:'Телефон',todos:'Все',soloActivos:'Только активные',resumen:'Сводка',graficos:'Графики',camposCruzados:'Перекрёстные поля',aiAnalisis:'AI анализ',resumenAI:'AI сводка',analizando:'Анализирую...',pregunta:'Напишите вопрос...',sinEvento:'Нет мероприятия',cerrarSesion:'Выйти?',confirmar:'Подтвердить',registros:'записей',noEncontrado:'Не найдено',obligatorio:'обязательно',exportado:'Экспортировано',guardado:'Сохранено',reactivado:'Реактивировано',salidaReg:'Выезд зарегистрирован',llegadaReg:'Прибытие зарегистрировано',bloqueado:'Заблокировано',sinPermiso:'Нет разрешения'},
  tr:{on:'AÇIK',off:'KAPALI',dashboard:'Panel',referencia:'Referans',ingresos:'Girişler',embalaje:'Ambalaj',conductores:'Sürücüler',agenda:'Program',analisis:'Analizler',historial:'Geçmiş',archivos:'Dosyalar',papelera:'Çöp kutusu',impresion:'Yazdırma',recintos:'Mekanlar',eventos:'Etkinlikler',mensajes:'Mesajlar',usuarios:'Kullanıcılar',empresas:'Şirketler',migracion:'Geçiş',buscar:'Ara...',tema:'Tema',salir:'Çıkış',guardar:'Kaydet',cancelar:'İptal',eliminar:'Sil',importar:'İçe aktar',plantilla:'Şablon',excel:'Excel',limpiar:'Temizle',vaciar:'Boşalt',nuevo:'Yeni',editar:'Düzenle',ver:'Görüntüle',sinDatos:'Veri yok',sinResultados:'Sonuç yok',enRecinto:'Alanda',hoy:'Bugün',total:'Toplam',lista:'Liste',especial:'Özel',modificaciones:'Değişiklikler',campos:'Alanlar',acciones:'İşlemler',estado:'Durum',entrada:'Giriş',salida:'Çıkış',matricula:'Plaka',conductor:'Sürücü',empresa:'Şirket',hall:'Salon',stand:'Stand',telefono:'Telefon',todos:'Tümü',soloActivos:'Sadece aktif',resumen:'Özet',graficos:'Grafikler',camposCruzados:'Çapraz alanlar',aiAnalisis:'AI analiz',resumenAI:'AI özet',analizando:'Analiz ediliyor...',pregunta:'Sorunuzu yazın...',sinEvento:'Etkinlik yok',cerrarSesion:'Çıkış yapılsın mı?',confirmar:'Onayla',registros:'kayıt',noEncontrado:'Bulunamadı',obligatorio:'zorunlu',exportado:'Dışa aktarıldı',guardado:'Kaydedildi',reactivado:'Yeniden etkinleştirildi',salidaReg:'Çıkış kaydedildi',llegadaReg:'Varış kaydedildi',bloqueado:'Kilitli',sinPermiso:'İzin yok'},
  ar:{on:'تشغيل',off:'إيقاف',dashboard:'لوحة التحكم',referencia:'المرجع',ingresos:'الدخول',embalaje:'التعبئة',conductores:'السائقون',agenda:'الجدول',analisis:'التحليلات',historial:'السجل',archivos:'الملفات',papelera:'سلة المحذوفات',impresion:'الطباعة',recintos:'المواقع',eventos:'الفعاليات',mensajes:'الرسائل',usuarios:'المستخدمون',empresas:'الشركات',migracion:'الترحيل',buscar:'بحث...',tema:'المظهر',salir:'خروج',guardar:'حفظ',cancelar:'إلغاء',eliminar:'حذف',importar:'استيراد',plantilla:'قالب',excel:'Excel',limpiar:'تنظيف',vaciar:'إفراغ',nuevo:'جديد',editar:'تعديل',ver:'عرض',sinDatos:'لا توجد بيانات',sinResultados:'لا توجد نتائج',enRecinto:'في الموقع',hoy:'اليوم',total:'الإجمالي',lista:'قائمة',especial:'خاص',modificaciones:'التعديلات',campos:'الحقول',acciones:'الإجراءات',estado:'الحالة',entrada:'دخول',salida:'خروج',matricula:'لوحة الترخيص',conductor:'سائق',empresa:'شركة',hall:'قاعة',stand:'جناح',telefono:'هاتف',todos:'الكل',soloActivos:'النشطة فقط',resumen:'ملخص',graficos:'رسوم بيانية',camposCruzados:'حقول متقاطعة',aiAnalisis:'تحليل AI',resumenAI:'ملخص AI',analizando:'جاري التحليل...',pregunta:'اكتب سؤالك...',sinEvento:'لا توجد فعالية',cerrarSesion:'تسجيل الخروج؟',confirmar:'تأكيد',registros:'سجلات',noEncontrado:'غير موجود',obligatorio:'مطلوب',exportado:'تم التصدير',guardado:'تم الحفظ',reactivado:'أعيد تفعيله',salidaReg:'تم تسجيل الخروج',llegadaReg:'تم تسجيل الوصول',bloqueado:'محظور',sinPermiso:'بدون إذن'},
  ca:{on:'ON',off:'OFF',dashboard:'Tauler',referencia:'Referència',ingresos:'Entrades',embalaje:'Embalatge',conductores:'Conductors',agenda:'Agenda',analisis:'Anàlisis',historial:'Historial',archivos:'Fitxers',papelera:'Paperera',impresion:'Impressió',recintos:'Recintes',eventos:'Esdeveniments',mensajes:'Missatges',usuarios:'Usuaris',empresas:'Empreses',migracion:'Migració',buscar:'Cercar...',tema:'Tema',salir:'Sortir',guardar:'Desar',cancelar:'Cancel·lar',eliminar:'Eliminar',importar:'Importar',plantilla:'Plantilla',excel:'Excel',limpiar:'Netejar',vaciar:'Buidar',nuevo:'Nou',editar:'Editar',ver:'Veure',sinDatos:'Sense dades',sinResultados:'Sense resultats',enRecinto:'Al recinte',hoy:'Avui',total:'Total',lista:'Llista',especial:'Especial',modificaciones:'Modificacions',campos:'Camps',acciones:'Accions',estado:'Estat',entrada:'Entrada',salida:'Sortida',matricula:'Matrícula',conductor:'Conductor',empresa:'Empresa',hall:'Pavelló',stand:'Estand',telefono:'Telèfon',todos:'Tots',soloActivos:'Només actius',resumen:'Resum',graficos:'Gràfics',camposCruzados:'Camps encreuats',aiAnalisis:'Anàlisi IA',resumenAI:"Resum IA de l'esdeveniment",analizando:'Analitzant...',pregunta:'Escriu la teva pregunta...',sinEvento:'Sense esdeveniment',cerrarSesion:'Tancar sessió?',confirmar:'Confirmar',registros:'registres',noEncontrado:'No trobat',obligatorio:'obligatori',exportado:'Exportat',guardado:'Desat',reactivado:'Reactivat',salidaReg:'Sortida registrada',llegadaReg:'Arribada registrada',bloqueado:'Bloquejat',sinPermiso:'Sense permís'},
  eu:{on:'ON',off:'OFF',dashboard:'Aginte-mahaia',referencia:'Erreferentzia',ingresos:'Sarrerak',embalaje:'Ontziratzea',conductores:'Gidariak',agenda:'Agenda',analisis:'Analisiak',historial:'Historiala',archivos:'Fitxategiak',papelera:'Zakarrontzia',impresion:'Inprimatzea',recintos:'Guneak',eventos:'Ekitaldiak',mensajes:'Mezuak',usuarios:'Erabiltzaileak',empresas:'Enpresak',migracion:'Migrazioa',buscar:'Bilatu...',tema:'Gaia',salir:'Irten',guardar:'Gorde',cancelar:'Ezeztatu',eliminar:'Ezabatu',importar:'Inportatu',plantilla:'Txantiloia',excel:'Excel',limpiar:'Garbitu',vaciar:'Hustu',nuevo:'Berria',editar:'Editatu',ver:'Ikusi',sinDatos:'Daturik ez',sinResultados:'Emaitzarik ez',enRecinto:'Gunean',hoy:'Gaur',total:'Guztira',lista:'Zerrenda',especial:'Berezia',modificaciones:'Aldaketak',campos:'Eremuak',acciones:'Ekintzak',estado:'Egoera',entrada:'Sarrera',salida:'Irteera',matricula:'Matrikula',conductor:'Gidaria',empresa:'Enpresa',hall:'Aretoa',stand:'Erakuslekua',telefono:'Telefonoa',todos:'Denak',soloActivos:'Aktiboak soilik',resumen:'Laburpena',graficos:'Grafikoak',camposCruzados:'Gurutzatutako eremuak',aiAnalisis:'AI analisia',resumenAI:'AI laburpena',analizando:'Aztertzen...',pregunta:'Idatzi zure galdera...',sinEvento:'Ekitaldirik ez',cerrarSesion:'Saioa itxi?',confirmar:'Berretsi',registros:'erregistro',noEncontrado:'Ez da aurkitu',obligatorio:'derrigorrezkoa',exportado:'Esportatuta',guardado:'Gordeta',reactivado:'Berraktibatuta',salidaReg:'Irteera erregistratuta',llegadaReg:'Iritsiera erregistratuta',bloqueado:'Blokeatuta',sinPermiso:'Baimenik gabe'},
  gl:{on:'ON',off:'OFF',dashboard:'Panel',referencia:'Referencia',ingresos:'Entradas',embalaje:'Embalaxe',conductores:'Condutores',agenda:'Axenda',analisis:'Análises',historial:'Historial',archivos:'Ficheiros',papelera:'Papeleira',impresion:'Impresión',recintos:'Recintos',eventos:'Eventos',mensajes:'Mensaxes',usuarios:'Usuarios',empresas:'Empresas',migracion:'Migración',buscar:'Buscar...',tema:'Tema',salir:'Saír',guardar:'Gardar',cancelar:'Cancelar',eliminar:'Eliminar',importar:'Importar',plantilla:'Modelo',excel:'Excel',limpiar:'Limpar',vaciar:'Baleirar',nuevo:'Novo',editar:'Editar',ver:'Ver',sinDatos:'Sen datos',sinResultados:'Sen resultados',enRecinto:'No recinto',hoy:'Hoxe',total:'Total',lista:'Lista',especial:'Especial',modificaciones:'Modificacións',campos:'Campos',acciones:'Accións',estado:'Estado',entrada:'Entrada',salida:'Saída',matricula:'Matrícula',conductor:'Condutor',empresa:'Empresa',hall:'Pavillón',stand:'Stand',telefono:'Teléfono',todos:'Todos',soloActivos:'Só activos',resumen:'Resumo',graficos:'Gráficos',camposCruzados:'Campos cruzados',aiAnalisis:'Análise IA',resumenAI:'Resumo IA do evento',analizando:'Analizando...',pregunta:'Escribe a túa pregunta...',sinEvento:'Sen evento',cerrarSesion:'Pechar sesión?',confirmar:'Confirmar',registros:'rexistros',noEncontrado:'Non atopado',obligatorio:'obrigatorio',exportado:'Exportado',guardado:'Gardado',reactivado:'Reactivado',salidaReg:'Saída rexistrada',llegadaReg:'Chegada rexistrada',bloqueado:'Bloqueado',sinPermiso:'Sen permiso'},
};
...
// ═══════════════════════════════════════════════════════════════════════
function tr(k){ return (I18N_OP[CUR_LANG]&&I18N_OP[CUR_LANG][k])||I18N_OP.es[k]||k; }
...
function askDelEvento(id){if(!isSA()){toast('Solo SuperAdmin','var(--red)');return;}const ev=DB.eventos.find(x=>x.id===id);if(!ev)return;askDel('Eliminar evento','<b>'+ev.nombre+'</b>',()=>{if(!DB.eventosPapelera)DB.eventosPapelera=[];DB.eventosPapelera.unshift({...ev,_deletedBy:_getCU()?.nombre||'?',_deletedTs:nowLocal()});DB.eventos=DB.eventos.filter(x=>x.id!==id);if(DB.activeEventId===id){DB.activeEventId=null;DB.activeEventIds=[];}logAudit('del_evento','evento','Eliminado: '+ev.nombre);saveDB();renderEventosTab();});}
...
function showAgDetalle(id){
  ...
  setTimeout(()=>{const s=document.getElementById('beuModalSave');if(s)s.style.display='none';},50);
}
...
// ═══ EXPORT FUNCTIONS ═══
function exportIngresos(){if(!canExport()){toast('Sin permiso','var(--red)');return;}if(!DB.ingresos.length){toast('Sin datos','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(DB.ingresos.map(i=>({Pos:i.pos||'',Matricula:i.matricula,Llamador:i.llamador||'',Referencia:i.referencia||'',Nombre:i.nombre||'',Apellido:i.apellido||'',Empresa:i.empresa||'',Montador:i.montador||'',Expositor:i.expositor||'',Hall:(i.halls||[i.hall||'']).join('/'),Stand:i.stand||'',Remolque:i.remolque||'',Telefono:i.telefono||'',Email:i.email||'',Comentario:i.comentario||'',Entrada:fmt(i.entrada),Salida:i.salida?fmt(i.salida):'En recinto'}))),'Ingresos');const fn='ingresos_'+new Date().toISOString().slice(0,10)+'.xlsx';XLSX.writeFile(wb,fn);logExport('Ingresos',fn);toast('✅ Exportado');}
function exportFlota(){if(!canExport()){toast('Sin permiso','var(--red)');return;}if(!DB.movimientos.length){toast('Sin datos','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(DB.movimientos.map(m=>({Pos:m.posicion||'',Estado:m.status,Tractora:m.matricula,Remolque:m.remolque||'',Empresa:m.empresa||'',Hall:m.hall||''}))),'Flota');const fn='flota_'+new Date().toISOString().slice(0,10)+'.xlsx';XLSX.writeFile(wb,fn);logExport('Flota',fn);toast('✅ Exportado');}
function exportConductores(){if(!canExport()){toast('Sin permiso','var(--red)');return;}if(!DB.conductores.length){toast('Sin datos','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(DB.conductores.map(c=>({Nombre:c.nombre,Apellido:c.apellido,Empresa:c.empresa||'',Matricula:c.matricula||'',Telefono:c.telefono||''}))),'Conductores');XLSX.writeFile(wb,'conductores.xlsx');logExport('Conductores','conductores.xlsx');toast('✅ Exportado');}
function exportAgenda(){if(!canExport()){toast('Sin permiso','var(--red)');return;}if(!DB.agenda.length){toast('Sin datos','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(DB.agenda.map(a=>({Fecha:a.fecha,HoraPlan:a.hora,HoraReal:a.horaReal||'',Matricula:a.matricula,Conductor:a.conductor||'',Empresa:a.empresa||'',Hall:a.hall||'',Estado:a.estado}))),'Agenda');const fn='agenda_'+new Date().toISOString().slice(0,10)+'.xlsx';XLSX.writeFile(wb,fn);logExport('Agenda',fn);toast('✅ Exportado');}
function exportListaNegra(){if(!canExport()){toast('Sin permiso','var(--red)');return;}if(!DB.listaNegra.length){toast('Sin datos','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(DB.listaNegra.map(ln=>({Matricula:ln.matricula,Nivel:ln.nivel,Motivo:ln.motivo,Empresa:ln.empresa||'',Hasta:ln.hasta||''}))),'ListaNegra');XLSX.writeFile(wb,'listanegra.xlsx');toast('✅ Exportado');}
function exportMensajes(){if(!canExport()){toast('Sin permiso','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(DB.mensajesRampa.map(m=>({Tipo:m.tipo,Titulo:m.titulo,Mensaje:m.mensaje,Autor:m.autor,Fecha:m.ts}))),'Mensajes');XLSX.writeFile(wb,'mensajes.xlsx');toast('✅ Exportado');}
function exportEventos(){if(!canExport()){toast('Sin permiso','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(DB.eventos.map(e=>({Nombre:e.nombre,Inicio:e.ini,Fin:e.fin,Recinto:e.recinto||'',Ciudad:e.ciudad||'',Activo:e.activo?'SI':'NO'}))),'Eventos');XLSX.writeFile(wb,'eventos.xlsx');toast('✅ Exportado');}
...
function renderCamposSubtab(col){if(!canCampos())return '<div style="padding:16px;text-align:center;color:var(--text3);font-size:12px">Sin permiso para configurar campos visibles.</div>';return _renderCamposSubtab(col);}

function renderMensajesTab(){
  const el=document.getElementById('tabContent');if(!el)return;
  const msgs=DB.mensajesRampa||[];
  el.innerHTML=`<div class="sec-hdr"><div class="sec-ttl">📢 Mensajes de Rampa (${msgs.length})</div><div class="sec-act">${canExport()?'<button class="btn btn-gh btn-sm" onclick="exportMensajes()">⬇ Excel</button>':''}${isSup()?`<button class="btn btn-p btn-sm" onclick="window._op.openMsgModal()">+ Mensaje</button>`:''}${msgs.length?`<button class="btn btn-gh btn-sm" onclick="marcarTodosMsgLeidos()">✓ Leer todos</button>`:''}</div></div>
...
function openMovModal(m){
  ...
  _modal(m?'Editar movimiento':'Nuevo movimiento',`
    ...
    const mat=normPlate(gv('fmMat'));if(!mat){toast('Matrícula obligatoria','var(--red)');return;}
    ...
    document.getElementById('beuModal').remove();
    saveDB();renderFlota();toast('✅ Guardado','var(--green)');
  });
...
window._op = {
  ...
  openEmpresaModal: (id) => {
    ...
    `, async () => {
      ...
      const data = { id: emp.id||uid(), nombre:nom, cif:gv('emCif'), email:gv('emEmail'), contacto:gv('emCont'), tel:gv('emTel'), nivel: emp.nivel||'semi', vehiculos:emp.vehiculos||[] };
  ...
  resetAllData, openLangPicker, setLang, handleLogout, cycleTheme,
...
window.debounceSearch = (k, fn) => { clearTimeout(window['_dbt_'+k]); window['_dbt_'+k] = setTimeout(fn, 50); };
...
// Analytics aliases
window._anaSub = 'resumen';
window._anaCol = 'Referencia';
window._anaField1 = '';
window._anaField2 = '';
window._anaAIQuery = '';
window._anaAIResult = '';
window._op.runAIAnalysis = runAIAnalysis;
window._op.renderAnalytics = renderAnalytics;

// Dashboard AI summary
async function dashAISummary() {
  const btn = document.getElementById('dashAIBtn');
  const res = document.getElementById('dashAIResult');
  if (!res) return;
  res.style.display = 'block';
  res.textContent = '🔄 Generando resumen...';
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Analizando...'; }
  const allIngs = [...DB.ingresos, ...(DB.ingresos2||[])];
  const today = new Date().toISOString().slice(0,10);
  const summary = `Ref:${DB.ingresos.length} Ing:${(DB.ingresos2||[]).length} EnRecinto:${allIngs.filter(i=>!i.salida).length} Hoy:${allIngs.filter(i=>i.entrada?.startsWith(today)).length} Agenda:${(DB.agenda||[]).length} Conductores:${(DB.conductores||[]).length} Mensajes:${(DB.mensajesRampa||[]).length}`;
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 500, messages: [{ role: 'user', content: `Eres un asistente de control logístico de eventos. Da un resumen ejecutivo breve (max 150 palabras) en español del estado actual del evento con estos datos: ${summary}. Usa emojis. Incluye 2-3 recomendaciones.` }] })
    });
    const data = await resp.json();
    res.textContent = data.content?.map(b=>b.text||'').join('\n') || 'Sin respuesta';
  } catch(e) {
    res.textContent = `📊 Resumen local:\n📌 ${DB.ingresos.length} ref. + ${(DB.ingresos2||[]).length} ing. registrados\n🏭 ${allIngs.filter(i=>!i.salida).length} vehículos en recinto\n📅 ${allIngs.filter(i=>i.entrada?.startsWith(today)).length} llegadas hoy\n📋 ${(DB.agenda||[]).length} citas en agenda\n📢 ${(DB.mensajesRampa||[]).length} mensajes`;
  }
  if (btn) { btn.disabled = false; btn.textContent = '🤖 Resumen AI del evento'; }
}
window._op.dashAISummary = dashAISummary;
...
function globalSearch(q) {
  ...
  q = q.toLowerCase();
  const results = [];
  DB.ingresos.forEach(i => { if (`${i.matricula} ${i.nombre||''} ${i.empresa||''} ${i.referencia||''}`.toLowerCase().includes(q)) results.push({tab:'ingresos',label:'Ref: '+i.matricula,id:i.id}); });
  (DB.ingresos2||[]).forEach(i => { if (`${i.matricula} ${i.nombre||''} ${i.empresa||''}`.toLowerCase().includes(q)) results.push({tab:'ingresos2',label:'Ing: '+i.matricula,id:i.id}); });
  (DB.agenda||[]).forEach(a => { if (`${a.matricula||''} ${a.conductor||''} ${a.empresa||''}`.toLowerCase().includes(q)) results.push({tab:'agenda',label:'Ag: '+(a.matricula||a.conductor),id:a.id}); });
  (DB.conductores||[]).forEach(c => { if (`${c.nombre||''} ${c.empresa||''} ${c.matricula||''}`.toLowerCase().includes(q)) results.push({tab:'conductores',label:'Cond: '+c.nombre,id:c.id}); });
  // Show results dropdown
  let dd = document.getElementById('globalSearchDD');
  if (!dd) { dd = document.createElement('div'); dd.id = 'globalSearchDD'; dd.style.cssText = 'position:absolute;top:100%;left:0;right:0;background:var(--bg2);border:1.5px solid var(--border2);border-radius:8px;max-height:200px;overflow-y:auto;z-index:300;box-shadow:0 8px 24px rgba(0,0,0,.15)'; document.getElementById('globalSearch').parentElement.appendChild(dd); }
  if (!results.length) { dd.innerHTML = '<div style="padding:8px;font-size:11px;color:var(--text3)">Sin resultados</div>'; return; }
  dd.innerHTML = results.slice(0,10).map(r => `<div style="padding:6px 10px;cursor:pointer;font-size:12px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between" onmousedown="window._op.goTab('${r.tab}');document.getElementById('globalSearchDD').remove()"><span>${esc(r.label)}</span><span style="font-size:10px;color:var(--text3)">${r.tab}</span></div>`).join('');
}
window._op.globalSearch = globalSearch;
// Close dropdown on blur
document.addEventListener('click', e => { if (!e.target.closest('#globalSearch')&&!e.target.closest('#globalSearchDD')) { const dd=document.getElementById('globalSearchDD'); if(dd) dd.remove(); }});

// Generic template download for any collection
function dlTemplateForCol(col) {
  const templates = {
    ingresos: [['Matricula','Llamador','Referencia','Nombre','Apellido','Empresa','Montador','Expositor','Hall','Stand','Remolque','Pasaporte','Telefono','Email','Comentario','Idioma','Pos']],
    ingresos2: [['Matricula','Nombre','Apellido','Empresa','Hall','Stand','Remolque','Telefono','Comentario','Pos']],
    conductores: [['Nombre','Apellido','Empresa','Matricula','Remolque','Telefono','Hall','TipoVehiculo','Idioma']],
    agenda: [['Matricula','Fecha','HoraPlan','Remolque','Conductor','Empresa','Hall','Stand','Telefono','Notas']],
    movimientos: [['Matricula','Remolque','Nombre','Apellido','Empresa','Hall','TipoCarga','Status','Posicion']],
    mensajesRampa: [['Tipo','Titulo','Mensaje']],
    editHistory: [['Matricula','Accion','Usuario','Detalle']],
    usuarios: [['Nombre','Email','Rol','PIN']],
    empresas: [['Nombre','CIF','Email','Contacto','Telefono','Tipo']],
    preregistros: [['Matricula','Empresa','Conductor','Telefono','Fecha','Hora']],
  };
  const header = templates[col] || [['Campo1','Campo2','Campo3']];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(header), col.slice(0,31));
  XLSX.writeFile(wb, 'plantilla_' + col + '.xlsx');
  toast('📋 Plantilla descargada', 'var(--blue)');
}
window._op.dlTemplateForCol = dlTemplateForCol;
