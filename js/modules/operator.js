...
function renderIngresos() {
  ...
  let h = `<div style="display:flex;gap:4px;margin-bottom:6px;flex-wrap:wrap">
    <button class="btn btn-sm btn-p">📋 Lista</button>
    ...
    sorted.forEach(i => {
      ...
      h += `<tr style="${inRec?'background:var(--gll)':''}">
        ...
        <td><div style="display:flex;gap:2px;flex-wrap:wrap">
          ...
          ${canStatus()&&inRec?`<button class="btn btn-xs btn-warning" onclick="window._op.registrarSalida('${i.id}','ingresos')">↩ Salida</button>`:''}
          ${canStatus()&&!inRec?`<button class="btn btn-xs btn-s" onclick="window._op.reactivar('${i.id}','ingresos')">↺ Reactivar</button>`:''}
    ...
    sorted.forEach(i => {
      ...
      h += `<tr style="${inRec?'background:var(--gll)':''}">
        ...
        <td><div style="display:flex;gap:2px">
          ...
          ${canStatus()&&inRec?`<button class="btn btn-xs btn-warning" onclick="window._op.registrarSalida('${i.id}','ingresos2')">↩</button>`:''}
...
function openUserModal(id) {
  ...
  _modal(existing ? 'Editar usuario' : 'Nuevo usuario', `
    <div class="sg sg2">
      ...
      ${!existing?`
      <div class="fg"><label class="flbl">Contraseña (acceso email) <span style="font-size:10px;color:var(--text3)">mín. 8 caracteres</span></label><input id="uPass" type="password" placeholder="Contraseña para entrar con email"></div>
      <div class="fg"><label class="flbl">PIN (acceso rápido) <span style="font-size:10px;color:var(--text3)">mín. 4 dígitos</span></label><input id="uPin" type="password" maxlength="8" inputmode="numeric" placeholder="----"></div>
      `:''}
    </div>
    <div class="fg"><label class="flbl" style="margin-bottom:4px">Pestañas habilitadas</label>
      <div style="display:flex;flex-wrap:wrap;gap:4px" id="uTabsWrap">
        ${TAB_DEFS.map(t=>`<label style="display:flex;align-items:center;gap:3px;font-size:11px;cursor:pointer"><input type="checkbox" class="uTabChk" value="${t.id}" ${(u.tabs||DEFAULT_TABS[u.rol||'visor']||DEFAULT_TABS.visor||[]).includes(t.id)?'checked':''}>${t.ico} ${t.label}</label>`).join('')}
      </div>
    </div>
  `, async () => {
    ...
    const saveBtn = document.querySelector('#dynModal .btn-p');
    if (saveBtn) { saveBtn.disabled=true; saveBtn.textContent='Guardando…'; }

    try {
      let userId = u.id || uid();

      // NEW user with email+password → create in Firebase Auth
      if (!existing && email && pass && pass.length >= 8) {
        try {
          ...
          if (e.code === 'auth/email-already-in-use') {
            toast('⚠️ Email ya existe en Firebase Auth — creando solo doc Firestore', 'var(--amber)');
          } else {
            toast('❌ Error creando usuario: ' + e.message, 'var(--red)');
      ...
      const data = { id:userId, nombre:nom, email, rol, lang:gv('uLang'), tabs };
      ...
      if (!DB.usuarios.find(x=>x.id===data.id)) DB.usuarios.push(data);
      else { const idx=DB.usuarios.findIndex(x=>x.id===data.id); if(idx>=0)DB.usuarios[idx]=data; }
...
function _renderListaNegra(el) {
  ...
  <div class="sec-hdr">
    ...
    <div class="sec-act">
      ...
      <button class="btn btn-gh btn-sm" onclick="window._op.exportExcel('listaNegra')">⬇ Excel</button>
    </div>
  </div>
  ...
  if (!items.length) {
    ...
    items.forEach(ln => {
      ...
      h += `<tr>
        ...
        <td><div style="display:flex;gap:2px">
          ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="window._op.openLNModal('${ln.id}')">✏️</button>`:''}
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${ln.id}','listaNegra')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
    ...
    items.forEach(e => {
      ...
      h += `<tr>
        ...
        <td><div style="display:flex;gap:2px">
          ...
          ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="window._op.openEEModal('${e.id}')">✏️</button>`:''}
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${e.id}','enEspera')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
...
function renderEmpresasTab() {
  ...
  <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;margin-bottom:8px">
    ...
    <button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(sub==='empresas'?'empresas':'preregistros')">⬇ Excel</button>
  </div>`;
      ...
      filtered.forEach(emp => {
        ...
        h += `<tr>
          ...
          <td><div style="display:flex;gap:2px">
            ...
            ${isSA() ? `<button class="btn btn-edit btn-xs" onclick="window._op.openEmpresaModal('${emp.id}')">✏️</button>` : ''}
            ...
            ${isSA() && emp.nivel === 'blocked'  ? `<button class="btn btn-gh btn-xs" onclick="window._op.setEmpresaNivel('${emp.id}','semi')">↩</button>` : ''}
          </div></td>
        </tr>`;
      });
      h += '</tbody></table></div>';
...
window._op = {
  ...
  openLNModal, openEEModal, marcarEELlegado,
