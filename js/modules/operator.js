...
function _ingEventos(tab){
  ...
  ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>'+_M('events')+'</th><th>Fechas</th><th>Recinto</th><th>Estado</th><th>Puertas</th><th>Acc.</th></tr></thead><tbody>
...
function renderEventosTab() {
  ...
  if (!items.length) { h += '<div class="empty"><div class="ei">📅</div><div class="et">Sin eventos configurados</div></div>'; }
    ...
    h += '<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>'+_M('events')+'</th><th>Fechas</th><th>Recinto</th><th>Estado</th><th></th></tr></thead><tbody>';
    items.forEach(ev => {
...
const _MOD_I18N={
  es:{newEntry:'Nueva entrada',editEntry:'Editar ingreso',newVeh:'Añadir vehículo',editVeh:'Editar vehículo embalaje',comment:'Comentario',driverLang:'Idioma conductor',position:'Posición',cargoType:'Tipo Carga',status:'Estado',save:'Guardar',cancel:'Cancelar',plateMandatory:_M('plateMandatory'),saved:_M('saved'),add:_M('add'),newDriver:'Nuevo conductor',editDriver:'Editar conductor',newBooking:'Nueva reserva',editBooking:'Editar reserva',date:'Fecha',time:'Hora',schedTime:'Hora prevista',realTime:'Hora real',bookingStatus:'Estado',notes:'Notas',refBooking:'Referencia/Booking',blocked:'🚫 MATRÍCULA BLOQUEADA',continueQ:'¿Continuar de todas formas?',reason:'Motivo',specialList:'en lista especial',
    // Dashboard
    goodMorning:''+_M('goodMorning')+'',goodAfternoon:'Buenas tardes',goodEvening:'Buenas noches',noActiveEvent:'Sin evento activo',onPremises:''+_M('onPremises')+'',references:'Referencias',entries:'Ingresos',today:'Hoy',agendaToday:'Agenda hoy',messages:'Mensajes',last7days:'Ingresos últimos 7 días',refVsIng:'Ref vs Ing',activeHalls:'' + _M('activeHalls') + '',noHallData:'' + _M('noHallData') + '',noApptToday:'' + _M('noApptToday') + '',viewAgenda:'' + _M('viewAgenda') + '',lastEntries:'' + _M('lastEntries') + '',noEntriesReg:'' + _M('noEntriesReg') + '',unreadMsg:'mensaje(s) sin leer',view:'Ver',all:'Todos',
    // Subtabs & buttons
    list:'Lista',special:'Especial',modifications:'Modificaciones',fields:'Campos',import:'Importar',template:'Plantilla',excel:'Excel',clean:'Limpiar',empty:'Vaciar',activeOnly:''+_M('activeOnly')+'',searchPlaceholder:'Pos, matrícula, nombre...',reg:'reg.',noRecords:_M('noRecords'),noPermImport:''+_M('noPermImport')+'',
    // Table headers
    ...
    // Actions
    markExit:''+_M('markExit')+'',reactivate:'Reactivar',delete:'Eliminar',edit:'Editar',print:'Imprimir',details:'Detalles',confirm:'Confirmar',
    // Flota/Embalaje
    ...
    printing:'Impresión',
    // Recintos
    venues:'Recintos',newVenue:''+_M('newVenue')+'',editVenue:'Editar recinto',venueName:''+_M('venueName')+'',halls:'Halls',gates:'Puertas',
    // Eventos
    events:'Eventos',newEvent:'Nuevo evento',editEvent:'Editar evento',eventName:'Nombre evento',startDate:''+_M('startDate')+'',endDate:''+_M('endDate')+'',activate:'Activar',deactivate:'Desactivar',
    // Mensajes
  ...
  pt:{newEntry:'Nova entrada',editEntry:'Editar entrada',newVeh:'Adicionar veículo',editVeh:'Editar veículo',comment:'Comentário',driverLang:'Idioma condutor',position:'Posição',cargoType:'Tipo carga',status:'Estado',save:'Guardar',cancel:'Cancelar',plateMandatory:'Matrícula obrigatória',saved:_M('saved'),add:'+ Adicionar',newDriver:'Novo condutor',editDriver:'Editar condutor',newBooking:'Nova reserva',editBooking:'Editar reserva',date:'Data',time:'Hora',schedTime:'Hora prevista',realTime:'Hora real',bookingStatus:'Estado',notes:'Notas',refBooking:'Referência/Reserva',blocked:'🚫 MATRÍCULA BLOQUEADA',continueQ:'Continuar mesmo assim?',reason:'Motivo',specialList:'em lista especial',
    goodMorning:'Bom dia',goodAfternoon:'Boa tarde',goodEvening:'Boa noite',noActiveEvent:'Sem evento ativo',onPremises:'No recinto',references:'Referências',entries:'Entradas',today:'Hoje',agendaToday:'Agenda hoje',messages:'Mensagens',last7days:'Entradas últimos 7 dias',refVsIng:'Ref vs Ent',activeHalls:'Halls mais ativos',noHallData:'Sem dados hall',noApptToday:'Sem compromissos',viewAgenda:'' + _M('viewAgenda') + '',lastEntries:'Últimas entradas',noEntriesReg:'Sem entradas registadas',unreadMsg:'mensagem(ns) não lida(s)',view:'Ver',all:'Todos',
