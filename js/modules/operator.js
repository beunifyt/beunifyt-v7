...
function _shellHTML(user, ev) {
  ...
  return `
<style>
#opShellWrap{display:flex;flex-direction:column;min-height:100vh;background:var(--bg)}
#opHdr{
  background:var(--bg2);border-bottom:1px solid var(--border);
  padding:0 12px;height:44px;display:flex;align-items:center;
  position:sticky;top:0;z-index:200;gap:6px
}
#opHdr .op-logo{display:flex;align-items:center;gap:6px;flex-shrink:0}
#opHdr .op-logo-txt{font-size:16px;font-weight:900;letter-spacing:-.4px;white-space:nowrap;color:var(--text)}
#opHdr .op-logo-txt span{color:var(--text3);font-weight:400}
.op-cnts{display:flex;align-items:center;gap:4px;flex:1;justify-content:center;overflow-x:auto;padding:0 6px;scrollbar-width:none}
.op-cnt{display:flex;flex-direction:column;align-items:center;padding:3px 7px;border:1.5px solid var(--border2);border-radius:var(--r);background:var(--bg3);min-width:40px;flex-shrink:0}
.op-cv{font-size:15px;font-weight:900;line-height:1.1;font-family:'JetBrains Mono',monospace;color:var(--text)}
.op-cl{font-size:8px;color:var(--text3);font-weight:700;margin-top:2px;text-transform:uppercase}
.op-hdr-r{display:flex;align-items:center;gap:3px;flex-shrink:0}
.op-sep{width:1px;height:20px;background:var(--border);display:inline-block;margin:0 4px}
#opTabs{display:flex;align-items:center;gap:2px;padding:2px 8px;background:var(--bg3);border-bottom:1px solid var(--border);overflow-x:auto;position:sticky;top:44px;z-index:199;scrollbar-width:none;justify-content:center;flex-wrap:wrap}
#opTabs::-webkit-scrollbar{display:none}
#opContent{padding:6px 10px;max-width:1400px;margin:0 auto;width:100%}
</style>
...
<div id="opHdr">
  <div class="op-logo">
    ...
    <span class="op-logo-txt">BeUnify<span>T</span></span>
  ...
  <div class="op-cnts">
    ...
    <div class="op-cnt" style="border-color:var(--amber)"><span class="op-cv" id="cntRef" style="color:var(--amber)">0</span><span class="op-cl">REF.</span></div>
  </div>
  ...
  <div class="op-hdr-r">
    ...
    <span class="op-sep"></span>
    <button class="btn btn-gh btn-sm" id="btnTheme" style="font-size:12px;border-radius:20px;padding:3px 9px;flex-shrink:0">☀️ Tema</button>
    <button class="btn btn-gh btn-sm" id="btnLang" style="font-size:14px;border-radius:20px;padding:3px 8px;flex-shrink:0" title="Idioma">🌐</button>
    <button class="btn btn-gh btn-sm" id="btnExportDay" style="font-size:14px;border-radius:20px;padding:3px 8px;flex-shrink:0" title="Backup del dia">💾</button>
    <button class="btn btn-gh btn-sm" id="btnLogout" style="border-radius:20px;padding:3px 10px;font-size:11px;font-weight:700;display:inline-flex;align-items:center;gap:4px;flex-shrink:0">
...
function _renderTabla(tabId) {
  ...
  cnt.innerHTML = `
    <!-- SUBTABS + ACCIONES identico a v6 -->
    <div style="display:flex;align-items:center;gap:3px;padding:4px 0;flex-wrap:nowrap;min-height:34px;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none">
      ...
      ${_perms.canEdit?`<button class="btn btn-sm btn-gh" onclick="window._opToggleAF()" style="flex-shrink:0;border-radius:20px" id="btnAF">⚡ ON</button>`:''}
      ${_perms.canEdit?`<button class="btn btn-sm btn-gh" onclick="window._opTogglePos()" style="flex-shrink:0;border-radius:20px" id="btnPos">🔢 ON</button>`:''}
    ...
    ${items.map(i=>`<tr>
      ...
      <td style="white-space:nowrap">
        <!-- Botones en columna identicos a v6 -->
        <div style="display:flex;flex-direction:column;gap:3px;align-items:flex-start">
          <div style="display:flex;gap:2px">
            ${_perms.canPrint?`<button class="btn btn-gh btn-xs" onclick="window._opPrint('${i.id}')" title="Imprimir Normal" style="border-radius:50%;width:26px;height:26px;padding:0;display:inline-flex;align-items:center;justify-content:center">🖨</button>`:''}
            ${_perms.canPrint?`<button class="btn btn-xs" onclick="window._opPrintTrq('${i.id}')" title="Imprimir Troquelado" style="background:#7c3aed;color:#fff;border-radius:50%;width:26px;height:26px;padding:0;display:inline-flex;align-items:center;justify-content:center">✂</button>`:''}
            ${_perms.canEdit?`<button class="btn btn-edit btn-xs" onclick="window._opEdit('${i.id}')" style="border-radius:50%;width:26px;height:26px;padding:0;display:inline-flex;align-items:center;justify-content:center">✏️</button>`:''}
          </div>
          ...
          ${i.salida&&_perms.canEdit?`<button class="btn btn-success btn-xs" onclick="window._opReactivar('${i.id}')" style="border-radius:20px;padding:2px 10px;font-size:10px;font-weight:700">↺ Reactivar</button>`:''}
          ${_perms.canDelete?`<button class="btn btn-danger btn-xs" onclick="window._opDel('${i.id}')" style="border-radius:50%;width:26px;height:26px;padding:0;display:inline-flex;align-items:center;justify-content:center">🗑</button>`:''}
        </div>
      </td>
    </tr>`).join('')}
    </tbody></table></div>`
...
window._vaciarTab  = ()=>toast('Vaciar — requiere confirmacion admin','var(--red)');
window._opImportar = ()=>toast('Importar Excel — disponible en proxima version','var(--blue)');
window._opPlantilla= ()=>toast('Plantilla Excel — disponible en proxima version','var(--blue)');
window._opToggleAF = ()=>{ const b=document.getElementById('btnAF'); if(b) b.textContent=b.textContent.includes('ON')?'⚡ OFF':'⚡ ON'; };
window._opTogglePos= ()=>{ const b=document.getElementById('btnPos'); if(b) b.textContent=b.textContent.includes('ON')?'🔢 OFF':'🔢 ON'; };
...
function _bindShell() {
  ...
  document.getElementById('btnLang')?.addEventListener('click', ()=>{
    toast('Selector de idioma — disponible en proxima version','var(--blue)');
  });
  document.getElementById('btnExportDay')?.addEventListener('click', ()=>{
    window._exportExcel(_activeTab==='referencia'?'referencia':'ingresos');
  });
