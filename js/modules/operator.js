...
// ── Global aliases so v6 onclick handlers work in module scope ──
window.goTab = goTab;
...
window.showIngDetalle = (id) => window._op.showDetalle(id, 'ingreso');
window.marcarSalidaIng = (id) => window._op.registrarSalida(id, 'ingresos');
window.reactivarIngreso = (id) => window._op.reactivar(id, 'ingresos');
window.askDelIng = (id) => window._op.eliminar(id, 'ingresos');
window.exportIngresos = () => window._op.exportExcel('ingresos');
window.cleanTab = (tab) => window._op.vaciarTab(tab);
window.openEEModal = (item) => window._op.openIngModal(item?.id || null, 'espera');
window.askDelEE = (id) => window._op.eliminar(id, 'espera');
window.marcarEELlegado = (id) => window._op.registrarSalida(id, 'espera');
window.renderCamposSubtab = _renderCamposSubtab;
window.sortIngTable = (col) => window._op.sortTab && window._op.sortTab('ingresos', col);
window.debounceSearch = (k, fn) => { clearTimeout(window._dbt); window._dbt = setTimeout(fn, 200); };

window.renderFlota = renderFlota;
window.renderConductores = renderConductores;
window.renderAgenda = renderAgenda;
window.askDelFlota = (id) => window._op.eliminar(id, 'flota');
window.marcarSalidaFlota = (id) => window._op.registrarSalida(id, 'flota');
window.askDelCond = (id) => window._op.eliminar(id, 'conductores');
window.exportConductores = () => window._op.exportExcel('conductores');
window.askDelAgenda = (id) => window._op.eliminar(id, 'agenda');
window.marcarCitaHecha = (id) => window._op.setAgendaEstado(id, 'HECHO');
