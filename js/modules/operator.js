...
function renderIngresos() {
  ...
  if (_ingSub === 'historial')  { window._op.goTab('vehiculos'); return; }
  ...
  const _subLbl = _ingSub;
  let h = '';

  // Toolbar - subtab buttons with active state
  h += '<div style="display:flex;align-items:center;gap:3px;padding:4px 0;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none;flex-wrap:nowrap">';
  h += '<button class="btn btn-sm ' + (_subLbl==='lista'?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._ingSub=\'lista\';window._op.renderIngresos()">📋 Lista</button>';
  h += '<button class="btn btn-sm ' + (_subLbl==='listanegra'?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._ingSub=\'listanegra\';window._op.renderIngresos()">⭐ Especial (' + (DB.listaNegra||[]).length + ')</button>';
  h += '<button class="btn btn-sm ' + (_subLbl==='historial'?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._ingSub=\'historial\';window._op.renderIngresos()">✏️ Modificaciones</button>';
  h += '<button class="btn btn-sm ' + (_subLbl==='espera'?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._ingSub=\'espera\';window._op.renderIngresos()">⏳ En espera (' + (DB.enEspera||[]).filter(e=>e.estado==='pendiente').length + ')</button>';
  if (canCampos()) h += '<button class="btn btn-sm ' + (_subLbl==='campos'?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._ingSub=\'campos\';window._op.renderIngresos()">⊙ Campos</button>';
  h += '<span style="flex:1"></span>';
  if (canAdd()) h += '<button class="btn btn-sm" style="background:#0d9f6e;color:#fff;border:none;border-radius:20px;font-weight:700;flex-shrink:0" onclick="window._op.openIngModal(null)">+ Ingreso</button>';
  h += '<button class="btn btn-sm ' + (_autoFillOn?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._op.toggleAutoFill()">⚡ ' + (_autoFillOn?'ON':'OFF') + '</button>';
  h += '<button class="btn btn-sm ' + (_posAutoOn?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._op.togglePosAuto()">🔢 ' + (_posAutoOn?'ON':'OFF') + '</button>';
  if (canImport()) {
    h += '<button class="btn btn-gh btn-sm" style="flex-shrink:0" onclick="window._op.dlTemplateIngresos()"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg> Plantilla</button>';
    h += '<label class="btn btn-gh btn-sm" style="cursor:pointer;flex-shrink:0"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Importar<input type="file" accept=".xlsx,.xls" style="display:none" onchange="window._op.importExcel(this,\'ingresos\')"></label>';
  }
  if (canExport()) h += '<button class="btn btn-gh btn-sm" style="flex-shrink:0" onclick="window._op.exportExcel(\'ingresos\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Excel</button>';
  if (canClean()) h += '<button class="btn btn-gh btn-sm" style="flex-shrink:0" onclick="window._op.vaciarTab(\'ingresos\')">Limpiar</button>';
  if (isSA()) h += '<button class="btn btn-danger btn-sm" style="flex-shrink:0" onclick="window._op.vaciarTab(\'ingresos\')">💥 Vaciar</button>';
  h += '</div>';

  // Search row
  h += '<div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;overflow-x:auto;scrollbar-width:none;flex-wrap:nowrap">';
  h += '<div class="sbox" style="flex:0 1 240px;min-width:120px"><span class="sico">🔍</span><input type="search" placeholder="Pos, matricula, nombre..." value="' + esc(iF.q||'') + '" oninput="window._iF.q=this.value;window._op.renderIngresos()" id="qIngInput" style="font-size:12px"></div>';
  h += '<input type="date" value="' + (iF.fecha||'') + '" oninput="window._iF.fecha=this.value;window._op.renderIngresos()" style="height:30px;padding:3px 6px;font-size:11px;width:115px;box-sizing:border-box">';
  h += '<span class="pill" style="font-size:11px;cursor:pointer;flex-shrink:0;border:1.5px solid ' + (iF.activos?'var(--blue)':'var(--border)') + ';background:' + (iF.activos?'var(--blue)':'var(--bg2)') + ';color:' + (iF.activos?'#fff':'var(--text3)') + '" onclick="window._iF.activos=!window._iF.activos;window._op.renderIngresos()">Solo activos</span>';
  if (iF.q||iF.fecha||iF.activos) h += '<span class="pill" style="cursor:pointer;background:var(--rll);color:var(--red);border:1px solid #fecaca;font-size:11px;flex-shrink:0" onclick="window._iF={q:\'\',fecha:\'\',hall:\'\',activos:false};window._op.renderIngresos()">✕</span>';
  h += '<span style="font-size:10px;color:var(--text3)">' + sorted.length + ' reg.</span>';
  h += '</div>';
  ...
  if (halls.length) {
    ...
    h += '<span class="pill" style="font-size:10px;font-weight:700;padding:2px 8px;cursor:pointer;background:' + (!iF.hall?'#e0f2fe':'#dbeafe') + ';color:' + (!iF.hall?'#0369a1':'#1e40af') + ';border:1.5px solid ' + (!iF.hall?'#7dd3fc':'#93c5fd') + '" onclick="window._iF.hall=\'\';window._op.renderIngresos()">Todos</span>';
    halls.forEach(hall => {
      h += '<span class="pill" style="font-size:10px;font-weight:700;padding:2px 8px;cursor:pointer;background:' + (iF.hall===hall?'#3b82f6':'#dbeafe') + ';color:' + (iF.hall===hall?'#fff':'#1e40af') + ';border:1.5px solid ' + (iF.hall===hall?'#2563eb':'#93c5fd') + '" onclick="window._iF.hall=\'' + hall + '\';window._op.renderIngresos()">' + hall + '</span>';
    });
    h += '</div>';
  }
...
function renderIngresos2() {
  ...
  const _subLbl2 = window._ingSub2 || 'lista';
  let h = '';

  // Toolbar ingresos2 - with active state
  h += '<div style="display:flex;align-items:center;gap:3px;padding:4px 0;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none;flex-wrap:nowrap">';
  h += '<button class="btn btn-sm ' + (_subLbl2==='lista'?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._ingSub2=\'lista\';window._op.renderIngresos2()">📋 Lista</button>';
  h += '<button class="btn btn-sm ' + (_subLbl2==='listanegra'?'btn-p':'btn-gh') + '" style="flex-shrink:0" onclick="window._ingSub2=\'listanegra\';window._op.renderIngresos2()">⭐ Especial</button>';
  h += '<button class="btn btn-sm btn-gh" style="flex-shrink:0" onclick="window._op.goTab(\'vehiculos\')">✏️ Modif.</button>';
  if (canCampos()) h += '<button class="btn btn-sm btn-gh" style="flex-shrink:0" onclick="window._ingSub2=\'campos\';window._op.renderIngresos2()">⊙ Campos</button>';
  h += '<span style="flex:1"></span>';
  if (canAdd()) h += '<button class="btn btn-sm" style="background:#0d9f6e;color:#fff;border:none;border-radius:20px;font-weight:700;flex-shrink:0" onclick="window._op.openIngModal(null,\'ingresos2\')">+ Ingreso</button>';
  if (canImport()) {
    h += '<button class="btn btn-gh btn-sm" style="flex-shrink:0" onclick="window._op.dlTemplateIngresos2()"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg> Plantilla</button>';
  ...
  if (isSA()) h += '<button class="btn btn-danger btn-sm" style="flex-shrink:0" onclick="window._op.vaciarTab(\'ingresos2\')">💥 Vaciar</button>';
  h += '</div>';

  // Search row
  h += '<div style="display:flex;align-items:center;gap:4px;margin-bottom:4px;flex-wrap:nowrap">';
  h += '<div class="sbox" style="flex:0 1 240px;min-width:120px"><span class="sico">🔍</span><input type="search" placeholder="Pos, matrícula, nombre..." value="' + esc(iF.q2||'') + '" oninput="window._iF.q2=this.value;window._op.renderIngresos2()" style="font-size:12px"></div>';
  h += '<input type="date" value="' + (iF.fecha2||'') + '" oninput="window._iF.fecha2=this.value;window._op.renderIngresos2()" style="height:30px;padding:3px 6px;font-size:11px;width:115px;box-sizing:border-box">';
  h += '<span class="pill" style="font-size:11px;cursor:pointer;flex-shrink:0;border:1.5px solid ' + (iF.activos2?'var(--blue)':'var(--border)') + ';background:' + (iF.activos2?'var(--blue)':'var(--bg2)') + ';color:' + (iF.activos2?'#fff':'var(--text3)') + '" onclick="window._iF.activos2=!window._iF.activos2;window._op.renderIngresos2()">Solo activos</span>';
  if (iF.q2||iF.fecha2||iF.activos2) h += '<span class="pill" style="cursor:pointer;background:var(--rll);color:var(--red);border:1px solid #fecaca;font-size:11px;flex-shrink:0" onclick="window._iF.q2=\'\';window._iF.fecha2=\'\';window._iF.activos2=false;window._op.renderIngresos2()">✕</span>';
  h += '<span style="font-size:10px;color:var(--text3)">' + sorted.length + ' reg.</span>';
  h += '</div>';
...
function renderAgenda() {
  const el = document.getElementById('tabContent'); if (!el) return;
  const _agSub = window._agSub || 'lista';
  if (_agSub === 'campos') { _renderCamposSubtab(el, 'agenda'); return; }
  const today = new Date().toISOString().slice(0, 10);
  ...
  <div style="display:flex;align-items:center;gap:3px;padding:4px 0;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none;flex-wrap:nowrap">
    ...
    <span style="flex:1"></span>
    ${canCampos()?'<button class="btn btn-sm btn-gh" style="flex-shrink:0" onclick="window._agSub=\'campos\';window._op.renderAgenda()">⊙ Campos</button>':''}
