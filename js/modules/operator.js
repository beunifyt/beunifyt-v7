...
const DEFAULT_TABS = {
  superadmin:['dash','ingresos','ingresos2','flota','conductores','agenda','analytics','vehiculos','auditoria','papelera','recintos','eventos','mensajes','usuarios','impresion','empresas'],
  supervisor:['dash','ingresos','ingresos2','flota','conductores','agenda','analytics','vehiculos','auditoria','papelera','recintos','eventos','usuarios','impresion'],
  controlador_rampa:['ingresos','ingresos2'],
  editor:['ingresos','ingresos2','conductores','agenda','impresion'],
  visor:['ingresos','ingresos2','agenda'],
  empresa:['empresas']
...
// ═══════════════════════════════════════════════════════════════════════
function _modal(title, body, onSave) {
  const existing = document.getElementById('beuModal');
  if (existing) existing.remove();
  const ov = document.createElement('div');
  ov.id = 'beuModal';
  ov.className = 'ov open';
  ov.innerHTML = `<div class="modal"><div class="mhdr"><span class="mttl">${title}</span><button class="btn-x" onclick="document.getElementById('beuModal').remove()">✕</button></div><div id="beuModalBody">${body}</div><div class="ffoot"><button class="btn btn-gh" onclick="document.getElementById('beuModal').remove()">Cancelar</button><button class="btn btn-p" id="beuModalSave">Guardar</button></div></div>`;
  document.body.appendChild(ov);
  ov.addEventListener('click', e => { if (e.target === ov) ov.remove(); });
  document.getElementById('beuModalSave').addEventListener('click', () => { if (onSave) onSave(); });
}
...
function marcarSalidaIng2(id){const i=(DB.ingresos2||[]).find(x=>x.id===id);if(!i)return;i.salida=nowLocal();saveDB();renderIngresos2();renderHdr();}
function reactivarIngreso2(id){const i=(DB.ingresos2||[]).find(x=>x.id===id);if(!i)return;i.salida=null;saveDB();renderIngresos2();renderHdr();toast('↺ Salida anulada','var(--amber)');}
...
function printIngreso2(id){toast('🖨 Imprimiendo...','var(--blue)');if(window._imp&&window._imp.printIngreso2)window._imp.printIngreso2(id);}

// ═══ DETAIL VIEWS ═══
function showIngDetalle(id,source){
  source=source||'ingresos';
  const item=[...DB.ingresos,...(DB.ingresos2||[])].find(x=>x.id===id);
  if(!item){toast('No encontrado','var(--red)');return;}
  const trackH=item.tracking&&item.tracking.length?`<div style="margin-top:10px"><div style="font-size:11px;font-weight:800;margin-bottom:4px">📡 Tracking</div><div class="track-row">${TRACKING_STEPS.map(s=>{const t=item.tracking.find(x=>x.step===s.id);const cls=t?'td-done':'';return`<div class="track-step ${cls}"><div class="track-dot">${s.ico}</div><div class="track-lbl">${s.name}</div>${t?`<div class="track-time">${fmt(t.ts,'t')}</div>`:''}</div>`;}).join('')}</div></div>`:'';
  _modal(item.matricula+' — Detalle',`
    <div class="sg sg2">
      <div class="fg"><span class="flbl">Matrícula</span><span class="mchip">${item.matricula}</span></div>
      <div class="fg"><span class="flbl">Pos</span><b>${item.pos||'–'}</b></div>
      <div class="fg"><span class="flbl">Llamador</span>${item.llamador||'–'}</div>
      <div class="fg"><span class="flbl">Ref</span>${item.referencia||'–'}</div>
      <div class="fg"><span class="flbl">Conductor</span>${item.nombre||''} ${item.apellido||''}</div>
      <div class="fg"><span class="flbl">Empresa</span>${item.empresa||'–'}</div>
      <div class="fg"><span class="flbl">Hall</span>${(item.halls||[item.hall||'']).filter(Boolean).map(h=>hBadge(h)).join(' ')||'–'}</div>
      <div class="fg"><span class="flbl">Stand</span>${item.stand||'–'}</div>
      <div class="fg"><span class="flbl">Remolque</span>${item.remolque||'–'}</div>
      <div class="fg"><span class="flbl">Teléfono</span>${item.telefono?telLink(item.telPais||'',item.telefono):'–'}</div>
      <div class="fg"><span class="flbl">Entrada</span>${fmt(item.entrada)}</div>
      <div class="fg"><span class="flbl">Salida</span>${item.salida?fmt(item.salida):'<span class="pill pill-g">En recinto</span>'}</div>
      <div class="fg"><span class="flbl">Comentario</span>${item.comentario||'–'}</div>
      <div class="fg"><span class="flbl">Evento</span>${item.eventoNombre||'–'}</div>
    </div>${trackH}
  `, null);
  // Remove save button for detail view
...
function exportIngresos2(){if(!canExport()){toast('Sin permiso','var(--red)');return;}if(!(DB.ingresos2||[]).length){toast('Sin datos','var(--red)');return;}const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet((DB.ingresos2||[]).map(i=>({Pos:i.pos||'',Matricula:i.matricula,Nombre:i.nombre||'',Apellido:i.apellido||'',Empresa:i.empresa||'',Hall:(i.halls||[i.hall||'']).join('/'),Stand:i.stand||'',Telefono:i.telefono||'',Entrada:fmt(i.entrada),Salida:i.salida?fmt(i.salida):'En recinto'}))),'Ingresos');const fn='ingresos_libre_'+new Date().toISOString().slice(0,10)+'.xlsx';XLSX.writeFile(wb,fn);logExport('Ingresos-Libre',fn);toast('✅ Exportado');}
...
function dlTemplateFlota(){const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([['Matricula','Remolque','Nombre','Apellido','Empresa','Hall','TipoCarga','Status','Posicion']]),'Embalaje');XLSX.writeFile(wb,'plantilla_embalaje.xlsx');toast('📥 Plantilla','var(--blue)');}
