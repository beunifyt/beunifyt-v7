...
import { isSA, isSup, hasPerm, canEdit, canExport, canImport, canClean, logout } from '../auth.js';
...
let DB = {
  ...
  mensajesRampa:[], auditLog:[], exportLog:[], editHistory:[],
...
export async function initOperator() {
  ...
  // Restore last tab
  try { curTab = localStorage.getItem('beu_tab') || 'dash'; } catch {}
  _validateTab();
...
async function _loadData() {
  ...
  try {
    DB.usuarios    = await fsGetAll('users');
...
.ev-pill{background:#e0f2fe;border:1px solid #7dd3fc;color:#0369a1;border-radius:20px;padding:2px 8px;font-size:11px;font-weight:700}
...
<div id="appHdr">
  <div style="display:flex;align-items:center;gap:7px;flex-shrink:0">
    <svg viewBox="0 0 140 140" width="22" height="22"><rect width="140" height="140" rx="28" fill="#030812"/><polygon points="70,10 122,40 122,100 70,130 18,100 18,40" stroke="#00ffc8" stroke-width="2" fill="#00ffc808"/><circle cx="70" cy="70" r="9" fill="#00ffc8"/></svg>
    <span style="font-family:monospace;font-size:14px;font-weight:700"><span style="color:#00b89a">Be</span>Unify<span style="color:#00b89a">T</span></span>
  </div>
  ...
  <div style="display:flex;align-items:center;gap:3px;flex-shrink:0">
    ...
    <button style="border:none;background:none;font-size:12px;font-weight:500;color:var(--text2);cursor:pointer" onclick="window._op.openLangPicker()">🌐</button>
    <span style="font-size:12px;font-weight:500;color:var(--text2)">${esc(user?.nombre || 'Usuario')}</span>
    <span style="width:1px;height:20px;background:var(--border);margin:0 4px;display:inline-block"></span>
    <button class="btn btn-gh btn-sm" onclick="window._op.handleLogout()">→ Salir</button>
  </div>
</div>

<div id="mainTabs">
  ${tabs.map(t => `<button class="btn-tab" data-tab="${t.id}" onclick="window._op.goTab('${t.id}')">${t.ico} ${t.label}</button>`).join('')}
</div>
...
const TAB_DEFS = [
  ...
  {id:'ingresos',   label:'Referencia',  ico:'🔖'},
  {id:'ingresos2',  label:'Ingresos',    ico:'🚛'},
  {id:'flota',      label:'Embalaje',    ico:'📦'},
  {id:'conductores',label:'Conductores', ico:'👤'},
  {id:'agenda',     label:'Agenda',      ico:'📅'},
  {id:'analytics',  label:'Análisis',    ico:'📈'},
  {id:'vehiculos',  label:'Historial',   ico:'📜'},
  {id:'auditoria',  label:'Archivos',    ico:'📂'},
  {id:'papelera',   label:'Papelera',    ico:'🗑'},
  {id:'impresion',  label:'Impresión',   ico:'🖨'},
  {id:'recintos',   label:'Recintos',    ico:'🏟'},
  {id:'eventos',    label:'Eventos',     ico:'📅'},
  {id:'mensajes',   label:'Mensajes',    ico:'📢'},
  {id:'usuarios',   label:'Usuarios',    ico:'👥'},
];

const DEFAULT_TABS = {
  superadmin:        ['dash','ingresos','ingresos2','flota','conductores','agenda','analytics','vehiculos','auditoria','papelera','impresion','recintos','eventos','mensajes','usuarios'],
...
function goTab(tab) {
  curTab = tab;
  try { localStorage.setItem('beu_tab', tab); } catch {}
...
function renderHdr() {
  ...
  el.innerHTML = `
    ${ev ? `<span class="ev-pill" style="cursor:pointer" onclick="window._op.goTab('eventos')">${ev.ico||'📋'} ${esc(ev.nombre)}</span>` : '<span style="font-size:11px;color:var(--text3)">Sin evento activo</span>'}
...
function renderIngresos() {
  ...
  let h = `<div class="sec-hdr">
    <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;width:100%">
      <div class="sbox" style="flex:1;min-width:200px">
        <span class="sico">🔍</span>
        <input type="text" placeholder="Buscar matrícula, empresa, referencia, hall..." value="${esc(iF.q||'')}" oninput="window._iF.q=this.value;window._op.renderIngresos()" id="qIngInput">
      </div>
      <button class="btn btn-xs ${iF.activos?'btn-p':'btn-gh'}" onclick="window._iF.activos=!window._iF.activos;window._op.renderIngresos()">🟢 En recinto${iF.activos?' ✓':''}</button>
      <span style="font-size:11px;color:var(--text3);flex-shrink:0">${sorted.length} total · ${enRec} en recinto · ${hoy} hoy</span>
      ${canEdit()?'<button class="btn btn-p btn-sm" onclick="window._op.openIngModal(null)">+ Nueva entrada</button>':''}
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'ingresos\')">⬇ Excel</button>':''}
      ${canImport()?'<label class="btn btn-gh btn-sm" style="cursor:pointer">📥 Importar<input type="file" accept=".xlsx,.xls" style="display:none" onchange="window._op.importExcel(this,\'ingresos\')"></label>':''}
      ${isSup()?'<button class="btn btn-danger btn-sm" onclick="window._op.vaciarTab(\'ingresos\')">💥 Vaciar</button>':''}
    </div>
  </div>`;
  ...
  if (!sorted.length) {
    ...
    h += '<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>#</th><th>Matrícula</th><th>Empresa</th><th>Montador</th><th>Hall</th><th>Stand</th><th>Referencia</th><th>Hora entrada</th><th>Estado</th><th></th></tr></thead><tbody>';
    sorted.forEach(i => {
      ...
      h += `<tr style="${inRec?'background:var(--gll)':''}">
        ...
        <td><div style="display:flex;gap:2px;flex-wrap:wrap">
          ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="window._op.openIngModal('${i.id}','ingresos')">✏️</button>`:''}
          ${canEdit()&&inRec?`<button class="btn btn-xs btn-warning" onclick="window._op.registrarSalida('${i.id}','ingresos')">↩ Salida</button>`:''}
          ${canEdit()&&!inRec?`<button class="btn btn-xs btn-s" onclick="window._op.reactivar('${i.id}','ingresos')">↺ Reactivar</button>`:''}
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${i.id}','ingresos')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
...
function renderIngresos2() {
  ...
  let h = `<div class="sec-hdr">
    <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;width:100%">
      <div class="sbox" style="flex:1;min-width:200px">
        <span class="sico">🔍</span>
        <input type="text" placeholder="Buscar matrícula, empresa, conductor..." value="${esc(iF.q2||'')}" oninput="window._iF.q2=this.value;window._op.renderIngresos2()" id="qIng2Input">
      </div>
      <button class="btn btn-xs ${iF.activos2?'btn-p':'btn-gh'}" onclick="window._iF.activos2=!window._iF.activos2;window._op.renderIngresos2()">🟢 En recinto${iF.activos2?' ✓':''}</button>
      <span style="font-size:11px;color:var(--text3)">${sorted.length} total · ${items.filter(i=>!i.salida).length} en recinto · ${items.filter(i=>i.entrada?.startsWith(today)).length} hoy</span>
      ${canEdit()?'<button class="btn btn-p btn-sm" onclick="window._op.openIngModal(null,\'ingresos2\')">+ Nueva entrada</button>':''}
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'ingresos2\')">⬇ Excel</button>':''}
      ${canImport()?'<label class="btn btn-gh btn-sm" style="cursor:pointer">📥 Importar<input type="file" accept=".xlsx,.xls" style="display:none" onchange="window._op.importExcel(this,\'ingresos2\')"></label>':''}
    </div>
  </div>`;
  ...
  if (!sorted.length) {
    ...
    h += '<div class="tbl-wrap"><table class="dtbl"><thead><tr><th>#</th><th>Matrícula</th><th>Nombre/Conductor</th><th>Empresa</th><th>Hall</th><th>Hora entrada</th><th>Estado</th><th></th></tr></thead><tbody>';
    sorted.forEach(i => {
      const inRec = !i.salida;
      ...
      h += `<tr style="${inRec?'background:var(--gll)':''}">
        ...
        <td><div style="display:flex;gap:2px">
          ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="window._op.openIngModal('${i.id}','ingresos2')">✏️</button>`:''}
          ${canEdit()&&inRec?`<button class="btn btn-xs btn-warning" onclick="window._op.registrarSalida('${i.id}','ingresos2')">↩</button>`:''}
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${i.id}','ingresos2')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
...
function renderFlota() {
  ...
  const filtered = q ? items.filter(i => `${i.matricula} ${i.empresa||''} ${i.hall||''} ${i.tipoCarga||''} ${i.status||''} ${i.posicion||''}`.toLowerCase().includes(q)) : items;

  let h = `<div class="sec-hdr">
    <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;width:100%">
      <div class="sbox" style="flex:1;min-width:200px"><span class="sico">🔍</span><input type="text" placeholder="Buscar matrícula, empresa, hall..." value="${esc(iF.qFlota||'')}" oninput="window._iF.qFlota=this.value;window._op.renderFlota()"></div>
      <span style="font-size:11px;color:var(--text3)">${filtered.length} vehículos</span>
      ${canEdit()?'<button class="btn btn-p btn-sm" onclick="window._op.openFlotaModal(null)">+ Añadir</button>':''}
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'movimientos\')">⬇ Excel</button>':''}
      ${canImport()?'<label class="btn btn-gh btn-sm" style="cursor:pointer">📥 Importar<input type="file" accept=".xlsx,.xls" style="display:none" onchange="window._op.importExcel(this,\'movimientos\')"></label>':''}
    </div>
  </div>`;
  ...
  if (!filtered.length) { h += '<div class="empty"><div class="ei">📦</div><div class="et">Sin vehículos en embalaje</div></div>'; }
    ...
    filtered.forEach(i => {
      ...
      h += `<tr>
        ...
        <td><div style="display:flex;gap:2px">
          ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="window._op.openFlotaModal('${i.id}')">✏️</button>`:''}
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${i.id}','movimientos')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
...
function renderConductores() {
  ...
  let h = `<div class="sec-hdr">
    <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;width:100%">
      <div class="sbox" style="flex:1;min-width:200px"><span class="sico">🔍</span><input type="text" placeholder="Buscar nombre, matrícula, empresa..." value="${esc(iF.qCond||'')}" oninput="window._iF.qCond=this.value;window._op.renderConductores()"></div>
      <span style="font-size:11px;color:var(--text3)">${sorted.length} conductores</span>
      ${canEdit()?'<button class="btn btn-p btn-sm" onclick="window._op.openCondModal(null)">+ Añadir</button>':''}
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'conductores\')">⬇ Excel</button>':''}
    </div>
  </div>`;
  ...
  if (!sorted.length) { h += '<div class="empty"><div class="ei">👤</div><div class="et">Sin conductores</div></div>'; }
    ...
    sorted.forEach(c => {
      h += `<tr>
        ...
        <td><div style="display:flex;gap:2px">
          ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="window._op.openCondModal('${c.id}')">✏️</button>`:''}
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${c.id}','conductores')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
...
function renderAgenda() {
  ...
  let h = `<div class="sec-hdr">
    <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;width:100%">
      <input type="date" value="${fecha}" onchange="window._iF.agFecha=this.value;window._op.renderAgenda()" style="font-size:12px;padding:4px 8px;border-radius:var(--r)">
      <div class="sbox" style="flex:1;min-width:160px"><span class="sico">🔍</span><input type="text" placeholder="Buscar..." value="${esc(iF.qAg||'')}" oninput="window._iF.qAg=this.value;window._op.renderAgenda()"></div>
      <span style="font-size:11px;color:var(--text3)">${sorted.length} citas</span>
      ${canEdit()?'<button class="btn btn-p btn-sm" onclick="window._op.openAgendaModal(null)">+ Nueva cita</button>':''}
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'agenda\')">⬇ Excel</button>':''}
    </div>
  </div>`;
  ...
  if (!sorted.length) { h += '<div class="empty"><div class="ei">📅</div><div class="et">Sin citas para esta fecha</div></div>'; }
    ...
    sorted.forEach(a => {
      h += `<tr>
        ...
        <td><div style="display:flex;gap:2px">
          ${canEdit()?`<button class="btn btn-edit btn-xs" onclick="window._op.openAgendaModal('${a.id}')">✏️</button>`:''}
          ...
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${a.id}','agenda')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
...
function renderVehiculos() {
  ...
  let h = `<div class="sec-hdr">
    <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;width:100%">
      <div class="sbox" style="flex:1;min-width:200px"><span class="sico">🔍</span><input type="text" placeholder="Buscar matrícula, usuario, acción..." value="${esc(iF.qHist||'')}" oninput="window._iF.qHist=this.value;window._op.renderVehiculos()"></div>
      <span style="font-size:11px;color:var(--text3)">${filtered.length} registros</span>
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'editHistory\')">⬇ Excel</button>':''}
      ${isSA()?'<button class="btn btn-danger btn-sm" onclick="window._op.vaciarHistorial()">💥 Vaciar</button>':''}
    </div>
  </div>`;
...
function renderAuditoria() {
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;gap:4px;margin-bottom:8px;flex-wrap:wrap">
      ...
      ${isSA()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportAuditLog()">⬇ Excel</button>':''}
    </div>
    ${subContent}`;
}
...
function renderPapelera() {
  ...
  let h = `<div class="sec-hdr">
    <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;width:100%">
      <div class="sbox" style="flex:1;min-width:160px"><span class="sico">🔍</span><input type="text" placeholder="Buscar..." value="${esc(iF.qPap||'')}" oninput="window._iF.qPap=this.value;window._op.renderPapelera()"></div>
      <span style="font-size:11px;color:var(--text3)">${sorted.length} elementos</span>
      ${isSA()?'<button class="btn btn-danger btn-sm" onclick="window._op.vaciarPapelera()">💥 Vaciar papelera</button>':''}
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'papelera\')">⬇ Excel</button>':''}
    </div>
  </div>`;
  ...
  if (!items.length) { h += '<div class="empty"><div class="ei">🏟</div><div class="et">Sin recintos configurados</div></div>'; }
    ...
    items.forEach(r => {
      h += `<div class="card" style="position:relative">
        ...
        ${isSA()?`<div style="display:flex;gap:4px;margin-top:6px"><button class="btn btn-edit btn-xs" onclick="window._op.openRecintoModal('${r.id}')">✏️ Editar</button><button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${r.id}','recintos')">🗑</button></div>`:''}
      </div>`;
    });
    h += '</div>';
  }
  ...
  if (!items.length) { h += '<div class="empty"><div class="ei">📅</div><div class="et">Sin eventos configurados</div></div>'; }
    ...
    items.forEach(ev => {
      ...
      h += `<tr style="${isAct?'background:var(--gll)':''}">
        ...
        <td><div style="display:flex;gap:2px;flex-wrap:wrap">
          ${!isAct&&isSup()?`<button class="btn btn-s btn-xs" onclick="window._op.activarEvento('${ev.id}')">▶ Activar</button>`:''}
          ${isAct&&isSA()?`<button class="btn btn-warning btn-xs" onclick="window._op.desactivarEvento()">⏹ Desactivar</button>`:''}
          ${isSA()?`<button class="btn btn-edit btn-xs" onclick="window._op.openEventoModal('${ev.id}')">✏️</button>`:''}
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${ev.id}','eventos')">🗑</button>`:''}
        </div></td>
      </tr>`;
    });
    h += '</tbody></table></div>';
...
function _renderMensajesInline(el) {
  ...
  let h = `<div class="sec-hdr">
    ...
    <div class="sec-act">
      ...
      ${canExport()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'mensajesRampa\')">⬇ Excel</button>':''}
    </div>
  </div>`;
    ...
    msgs.forEach(m => {
      ...
      h += `<div style="padding:10px 14px;border-radius:var(--r2);border:1.5px solid var(--border);background:${typeColors[m.tipo]||'var(--bg2)'};${unread?'border-left:4px solid var(--blue)':''}">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
          ...
          ${isSA()?`<button class="btn btn-danger btn-xs" onclick="window._op.eliminar('${m.id}','mensajesRampa')">🗑</button>`:''}
        </div>
...
function renderUsuarios() {
  ...
  <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px;align-items:center">
    <div class="sbox" style="flex:1;min-width:160px"><span class="sico">🔍</span><input type="text" placeholder="Buscar nombre, usuario, email..." value="${esc(q)}" oninput="window._uQ=this.value;window._op.renderUsuarios()"></div>
    ...
    ${isSA()?'<button class="btn btn-gh btn-sm" onclick="window._op.exportExcel(\'usuarios\')">⬇ Excel</button>':''}
  </div>`;
  ...
  if (isSA()) h += `<div style="margin-top:24px;padding:14px;background:var(--rll);border:1.5px solid #fecaca;border-radius:var(--r2)"><div style="font-size:12px;font-weight:800;color:var(--red);margin-bottom:4px">⚠️ Zona peligrosa</div><button class="btn btn-danger btn-sm" onclick="window._op.resetAllData()">🗑 Borrar TODOS los datos del evento</button></div>`;
  el.innerHTML = h;
}
...
async function activarEvento(id) {
  DB.activeEventId = id;
  AppState.set('currentEvent', DB.eventos.find(e => e.id === id) || null);
...
function openIngModal(id, col = 'ingresos') {
  ...
  _modal(existing ? 'Editar ingreso' : 'Nueva entrada', `
    <div class="sg sg2">
      ...
      <div class="fg"><label class="flbl">Hall</label><input id="mHall" value="${esc(i.hall||'')}" placeholder="5"></div>
      <div class="fg"><label class="flbl">Stand</label><input id="mStand" value="${esc(i.stand||'')}" placeholder="A-101"></div>
      <div class="fg"><label class="flbl">Llamador/Referencia</label><input id="mRef" value="${esc(i.referencia||i.llamador||'')}" placeholder="REF-001"></div>
      ...
      <div class="fg"><label class="flbl">Idioma conductor</label>
        <select id="mLang"><option value="es">Español</option><option value="en">English</option><option value="fr">Français</option><option value="de">Deutsch</option><option value="it">Italiano</option><option value="pt">Português</option><option value="zh">中文</option><option value="ar">عربي</option></select>
      </div>
    </div>
  ...
  `, async () => {
    ...
    const data = {
      ...
      stand: gv('mStand'), referencia: gv('mRef'), llamador: gv('mRef'),
      montador: gv('mMont'), expositor: gv('mExpo'),
      telefono: gv('mTel'), lang: gv('mLang') || 'es',
  ...
  if (existing) { document.getElementById('mLang').value = i.lang || 'es'; }
}
...
function openUserModal(id) {
  ...
  _modal(existing ? 'Editar usuario' : 'Nuevo usuario', `
    <div class="sg sg2">
      <div class="fg"><label class="flbl">Nombre <span class="freq">*</span></label><input id="uNom" value="${esc(u.nombre||'')}"></div>
      <div class="fg"><label class="flbl">Email</label><input id="uEmail" type="email" value="${esc(u.email||'')}" placeholder="user@example.com"></div>
      <div class="fg"><label class="flbl">Rol</label>
        <select id="uRol">
          ${['superadmin','supervisor','controlador_rampa','editor','visor','empresa'].map(r=>`<option value="${r}" ${u.rol===r?'selected':''}>${{superadmin:'⭐ SuperAdmin',supervisor:'🔑 Supervisor',controlador_rampa:'🚦 Controlador Rampa',editor:'✏️ Editor',visor:'👁 Visor',empresa:'🏢 Empresa'}[r]||r}</option>`).join('')}
        </select>
      </div>
      <div class="fg"><label class="flbl">Idioma</label>
        <select id="uLang">${['es','en','fr','de','it','pt','ar','pl','ro','nl','hu','cs'].map(l=>`<option value="${l}" ${(u.lang||'es')===l?'selected':''}>${l.toUpperCase()}</option>`).join('')}</select>
      </div>
      ${!existing?`<div class="fg"><label class="flbl">PIN (mín. 4 dígitos)</label><input id="uPin" type="password" maxlength="8" inputmode="numeric" placeholder="----"></div>`:''}
    </div>
    <div class="fg"><label class="flbl" style="margin-bottom:4px">Pestañas habilitadas</label>
      <div style="display:flex;flex-wrap:wrap;gap:4px" id="uTabsWrap">
        ${TAB_DEFS.map(t=>`<label style="display:flex;align-items:center;gap:3px;font-size:11px;cursor:pointer"><input type="checkbox" class="uTabChk" value="${t.id}" ${(u.tabs||DEFAULT_TABS[u.rol||'visor']||[]).includes(t.id)?'checked':''}>${t.ico} ${t.label}</label>`).join('')}
      </div>
    </div>
  `, async () => {
    const nom = gv('uNom'); if (!nom) { toast('Nombre obligatorio','var(--red)');return; }
    const rol = gv('uRol');
    const tabs = [...document.querySelectorAll('.uTabChk:checked')].map(x=>x.value);
    const pin = gv('uPin');
    const data = { id:u.id||uid(), nombre:nom, email:gv('uEmail'), rol, lang:gv('uLang'), tabs };
    ...
    if (pin && pin.length >= 4) {
      ...
      data.pinSalt = salt;
      data.pin = null; // clear legacy
    }
    if (!DB.usuarios.find(x=>x.id===data.id)) DB.usuarios.push(data); else { const idx=DB.usuarios.findIndex(x=>x.id===data.id); if(idx>=0)DB.usuarios[idx]=data; }
