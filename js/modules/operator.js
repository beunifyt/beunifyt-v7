...
<div id="appHdr" style="background:var(--bg2);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 12px;height:44px;position:sticky;top:0;z-index:200">
  ...
  <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
    <button id="btnTheme" style="border:1px solid var(--border2);border-radius:20px;background:var(--bg3);color:var(--text2);font-size:11px;padding:3px 10px;cursor:pointer;display:flex;align-items:center;gap:4px" onclick="window._op.cycleTheme()"><span id="themeIcon">☀️</span> <span id="themeLbl">Tema</span></button>
...
function renderAnalytics() {
  ...
  <div class="sg sg3">
    ...
    <div class="card"><div style="font-weight:800;font-size:12px;margin-bottom:6px">🚗 Tipo vehículo</div>${barChart(byTipo,'#3b82f6','')}</div>
...
function _anlSmartInsights() {
  ...
  if (!total) { toast('Sin datos para analizar','var(--amber)'); return; }
...
function openIngModal(id, col = 'ingresos') {
  ...
  _modal(existing ? 'Editar ingreso' : 'Nueva entrada', `
    <div class="sg sg2">
      <div class="fg"><label class="flbl">Matrícula <span class="freq">*</span></label><div style="display:flex;gap:4px"><input id="mMat" value="${esc(i.matricula||'')}" style="text-transform:uppercase;flex:1" placeholder="AB1234CD"><button type="button" class="btn btn-gh" onclick="window._op.openCamScan()" style="padding:4px 10px;flex-shrink:0" title="Escanear matrícula">📷</button></div></div>
...
function cycleTheme() {
  ...
  _applyTheme(next);
  toast(`${THEME_ICONS[next]} ${THEME_NAMES[next] || next}`, 'var(--blue)');
}
...
<div style="max-width:700px">
  ...
  <div style="background:${totalDupes?'var(--all)':'var(--bg3)'};border:1px solid ${totalDupes?'#fde68a':'var(--border)'};border-radius:var(--r2);padding:14px;margin-bottom:12px">
    ...
    ${totalDupes ? `<div style="font-size:12px;color:#92400e;margin-bottom:6px">⚠️ ${totalDupes} posibles duplicados encontrados</div>` : '<div style="font-size:12px;color:var(--green)">✅ Sin duplicados detectados</div>'}
    <div style="display:flex;flex-wrap:wrap;gap:4px">${dupeResults.filter(c=>c.total>0).map(c=>`<span class="pill" style="font-size:10px;background:${c.dupes?'var(--rll)':'var(--bg2)'};border:1px solid ${c.dupes?'#fecaca':'var(--border)'}">${c.label}: ${c.total} reg. ${c.dupes?'('+c.dupes+' dupl.)':''}</span>`).join('')}</div>
  </div>

  <!-- EMAIL BACKUP OPTION -->
  <div style="background:var(--bll);border:1px solid #bfdbfe;border-radius:var(--r2);padding:14px;margin-bottom:16px">
    <div style="font-weight:700;margin-bottom:6px">📧 Enviar copia por email</div>
    <div style="display:flex;gap:6px;align-items:center">
      <input id="migEmail" type="email" placeholder="tu@email.com" value="${esc(AppState.get('currentUser')?.email||'')}" style="flex:1;font-size:12px">
      <button class="btn btn-p btn-sm" onclick="window._op._sendBackupEmail()">📨 Enviar</button>
    </div>
    <div style="font-size:10px;color:var(--text3);margin-top:4px">Se descargará el backup y se abrirá tu cliente de email con el archivo adjunto</div>
  </div>
  ...
  <div style="margin-top:16px;padding:12px;background:var(--rll);border:1.5px solid #fecaca;border-radius:var(--r2);font-size:12px">
    ⚠️ La importación <b>añade</b> registros. Para reemplazar, vacía la colección primero.
  </div>
</div>`;
}
...
function _sendBackupEmail() {
  ...
  toast('📨 Backup descargado. Adjúntalo al email.', 'var(--blue)');
}
...
window._op = {
  ...
  resetAllData, openLangPicker, setLang, handleLogout, cycleTheme, _anlExport, _anlSmartInsights,
  renderMigracion, exportarTodo, importarTodo, _sendBackupEmail,
  ...
  dlTemplateEmpresas, dlTemplateUsuarios,
};
