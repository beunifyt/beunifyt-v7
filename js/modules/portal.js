...
export async function initPortal() {
  ...
  // Allow SA preview mode OR actual empresa user
  if (user.rol !== 'empresa' && !user._saPreview) { logout(); return; }
  ...
  _renderShell();
  if (user._saPreview) {
    const b = document.getElementById('pSAPreviewBanner'); if(b) b.style.display='block';
  }
  await _loadData();
  _renderTab('empresa');
...
async function _loadData() {
  ...
  try {
    ...
    const _activeEvCfg = await fsGet('config/activeEvent');
    const activeEv = _eventos.find(e => e.id === _activeEvCfg?.id);
    if (activeEv) {
...
:root{--bg:#f8f9fc;--bg2:#fff;--bg3:#f1f3f8;--bg4:#e6e9f0;--text:#0c1222;--text2:#334155;--text3:#64748b;--border:#e2e5ef;--border2:#c8cdd9;--blue:#2563eb;--bll:#eef2ff;--green:#059669;--gll:#ecfdf5;--red:#dc2626;--rll:#fef2f2;--amber:#d97706;--all:#fffbeb;--teal:#0d9488;--r:8px;--r2:12px}
body{font-family:'Inter',system-ui,-apple-system,sans-serif;font-size:13px;background:#f0f2f7;color:var(--text);min-height:100vh;-webkit-font-smoothing:antialiased}
input,select,textarea{font-family:inherit;font-size:13px;outline:none;padding:9px 14px;border:1.5px solid var(--border2);border-radius:var(--r);background:var(--bg2);color:var(--text);transition:border-color .15s}
...
.btn{padding:9px 20px;font-size:13px;border-radius:20px}
...
.btn-s{background:var(--green);color:#fff}.btn-s:hover{background:#047857}
...
/* ── Executive Header ── */
#pHdr{display:flex;align-items:center;gap:12px;padding:0 24px;height:56px;background:linear-gradient(135deg,#0f172a 0%,#1e293b 100%);flex-shrink:0;position:sticky;top:0;z-index:100;box-shadow:0 2px 12px rgba(0,0,0,.15)}
#pHdr .logo-mark{display:flex;align-items:center;gap:10px}
#pHdr .logo-txt{font-family:'Oxanium',monospace;font-size:17px;font-weight:700;color:#fff;letter-spacing:-.3px}
#pHdr .logo-txt .t{color:#00ffc8}
#pHdr .badge-portal{background:rgba(0,255,200,.12);border:1px solid rgba(0,255,200,.25);color:#00ffc8;padding:3px 12px;border-radius:20px;font-size:10px;font-weight:800;letter-spacing:.5px;text-transform:uppercase}
#pHdr .user-area{display:flex;align-items:center;gap:8px}
#pHdr .user-name{font-size:13px;font-weight:600;color:#e2e8f0}
#pHdr .hdr-btn{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);color:#cbd5e1;border-radius:20px;padding:5px 12px;font-size:11px;font-weight:600;transition:all .15s}
#pHdr .hdr-btn:hover{background:rgba(255,255,255,.15);color:#fff}
/* ── Tabs ── */
#pTabs{display:flex;align-items:center;gap:0;padding:0 16px;background:var(--bg2);border-bottom:2px solid var(--border);overflow-x:auto;scrollbar-width:none}
#pTabs::-webkit-scrollbar{display:none}
.p-tab{padding:14px 18px;font-size:12px;font-weight:600;color:var(--text3);border-bottom:2.5px solid transparent;cursor:pointer;white-space:nowrap;display:flex;align-items:center;gap:6px;margin-bottom:-2px;transition:all .15s;letter-spacing:.2px}
.p-tab:hover{color:var(--text);background:var(--bg3)}
.p-tab.active{color:var(--blue);border-bottom-color:var(--blue);font-weight:700;background:transparent}
/* ── Body ── */
#pBody{max-width:960px;margin:0 auto;padding:24px 20px}
/* ── Cards ── */
.p-card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--r2);padding:24px;margin-bottom:16px;box-shadow:0 1px 4px rgba(0,0,0,.04)}
.p-card-title{font-size:17px;font-weight:800;margin-bottom:20px;display:flex;align-items:center;gap:10px;letter-spacing:-.2px}
/* ── Forms ── */
.fg{margin-bottom:14px}.fg label{display:block;font-size:10px;font-weight:700;color:var(--text3);margin-bottom:5px;text-transform:uppercase;letter-spacing:.6px}
.fg input,.fg select,.fg textarea{width:100%}
.sg2{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.sg3{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}
/* ── Vehicle cards ── */
.veh-card{border:1.5px solid var(--border);border-radius:var(--r2);padding:14px 16px;display:flex;align-items:flex-start;gap:12px;background:var(--bg2);transition:all .15s}
.veh-card:hover{border-color:var(--blue);box-shadow:0 2px 8px rgba(37,99,235,.08)}
.mchip{display:inline-flex;align-items:center;background:#0f172a;color:#e2f8f0;border-radius:6px;padding:4px 10px;font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:700;letter-spacing:.5px}
.mchip-sm{display:inline-flex;align-items:center;background:#0f172a;color:#e2f8f0;border-radius:4px;padding:2px 7px;font-family:'JetBrains Mono',monospace;font-size:11px;font-weight:700}
/* ── Status badges ── */
.sbadge{display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;letter-spacing:.3px}
.st-verif{background:var(--gll);color:var(--green);border:1px solid #a7f3d0}
.st-semi{background:var(--bll);color:var(--blue);border:1px solid #93c5fd}
.st-bloq{background:var(--rll);color:var(--red);border:1px solid #fca5a5}
.st-fira{background:var(--gll);color:var(--green);border:1px solid #a7f3d0}
.st-park{background:var(--all);color:var(--amber);border:1px solid #fcd34d}
.st-esp{background:var(--bll);color:var(--blue);border:1px solid #93c5fd}
.st-none{background:var(--bg3);color:var(--text3);border:1px solid var(--border)}
.live{display:inline-block;width:8px;height:8px;border-radius:50%;background:#22c55e;animation:pulse 1.5s infinite;margin-right:4px}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}
/* ── Tables ── */
.tbl-wrap{overflow-x:auto;border-radius:var(--r2);border:1px solid var(--border)}
.dtbl{width:100%;border-collapse:collapse;font-size:12px}
.dtbl thead{background:var(--bg3)}
.dtbl th{padding:10px 12px;font-weight:700;font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1.5px solid var(--border);text-align:left;white-space:nowrap}
.dtbl td{padding:10px 12px;border-bottom:1px solid var(--border);vertical-align:middle}
.dtbl tbody tr:hover{background:rgba(37,99,235,.02)}
.dtbl tbody tr:last-child td{border-bottom:none}
/* ── Modal ── */
.modal-bg{position:fixed;inset:0;background:rgba(15,23,42,.5);z-index:500;display:flex;align-items:center;justify-content:center;padding:16px;backdrop-filter:blur(2px)}
.modal-box{background:var(--bg2);border-radius:var(--r2);padding:24px;width:100%;max-width:540px;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.2)}
.modal-hdr{display:flex;align-items:center;margin-bottom:16px}
/* ── Empty state ── */
.empty{display:flex;flex-direction:column;align-items:center;padding:56px 20px;color:var(--text3);text-align:center}
.empty-ico{font-size:52px;margin-bottom:12px}
.empty-txt{font-size:17px;font-weight:700}
/* ── RGPD ── */
.rgpd-body{max-height:240px;overflow-y:auto;border:1px solid var(--border);border-radius:var(--r);padding:14px;background:var(--bg3);font-size:12px;line-height:1.7;color:var(--text2);margin-bottom:14px}
@media(max-width:640px){.sg2,.sg3{grid-template-columns:1fr}#pHdr{padding:0 12px}#pBody{padding:16px 12px}}
</style>

<div id="pHdr">
  <div class="logo-mark">
    <svg viewBox="0 0 140 140" width="30" height="30" style="flex-shrink:0">
      ...
      <circle cx="70" cy="70" r="9" fill="#00ffc8"/><circle cx="70" cy="70" r="3.5" fill="#030812"/>
    </svg>
    <span class="logo-txt"><span class="t">Be</span>Unify<span class="t">T</span></span>
    <span class="badge-portal">Portal</span>
  </div>
  <span style="flex:1"></span>
  <div class="user-area">
    <span id="pSyncStatus" style="font-size:11px;color:#64748b"></span>
    <span class="user-name">${esc(user?.nombre || '')}</span>
    <div style="position:relative">
      <button class="hdr-btn" id="pLangBtn" onclick="document.getElementById('pLangMenu').style.display=document.getElementById('pLangMenu').style.display==='none'?'block':'none'">
        🌐 <span id="pLangCode">${_lang().toUpperCase()}</span> ▾
      </button>
      <div id="pLangMenu" style="display:none;position:absolute;right:0;top:calc(100% + 6px);background:#1e293b;border:1px solid #334155;border-radius:10px;padding:6px;z-index:9999;min-width:140px;box-shadow:0 12px 36px #0006;max-height:300px;overflow-y:auto">
        ${LANGS.map(l => `<div onclick="window._portal.setLang('${l}')" style="padding:7px 12px;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;color:#e2e8f0;transition:background .1s" onmouseover="this.style.background='#334155'" onmouseout="this.style.background='transparent'">${l.toUpperCase()}</div>`).join('')}
      </div>
    </div>
    <button class="hdr-btn" onclick="window._portal.handleLogout()" style="margin-left:4px">
...
<div id="pSAPreviewBanner" style="display:none;background:linear-gradient(90deg,#f59e0b,#d97706);color:#fff;padding:8px 20px;font-size:12px;font-weight:700;text-align:center;letter-spacing:.3px">
  👁 Vista previa SA — <button onclick="window._portal.exitPreview()" style="background:rgba(0,0,0,.2);border:none;color:#fff;padding:3px 14px;border-radius:10px;cursor:pointer;font-weight:700;font-size:12px;margin-left:8px">← Volver al operador</button>
</div>
<div id="pTabs">
  ...
  <div class="p-tab" id="pt-pass" onclick="window._portal.goTab('pass')"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg> Contraseña</div>
</div>
...
function _renderTab(tab) {
  ...
  else if (tab === 'pass')      { body.innerHTML = _buildPassTab(); }
}
...
// ─── PUBLIC API ───────────────────────────────────────────────────────
function _buildPassTab() {
  const EYE = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
  return `<div style="max-width:400px;margin:0 auto;padding:16px 0">
    ...
    <div style="display:flex;flex-direction:column;gap:10px">
      <div>
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;color:var(--text3);margin-bottom:4px">Contraseña actual</div>
        <div style="position:relative;display:flex;align-items:center">
          <input id="passOld" type="password" style="width:100%;padding:8px 36px 8px 10px;border:1px solid var(--border2);border-radius:8px;font-size:13px;background:var(--bg2)">
          <button onclick="(function(){var i=document.getElementById('passOld');i.type=i.type==='password'?'text':'password';})()" style="position:absolute;right:8px;background:none;border:none;cursor:pointer;color:var(--text3);display:flex">${EYE}</button>
        </div>
      </div>
      <div>
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;color:var(--text3);margin-bottom:4px">Nueva contraseña</div>
        <div style="position:relative;display:flex;align-items:center">
          <input id="passNew" type="password" style="width:100%;padding:8px 36px 8px 10px;border:1px solid var(--border2);border-radius:8px;font-size:13px;background:var(--bg2)">
          <button onclick="(function(){var i=document.getElementById('passNew');i.type=i.type==='password'?'text':'password';})()" style="position:absolute;right:8px;background:none;border:none;cursor:pointer;color:var(--text3);display:flex">${EYE}</button>
        </div>
      </div>
      <div>
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;color:var(--text3);margin-bottom:4px">Confirmar nueva contraseña</div>
        <div style="position:relative;display:flex;align-items:center">
          <input id="passNew2" type="password" style="width:100%;padding:8px 36px 8px 10px;border:1px solid var(--border2);border-radius:8px;font-size:13px;background:var(--bg2)">
          <button onclick="(function(){var i=document.getElementById('passNew2');i.type=i.type==='password'?'text':'password';})()" style="position:absolute;right:8px;background:none;border:none;cursor:pointer;color:var(--text3);display:flex">${EYE}</button>
        </div>
      </div>
      <button onclick="window._portal.changePassword()" style="background:#2563eb;color:#fff;border:none;border-radius:8px;padding:10px;font-size:13px;font-weight:700;cursor:pointer;margin-top:4px">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
        Cambiar contraseña
      </button>
    </div>
  </div>`;
...
const _api = {
  ...
  exitPreview() {
    ...
    if (user?._saUser) {
      AppState.set('currentUser', user._saUser);
  ...
  async changePassword() {
    ...
    if (!oldP || !newP || !newP2) { toast('Todos los campos son obligatorios', 'var(--red)'); return; }
    if (newP !== newP2) { toast('Las contraseñas no coinciden', 'var(--red)'); return; }
    if (newP.length < 8) { toast('Mínimo 8 caracteres', 'var(--red)'); return; }
    try {
      const { reauthenticateWithCredential, EmailAuthProvider, updatePassword } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js');
      const { getBEUAuth } = await import('../firestore.js');
      const user = getBEUAuth().currentUser;
      const cred = EmailAuthProvider.credential(user.email, oldP);
      await reauthenticateWithCredential(user, cred);
      await updatePassword(user, newP);
      toast('✅ Contraseña cambiada', 'var(--green)');
