// BeUnifyT v8 — tabs/conductores.js
...
export function renderConductores() {
  ...
  el.innerHTML=`
    <div style="display:flex;align-items:center;gap:3px;padding:4px 0;flex-wrap:wrap;min-height:34px;border-bottom:1px solid var(--border);margin-bottom:4px;overflow-x:auto;scrollbar-width:none">
      <div class="sbox" style="flex:2;min-width:140px"><span class="sico">🔍</span><input type="search" placeholder="Nombre, matrícula, empresa..." value="${cF.q||''}" oninput="cF.q=this.value;debounceSearch('conductores',renderConductores)"></div>
      ${cF.q?`<span class="pill pill-r" onclick="cF.q='';renderConductores()">✕</span>`:''}
      <span style="font-size:11px;color:var(--text3)">${items.length} conductores</span>
      <span style="flex:1"></span>
      ${canAdd()?`<button class="btn btn-sm" style="background:#0d9f6e;color:#fff;border:none;border-radius:20px;font-weight:700" onclick="openCondModal()">+ Añadir</button>`:''}
    ...
    ${items.length?`<div class="tbl-wrap"><table class="dtbl"><thead><tr>${thSort('conductores','matricula','Matrícula')}${thSort('conductores','nombre','Nombre')}${thSort('conductores','empresa','Empresa')}<th>Teléfono</th><th>Hall</th><th>Idioma</th><th>Acc.</th></tr></thead><tbody>
      ${items.map(c=>{const l=LANGS_UI.find(x=>x.code===(c.idioma||''));return`<tr>
        <td>${c.matricula?`<span class="mchip" style="cursor:pointer" onclick="showCondDetalle('${c.id}')">${c.matricula}</span>`:'-'}</td>
        <td><b style="font-size:12px">${c.nombre} ${c.apellido||''}</b></td>
