...
// Adaptador Firestore. Reemplaza Firebase Realtime Database.
// Sin límite de conexiones simultáneas (resuelve el problema MWC).
...
export const fsCompany = {
  ...
  // Lectura unica sin listener — evita conexiones permanentes
...
// ══════════════════════════════════════════════════════════════
// Puerta — listener en tiempo real solo para operadores
// Max 35 conexiones simultaneas (MWC: 35 operadores)
// ══════════════════════════════════════════════════════════════
export const fsGate = {
...
export const fsOperators = {
  async list(eventId) {
    const { collection, getDocs } = await _fs();
    const snap = await getDocs(
      collection(_db, 'events', eventId, 'operators')
    );
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  },
...
// Helper: importar Firestore SDK on-demand
async function _fs() {
