...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Inter', system-ui, sans-serif; background: linear-gradient(135deg,#1e3a5f 0%,#0f172a 100%); color: var(--text, #0f172a); display: flex; align-items: center; justify-content: center; min-height: 100vh; overflow: auto; margin: 0; padding: 16px; }
...
<div class="login-wrap">
  ...
  <div class="login-footer">BeUnifyT v7 · 
    <select id="loginLangSel" onchange="window._auth.setLangDirect(this.value)" style="border:none;background:transparent;font-size:11px;color:var(--text3,#6b7280);cursor:pointer;font-family:inherit">
...
<div class="card">
  ...
  <div class="step on" id="rs1">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:16px">
      ${LANGS_UI.filter(l=>LANGS.includes(l.code)).map(l=>'<button onclick="window._regLang=\''+l.code+'\';window._auth.showRegister()" class="lang-btn" style="padding:5px 10px;border:1.5px solid #d1d5db;border-radius:20px;cursor:pointer;font-size:12px;background:'+(l.code===lang?'#dbeafe':'#fff')+';display:flex;align-items:center;gap:4px"><span>'+l.flag+'</span> '+l.name+'</button>').join('')}
    </div>
