...
import { getAuth, fsGet, fsSet, fsUpdate } from './firestore.js';
...
async function _loginEmail() {
  ...
  try {
    ...
    const cred = await signInWithEmailAndPassword(getAuth(), ident, pass);
    const fbUser = cred.user;

    // Load user record from Firestore
    const userDoc = await fsGet(`users/${fbUser.uid}`);
    if (!userDoc) {
      // Try legacy: match by email in users collection (migrated from v6)
      const legacy = await _findUserByEmail(ident);
      if (!legacy) { _loginFail(errEl); return; }
      _onUserLoaded(legacy);
    } else {
    ...
    if (code.includes('invalid-credential') || code.includes('wrong-password') || code.includes('user-not-found')) {
      ...
      _showErr(errEl, 'Error de conexión. Inténtalo de nuevo.');
...
function _onAuthSuccess() {
  ...
  if (user.rol === 'empresa') {
    import('./modules/portal.js').then(m => m.initPortal());
  } else {
    import('./modules/operator.js').then(m => m.initOperator());
  }
}
