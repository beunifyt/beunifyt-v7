...
function _setLangDirect(code) {
  ...
  try { localStorage.setItem('beu_lang', code); } catch {}
  _showLogin();
} catch {}
  _showLogin();
  // Restore form values
...
<div class="card">
  ...
  <div class="step on" id="rs1">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:16px">
      \${LANGS_UI.filter(l=>LANGS.includes(l.code)).map(l=>`<button onclick="window._regLang='\${l.code}';AppState.set('currentLang','\${l.code}');try{localStorage.setItem('beu_lang','\${l.code}');}catch(e){}window._auth.showRegister()" class="lang-btn" style="padding:5px 10px;border:1.5px solid #d1d5db;border-radius:20px;cursor:pointer;font-size:12px;background:\${l.code===lang?'#dbeafe':'#fff'};font-weight:\${l.code===lang?'900':''};display:flex;align-items:center;gap:4px"><span>\${l.flag}</span><span>\${l.name}</span></button>`).join('')}
    </div>
