...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .login-card { background: #fff; border-radius: 16px; padding: 32px 28px; width: 100%; max-width: 420px; box-shadow: 0 20px 60px rgba(0,0,0,.4); max-height: 90vh; overflow-y: auto; color: #111; }
  .login-logo { display: flex; align-items: center; justify-content: center; gap: 10px; margin-bottom: 24px; }
  .login-logo span { font-family: 'Oxanium', monospace; font-size: 22px; font-weight: 700; color: #0f172a; }
  .login-logo .teal { color: #00b89a; }
  .login-card input { width: 100%; padding: 10px 14px; border: 1.5px solid #e2e8f0; border-radius: 8px; font-size: 14px; margin-bottom: 12px; background: #fff; color: #111; font-family: inherit; outline: none; }
  .login-card input::placeholder { color: #9ca3af; }
  .login-card input:focus { border-color: #3b82f6; box-shadow: 0 0 0 3px rgba(59,130,246,.1); }
  .btn-login { width: 100%; padding: 11px; border: none; border-radius: 20px; background: #2563eb; color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; font-family: inherit; transition: background .15s; }
  .btn-login:hover { background: #1d4ed8; }
  .btn-login:disabled { opacity: .5; cursor: not-allowed; }
  .btn-pin { width: 100%; margin-top: 10px; padding: 10px; border: 1.5px solid #e2e8f0; border-radius: 20px; background: #f8fafc; color: #374151; font-size: 13px; font-weight: 600; cursor: pointer; font-family: inherit; transition: all .15s; }
  .btn-pin:hover { background: #e0f2fe; border-color: #7dd3fc; }
  ...
  .pdot { width: 14px; height: 14px; border-radius: 50%; border: 2px solid #2563eb; background: transparent; transition: background .15s; }
  .pdot.filled { background: #2563eb; }
  .login-footer { margin-top: 16px; text-align: center; font-size: 11px; color: rgba(255,255,255,.5); }
  .login-footer select { border: none; background: transparent; font-size: 11px; color: rgba(255,255,255,.6); cursor: pointer; font-family: inherit; }
  .login-footer select option { color: #333; background: #fff; }
</style>

<div class="login-wrap">
  <div class="login-card">
    <div class="login-logo">
      <svg viewBox="0 0 140 140" width="32" height="32"><rect width="140" height="140" rx="28" fill="#030812"/><polygon points="70,10 122,40 122,100 70,130 18,100 18,40" stroke="#00ffc8" stroke-width="2" fill="#00ffc808"/><polygon points="70,28 106,49 106,91 70,112 34,91 34,49" stroke="#00ffc8" stroke-width="1.2" fill="none" opacity="0.4"/><circle cx="70" cy="70" r="9" fill="#00ffc8"/><circle cx="70" cy="70" r="3.5" fill="#030812"/></svg>
  ...
  <div class="login-footer">BeUnifyT v7 · 
    <select id="loginLangSel" onchange="window._auth.setLangDirect(this.value)">
