...
import { I18N as LANG_I18N, LANGS_UI as LANG_FLAGS } from '../langs.js';
...
function _t(k){const l=AppState.get('currentLang')||'es';return(SHELL_I18N[l]&&SHELL_I18N[l][k])||SHELL_I18N.en[k]||SHELL_I18N.es[k]||k;}
function _tabLabel(tabId){const l=AppState.get('currentLang')||'es';return(LANG_I18N[l]&&LANG_I18N[l][tabId])||LANG_I18N.es[tabId]||tabId;}

// ─── SHELL I18N ──────────────────────────────────────────────────────
const SHELL_I18N = {
  es:{theme:'Tema',logout:'Salir',confirmLogout:'¿Cerrar sesión?',noEvent:'Sin evento',light:'Claro',dark:'Oscuro',soft:'Suave',contrast:'Alto contraste'},
  en:{theme:'Theme',logout:'Log out',confirmLogout:'Log out?',noEvent:'No event',light:'Light',dark:'Dark',soft:'Soft',contrast:'High contrast'},
  fr:{theme:'Thème',logout:'Quitter',confirmLogout:'Se déconnecter ?',noEvent:'Aucun événement',light:'Clair',dark:'Sombre',soft:'Doux',contrast:'Haut contraste'},
  de:{theme:'Thema',logout:'Abmelden',confirmLogout:'Abmelden?',noEvent:'Kein Event',light:'Hell',dark:'Dunkel',soft:'Sanft',contrast:'Hoher Kontrast'},
  it:{theme:'Tema',logout:'Esci',confirmLogout:'Disconnettersi?',noEvent:'Nessun evento',light:'Chiaro',dark:'Scuro',soft:'Morbido',contrast:'Alto contrasto'},
  pt:{theme:'Tema',logout:'Sair',confirmLogout:'Terminar sessão?',noEvent:'Sem evento',light:'Claro',dark:'Escuro',soft:'Suave',contrast:'Alto contraste'},
  pl:{theme:'Motyw',logout:'Wyloguj',confirmLogout:'Wylogować?',noEvent:'Brak wydarzenia',light:'Jasny',dark:'Ciemny',soft:'Łagodny',contrast:'Wysoki kontrast'},
  ro:{theme:'Temă',logout:'Ieșire',confirmLogout:'Deconectare?',noEvent:'Fără eveniment',light:'Deschis',dark:'Întunecat',soft:'Moale',contrast:'Contrast ridicat'},
};
function _shellT(key) { const lang = AppState.get('currentLang')||'es'; return (SHELL_I18N[lang]||SHELL_I18N.en)[key]||(SHELL_I18N.es)[key]||key; }
...
<div id="appHdr" style="background:var(--bg2);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 12px;height:44px;position:sticky;top:0;z-index:200">
  ...
  <div style="display:flex;align-items:center;gap:6px;flex-shrink:0">
    ...
    <span style="width:1px;height:20px;background:var(--border);margin:0 2px;display:inline-block"></span>
    <button class="btn btn-gh btn-sm" onclick="window._op.handleLogout()"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg> ${_t('logout')}</button>
  </div>
</div>
<div id="mainTabs">
...
export function toggleThemeMenu() {
  ...
  menu.innerHTML = THEME_ORDER.map(th => {
    ...
    return '<div onclick="window._op.selectTheme(\''+th+'\')" style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:6px;cursor:pointer;font-size:12px;color:var(--text);background:'+(active?'var(--bll)':'transparent')+';font-weight:'+(active?'700':'400')+'" onmouseover="this.style.background=\'var(--bg3)\'" onmouseout="this.style.background=\''+(active?'var(--bll)':'transparent')+'\'">'+
      '<span style="font-size:16px">'+THEME_ICONS[th]+'</span><span>'+_TN[th]+'</span>'+(active?'<span style="margin-left:auto;color:var(--green)">✓</span>':'')+
    '</div>';
  }).join('');
...
export function openLangPicker() {
  ...
  const langGridCSS = `
    ...
    .lang-item{display:flex;flex-direction:column;align-items:center;justify-content:center;width:64px;height:52px;border-radius:8px;cursor:pointer;border:2px solid var(--border);background:var(--bg2);transition:all .15s}
    .lang-item:hover{background:var(--bg3);border-color:var(--blue)}
    .lang-item.sel{border-color:var(--green);background:var(--gll)}
  `;
  const body = `<style>${langGridCSS}</style><div class="lang-grid">${LANG_FLAGS.map(l=>{
    const flag = l.flag.includes('<svg') ? l.flag : `<span style="font-size:22px">${l.flag}</span>`;
    return `<div class="lang-item${l.code===cur?' sel':''}" onclick="window._op.setLang('${l.code}');document.getElementById('dynModal')?.remove()" title="${l.name}"><div style="height:22px;display:flex;align-items:center;justify-content:center">${flag}</div><div style="font-size:9px;font-weight:700;margin-top:2px;color:var(--text2)">${l.name}</div></div>`;
  }).join('')}</div>`;
  const div = document.createElement('div'); div.id = 'dynModal'; div.className = 'modal-bg';
  div.innerHTML = `<div class="modal-box"><div class="modal-hdr"><div class="modal-ttl">🌐 ${_t('theme')==='Theme'?'Language':'Idioma'}</div><button style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--text3)" onclick="this.closest('.modal-bg').remove()">✕</button></div>${body}</div>`;
...
export async function setLang(lang) {
  ...
  // Re-render tabs with new language
  ...
  renderHdr();
  import('../utils.js').then(m => m.toast(`🌐 ${lang.toUpperCase()}`, 'var(--green)'));
}
