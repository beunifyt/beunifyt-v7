...
const CAMPO_GROUPS = {
  ingresos: [
    { label:'🚛 Vehículo', fields:['posicion','remolque','tipoVehiculo','descargaTipo'] },
    { label:'👤 Conductor', fields:['nombre','apellido','pasaporte','fechaNacimiento','fechaExpiracion','pais','telefono','email'] },
    { label:'📋 Evento / Logística', fields:['llamador','ref','empresa','hall','stand','puertaHall','acceso','montador','expositor','horario','comentario'] },
  ],
  ingresos2: [
    { label:'🚛 Vehículo', fields:['posicion','remolque','tipoVehiculo','descargaTipo'] },
    { label:'👤 Conductor', fields:['nombre','apellido','pasaporte','fechaNacimiento','pais','telefono','email'] },
    { label:'📋 Evento / Logística', fields:['llamador','ref','empresa','hall','stand','puertaHall','montador','expositor','comentario'] },
  ],
  agenda: [
    { label:'🚛 Vehículo', fields:['matricula','remolque','tipoCarga'] },
    { label:'👤 Conductor', fields:['conductor','telefono'] },
    { label:'📋 Evento / Logística', fields:['fecha','hora','empresa','referencia','montador','expositor','hall','stand','notas','estado'] },
  ],
  conductores: [
    { label:'🚛 Vehículo', fields:['matricula','remolque','tipoVehiculo'] },
    { label:'👤 Datos personales', fields:['nombre','apellido','pasaporte','fechaNacimiento','pais','telefono','email','idioma','notas'] },
    { label:'📋 Asignación', fields:['empresa','hall'] },
  ],
  movimientos: [
    { label:'🚛 Vehículo', fields:['posicion','matricula','remolque'] },
    { label:'👤 Conductor', fields:['nombre'] },
    { label:'📋 Logística', fields:['empresa','hall','status','tipoCarga'] },
  ],
};
...
function _renderCamposSubtab(el_unused, col) {
  ...
  const groups = CAMPO_GROUPS[col];
  if (groups) {
    groups.forEach(g => {
      h += `<div style="margin-bottom:14px">
        <div style="font-size:11px;font-weight:800;color:var(--text2);margin-bottom:6px;padding-bottom:4px;border-bottom:1px solid var(--border)">${g.label}</div>
  ...
  h += `
    <div style="margin-top:20px;padding-top:14px;border-top:1.5px solid var(--border);display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn btn-sm" style="background:var(--bg3);color:var(--text2);border:1.5px solid var(--border2);border-radius:20px;font-weight:700" onclick="window._op.resetCamposCfg('${col}')">↺ Resetear campos</button>
      ${isSA()?`<button class="btn btn-sm" style="background:var(--green);color:#fff;border:none;border-radius:20px;font-weight:700" onclick="window._op.addCustomCampo('${col}')">+ Campo personalizado</button>`:''}
    </div>
  </div>`;
  return h;
}
