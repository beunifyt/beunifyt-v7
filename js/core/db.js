...
export async function _seedIfEmpty() {
  try {
    // Check if already seeded — never re-seed after first time
    const seededFlag = await fsGet('config/seeded').catch(() => null);
    ...
    // Mark as seeded — will never run again
    await fsSet('config/seeded', { ts: nowLocal(), by: user.email || user.id }, false);
