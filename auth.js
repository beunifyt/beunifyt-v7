...
function paint() {
  ...
  _container.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;padding:16px;font-family:system-ui,sans-serif">
      <div style="background:#fff;border-radius:16px;padding:32px;width:100%;max-width:380px;box-shadow:0 4px 24px rgba(0,0,0,.08);position:relative">

        <div style="text-align:center;margin-bottom:24px">
          <div style="font-size:24px;font-weight:800;letter-spacing:-0.5px;font-style:italic">BeUnifyT</div>
          <div style="font-size:11px;color:#94a3b8;margin-top:2px;letter-spacing:1px">unified access control</div>
        </div>

        <div style="text-align:center;margin-bottom:20px;position:relative">
          <button id="langBtn" style="background:none;border:1px solid #e2e8f0;border-radius:8px;padding:6px 12px;cursor:pointer;font-size:13px;display:inline-flex;align-items:center;gap:6px">
            ${flagHtml(lang)} ${lang.toUpperCase()}
          </button>
          <div id="langDrop" style="display:none;position:absolute;top:100%;left:50%;transform:translateX(-50%);margin-top:6px;background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:6px;box-shadow:0 8px 24px rgba(0,0,0,.12);max-height:300px;overflow-y:auto;z-index:100;width:220px;flex-direction:column;gap:2px"></div>
        </div>
