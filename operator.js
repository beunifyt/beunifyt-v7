...
function goTab(tab) {
  ...
  const map = {
    ...
    analytics2:  function(){ import('./analytics2.js').then(m=>m.render(document.getElementById('tabContent'),AppState.get('currentUser'))).catch(e=>{console.error('[analytics2]',e);toast('Error cargando Event Tracker','var(--red)');});},
    analytics3:  function(){ import('./analytics3.js').then(m=>m.render(document.getElementById('tabContent'),AppState.get('currentUser'))).catch(e=>{console.error('[analytics3]',e);toast('Error cargando Precision','var(--red)');});},
    analytics4:  function(){ import('./analytics4.js').then(m=>m.render(document.getElementById('tabContent'),AppState.get('currentUser'))).catch(e=>{console.error('[analytics4]',e);toast('Error cargando AI Intel','var(--red)');});},
  ...
  try { (map[tab] || renderDash)(); } catch(e) { console.error('[goTab]', tab, e); if(tc) tc.innerHTML = '<div class="empty"><div class="ei">⚠️</div><div class="et">Error cargando '+tab+'</div><div class="es">'+e.message+'</div></div>'; }
}
...
function renderUsuarios() {
  ...
  if (!listBase.length) { h += '<div class="empty"><div class="ei">👥</div><div class="et">Sin usuarios</div></div>'; }
    ...
    const TN = { dash:'📊',ingresos:'🔖',ingresos2:'🚛',flota:'📦',conductores:'👤',agenda:'📅',analytics:'📈',analytics2:'🎯',analytics3:'📈',analytics4:'🤖',vehiculos:'📜',auditoria:'📂',papelera:'🗑',impresion:'🖨',recintos:'🏟',eventos:'📅',mensajes:'📢',usuarios:'👥' };
    listBase.forEach(u => {
  ...
  openEmpresaModal: (id) => {
    ...
    _modal(e ? 'Editar empresa' : 'Nueva empresa', `
      <div class="sg sg2">
        ...
        ${!e ? `<div class="fg"><label class="flbl">Contraseña acceso portal *</label>
          <div style="position:relative;display:flex;align-items:center">
    ...
    `, async () => {
      const nom = gv('emNom'); if (!nom) { toast('Nombre obligatorio','var(--red)'); throw new Error('cancel'); }
