...
// ── operator.js compat: mod.render(container, usuario) ──
export function render(container, usuario) {
  // operator.js passes DOM element; initImpresion expects element with id
  if (!container.id) container.id = 'impresion_host';
  initImpresion(container.id);
}
